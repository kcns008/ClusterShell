...
flowchart LR
    ...
    STORE --> CLI[CLI: clustershell rule]
...
The `DenialAggregator` (`crates/clustershell-sandbox/src/denial_aggregator.rs`) runs as a background tokio task inside the sandbox supervisor. It:
...
The flush interval defaults to 10 seconds (configurable via `CLUSTERSHELL_DENIAL_FLUSH_INTERVAL_SECS`).
...
The `mechanistic_mapper` module (`crates/clustershell-sandbox/src/mechanistic_mapper.rs`) generates draft policy recommendations deterministically, without requiring an LLM:
...
The gateway's `SubmitPolicyAnalysis` handler (`crates/clustershell-server/src/grpc.rs`) is deliberately thin:
...
Schema lives in `crates/clustershell-server/migrations/{sqlite,postgres}/003_create_policy_recommendations.sql`.
...
|--------|-------------|----------|--------|
| View rules | `clustershell rule get <name>` | `GetDraftPolicy` | List pending/approved/rejected chunks |
| Approve one | `clustershell rule approve <name> --chunk-id X` | `ApproveDraftChunk` | Merge rule into active policy, mark approved |
| Reject one | `clustershell rule reject <name> --chunk-id X` | `RejectDraftChunk` | Mark rejected (no policy change) |
| Approve all | `clustershell rule approve-all <name>` | `ApproveAllDraftChunks` | Bulk approve all pending chunks |
| History | `clustershell rule history <name>` | `GetDraftHistory` | Show timeline of proposals and decisions |
...
The `clustershell rule` command group provides review and approval:

```bash
# View pending recommendations
clustershell rule get my-sandbox

# Approve a specific chunk
clustershell rule approve my-sandbox --chunk-id abc123

# Approve all pending
clustershell rule approve-all my-sandbox

# Reject a chunk
clustershell rule reject my-sandbox --chunk-id xyz789
```
...
|---------------------|---------|-------------|
| `CLUSTERSHELL_DENIAL_FLUSH_INTERVAL_SECS` | `10` | How often the aggregator flushes and submits proposals |
| `CLUSTERSHELL_POLICY_POLL_INTERVAL_SECS` | `10` | How often the sandbox polls for policy updates |
