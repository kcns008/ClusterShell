...
| `grpc_client.rs` | gRPC client for fetching policy, provider environment, inference route bundles, policy polling/status reporting, proposal submission, and log push (`CachedClusterShellClient`) |
...
The `run_sandbox()` function in `crates/clustershell-sandbox/src/lib.rs` is the main orchestration entry point. It executes the following steps in order.
...
5. **TLS state for L7 inspection** (proxy mode only):
   ...
   - Write CA cert PEM and combined bundle (system CAs + sandbox CA) to `/etc/clustershell-tls/`
...
9. **Child process spawning** (`ProcessHandle::spawn()`):
   ...
   - Set environment variables: `CLUSTERSHELL_SANDBOX=1`, provider credentials, proxy URLs, TLS trust store paths
...
Policy data structures live in `crates/clustershell-sandbox/src/policy.rs`.
...
flowchart LR
    ...
    subgraph "gRPC mode (production)"
        F[CLUSTERSHELL_SANDBOX_ID] --> H[grpc_client::fetch_policy]
        G[CLUSTERSHELL_ENDPOINT] --> H
...
The OPA engine lives in `crates/clustershell-sandbox/src/opa.rs` and uses the `regorus` crate -- a pure-Rust Rego evaluator with no external OPA daemon dependency.

### Baked-in rules

The Rego rules are compiled into the binary via `include_str!("../data/sandbox-policy.rego")`. The package is `clustershell.sandbox`. Key rules:
...
1. `data.clustershell.sandbox.allow_network` -> bool
2. `data.clustershell.sandbox.deny_reason` -> string
3. `data.clustershell.sandbox.matched_network_policy` -> string (or `Undefined`)

Returns `PolicyDecision { allowed, reason, matched_policy }`.

#### `evaluate_network_action(input: &NetworkInput) -> Result<NetworkAction>`

Uses the same input JSON shape as `evaluate_network()`. Evaluates the `data.clustershell.sandbox.network_action` Rego rule, which returns one of two string values:
...
After L4 allows a connection, `query_endpoint_config(input)` evaluates `data.clustershell.sandbox.matched_endpoint_config` to get the full endpoint object. If the endpoint has a `protocol` field, `l7::parse_l7_config()` extracts the L7 config for protocol-aware inspection.
...
**File:** `crates/clustershell-sandbox/src/lib.rs` (`run_policy_poll_loop()`)
...
sequenceDiagram
    ...
    loop Every CLUSTERSHELL_POLICY_POLL_INTERVAL_SECS (default 10)
        PL->>GW: GetSandboxSettings(sandbox_id)
        GW-->>PL: policy + settings + config_revision
...
The `run_policy_poll_loop()` function in `crates/clustershell-sandbox/src/lib.rs` implements this loop:

1. **Connect once**: Create a `CachedClusterShellClient` that holds a persistent mTLS channel to the gateway. This avoids TLS renegotiation on every poll.
...
### `CachedClusterShellClient`

**File:** `crates/clustershell-sandbox/src/grpc_client.rs`

`CachedClusterShellClient` is a persistent gRPC client for the `ClusterShell` service. It wraps a `ClusterShellClient<Channel>` connected once at construction and reused for all subsequent calls.

```rust
pub struct CachedClusterShellClient {
    client: ClusterShellClient<Channel>,
}
...
- **`raw_client()`**: Return a clone of the underlying `ClusterShellClient<Channel>` for direct RPC calls (used by the log push task).
...
- `ReportPolicyStatusRequest` (`proto/clustershell.proto`): `sandbox_id`, `version`, `status` (enum), `load_error`
- `PolicyStatus` enum: `PENDING`, `LOADED`, `FAILED`, `SUPERSEDED`
- `SandboxPolicyRevision` (`proto/clustershell.proto`): Full revision metadata including `created_at_ms`, `loaded_at_ms`
...
**File:** `crates/clustershell-sandbox/src/sandbox/linux/landlock.rs`
...
**File:** `crates/clustershell-sandbox/src/sandbox/linux/seccomp.rs`
...
**File:** `crates/clustershell-sandbox/src/sandbox/linux/netns.rs`
...
**Files:** `crates/clustershell-sandbox/src/sandbox/linux/netns.rs` (`install_bypass_rules()`), `crates/clustershell-sandbox/src/bypass_monitor.rs`
...
| 4 | `-p tcp --syn -m limit --limit 5/sec --limit-burst 10 --log-prefix "clustershell:bypass:{ns}:"` | `LOG` | Log TCP SYN bypass attempts (rate-limited) |
| 5 | `-p tcp` | `REJECT --reject-with icmp-port-unreachable` | Reject TCP bypass attempts (fast-fail) |
| 6 | `-p udp -m limit --limit 5/sec --limit-burst 10 --log-prefix "clustershell:bypass:{ns}:"` | `LOG` | Log UDP bypass attempts, including DNS (rate-limited) |
| 7 | `-p udp` | `REJECT --reject-with icmp-port-unreachable` | Reject UDP bypass attempts (fast-fail) |

The LOG rules use the `--log-uid` flag to include the UID of the process that initiated the connection. The log prefix `clustershell:bypass:{namespace_name}:` enables the bypass monitor to filter `/dev/kmsg` for events belonging to a specific sandbox.
...
2. Reads lines via `BufReader`, filtering for the namespace-specific prefix `clustershell:bypass:{namespace_name}:`
...
The bypass detection subsystem is wired in `crates/clustershell-sandbox/src/lib.rs`:
...
**File:** `crates/clustershell-sandbox/src/proxy.rs`
...
When a CONNECT target is `inference.local`, the proxy TLS-terminates the client side and inspects the HTTP traffic to detect inference API calls. Matched requests are executed locally via the `clustershell-router` crate. The function `handle_inference_interception()` implements this path and returns an `InferenceOutcome`:
...
**Files:** `crates/clustershell-sandbox/src/lib.rs` (`build_inference_context`, `bundle_to_resolved_routes`, `spawn_route_refresh`), `crates/clustershell-sandbox/src/proxy.rs` (`InferenceContext`)

The sandbox executes inference requests locally using the `clustershell-router` crate. `InferenceContext` holds the router, API patterns, and a cached set of resolved routes:

```rust
pub struct InferenceContext {
    pub patterns: Vec<InferenceApiPattern>,
    router: clustershell_router::Router,
    routes: Arc<tokio::sync::RwLock<Vec<clustershell_router::config::ResolvedRoute>>>,
}
```
...
1. **Route file (standalone mode)**: `--inference-routes` / `CLUSTERSHELL_INFERENCE_ROUTES` points to a YAML file parsed by `RouterConfig::load_from_file()`. Routes are resolved via `config.resolve_routes()`. File loading or parsing errors are fatal (fail-fast), but an empty route list gracefully disables inference routing (returns `None`). The route file always takes precedence -- if both a route file and cluster credentials are present, the route file wins and the cluster bundle is not fetched.

2. **Cluster bundle (cluster mode)**: When `clustershell_endpoint` is available (and no route file is configured), routes are fetched from the gateway via `grpc_client::fetch_inference_bundle()`, which calls the `GetInferenceBundle` gRPC RPC on the `Inference` service. The RPC takes no arguments (the bundle is cluster-scoped, not per-sandbox). The gateway returns a `GetInferenceBundleResponse` containing resolved `ResolvedRoute` entries for the managed cluster route. These proto messages are converted to router `ResolvedRoute` structs by `bundle_to_resolved_routes()`, which maps provider types to auth headers and default headers via `clustershell_core::inference::auth_for_provider_type()`.
...
`ResolvedRoute` has a custom `Debug` implementation in `crates/clustershell-router/src/config.rs` that redacts the `api_key` field, printing `[REDACTED]` instead of the actual value. This prevents key leakage in log output and debug traces.
...
**Files:** `crates/clustershell-sandbox/src/l7/`
...
**File:** `crates/clustershell-sandbox/src/l7/tls.rs`

TLS termination enables the proxy to inspect HTTPS traffic by performing MITM decryption.

**Ephemeral CA lifecycle:**
1. At sandbox startup, `SandboxCa::generate()` creates a self-signed CA (CN: "ClusterShell Sandbox CA") using `rcgen`
2. The CA cert PEM and a combined bundle (system CAs + sandbox CA) are written to `/etc/clustershell-tls/`
...
**File:** `crates/clustershell-sandbox/src/l7/rest.rs`
...
- **`deny()`**: Sends an HTTP `403 Forbidden` JSON response with `Content-Type: application/json`, including the policy name, matched rule, and deny reason. Sets `Connection: close` and includes an `X-ClusterShell-Policy` header.
...
`relay_with_inspection()` in `crates/clustershell-sandbox/src/l7/relay.rs` is the main relay loop:
...
3. Evaluate `data.clustershell.sandbox.allow_request` and `data.clustershell.sandbox.request_deny_reason`
...
**File:** `crates/clustershell-sandbox/src/identity.rs`
...
**File:** `crates/clustershell-sandbox/src/procfs.rs`
...
**File:** `crates/clustershell-sandbox/src/process.rs`
...
- `CLUSTERSHELL_SANDBOX=1` (always set)
...
**File:** `crates/clustershell-sandbox/src/ssh.rs`
...
3. Set environment: `CLUSTERSHELL_SANDBOX=1`, `HOME=/sandbox`, `USER=sandbox`, `TERM={negotiated}`, proxy URLs, TLS trust store paths, provider credentials
...
`clustershell-sandbox` runs as PID 1 inside the container. In Linux, when a process exits, its parent must call `waitpid()` to collect the exit status; otherwise the process remains as a zombie. Orphaned processes (whose parent exits first) are reparented to PID 1, which becomes responsible for reaping them.
...
**File:** `crates/clustershell-sandbox/src/lib.rs`
...
|----------|----------|---------|---------|
| `CLUSTERSHELL_SANDBOX_COMMAND` | (trailing args) | `/bin/bash` | Command to execute inside sandbox |
| `CLUSTERSHELL_SANDBOX_ID` | `--sandbox-id` | | Sandbox ID for gRPC policy fetch |
| `CLUSTERSHELL_ENDPOINT` | `--clustershell-endpoint` | | Gateway gRPC endpoint |
| `CLUSTERSHELL_POLICY_RULES` | `--policy-rules` | | Path to Rego policy file |
| `CLUSTERSHELL_POLICY_DATA` | `--policy-data` | | Path to YAML data file |
| `CLUSTERSHELL_LOG_LEVEL` | `--log-level` | `warn` | Log level (trace/debug/info/warn/error) |
| `CLUSTERSHELL_POLICY_POLL_INTERVAL_SECS` | | `30` | Poll interval for gRPC policy updates (seconds). Only active in gRPC mode. |
| `CLUSTERSHELL_LOG_PUSH_LEVEL` | | `info` | Maximum tracing level for log push to gateway. Events above this level are not streamed. Only active in gRPC mode. |
| `CLUSTERSHELL_SSH_LISTEN_ADDR` | `--ssh-listen-addr` | | SSH server bind address |
| `CLUSTERSHELL_SSH_HANDSHAKE_SECRET` | `--ssh-handshake-secret` | | HMAC secret for SSH handshake |
| `CLUSTERSHELL_SSH_HANDSHAKE_SKEW_SECS` | `--ssh-handshake-skew-secs` | `300` | Allowed clock skew for handshake |
| `CLUSTERSHELL_INFERENCE_ROUTES` | `--inference-routes` | | Path to YAML inference routes file for standalone routing |

### Injected into child process

| Variable | Purpose |
|----------|---------|
| `CLUSTERSHELL_SANDBOX` | Always `"1"` -- signals the process is sandboxed |
...
- **`/var/log/clustershell.log`**: Fixed at `info` level, no ANSI, non-blocking writer
...
**File:** `crates/clustershell-sandbox/src/log_push.rs`
...
Key behaviors:
- **Level filtering**: Defaults to `INFO`. Configurable via the `CLUSTERSHELL_LOG_PUSH_LEVEL` environment variable (accepts `trace`, `debug`, `info`, `warn`, `error`). Events above the configured level are silently discarded.
...
**File:** `crates/clustershell-sandbox/src/main.rs`

The log push layer is set up in `main()` before calling `run_sandbox()`, only in gRPC mode (when both `--sandbox-id` and `--clustershell-endpoint` are present):
...
This means the push layer captures all tracing events the sandbox supervisor generates, filtered by `CLUSTERSHELL_LOG_PUSH_LEVEL` (default INFO).

### Background push task

**File:** `crates/clustershell-sandbox/src/log_push.rs` (`spawn_log_push_task()`, `run_push_loop()`)
...
2. **gRPC connection**: Connects a `CachedClusterShellClient` to the gateway. On connection failure, the task prints to stderr (cannot use tracing to avoid recursion) and exits.
...
**File:** `crates/clustershell-server/src/grpc.rs` (`push_sandbox_logs`)
...
**File:** `crates/clustershell-server/src/tracing_bus.rs`
...
**File:** `crates/clustershell-cli/src/main.rs` (command definition), `crates/clustershell-cli/src/run.rs` (`sandbox_logs()`)
...
`print_log_line()` in `crates/clustershell-cli/src/run.rs` formats each log line:
...
Example output:
```
[1708891234.567] [sandbox] [INFO ] [clustershell_sandbox::proxy] CONNECT api.example.com:443 dst_host=api.example.com action=allow
[1708891234.890] [gateway] [INFO ] [clustershell_server::grpc] ReportPolicyStatus: sandbox reported policy load result
```
...
**File:** `crates/clustershell-cli/src/run.rs`
...
sequenceDiagram
    ...
    LP->>LP: Check level >= CLUSTERSHELL_LOG_PUSH_LEVEL
...
| `CLUSTERSHELL_LOG_PUSH_LEVEL` unparseable | Falls back to INFO |

## Platform Support

Platform-specific code is abstracted through `crates/clustershell-sandbox/src/sandbox/mod.rs`.
...
- [Inference Routing](inference-routing.md) -- Inference interception, route management, and the `clustershell-router` crate
