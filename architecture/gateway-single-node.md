...
This document describes how ClusterShell bootstraps a single-node k3s gateway inside a Docker container, for both local and remote (SSH) targets.

## Goals and Scope

- Provide a single bootstrap flow through `clustershell-bootstrap` for local and remote gateway lifecycle.
- Keep Docker as the only runtime dependency for provisioning and lifecycle operations.
- Package the ClusterShell gateway as one container image, transferred to the target host via registry pull.
...
- `crates/clustershell-cli/src/main.rs`: CLI entry point; `clap`-based command parsing.
- `crates/clustershell-cli/src/run.rs`: CLI command implementations (`gateway_start`, `gateway_stop`, `gateway_destroy`, `gateway_info`, `doctor_logs`).
- `crates/clustershell-cli/src/bootstrap.rs`: Auto-bootstrap helpers for `sandbox create` (offers to deploy a gateway when one is unreachable).
- `crates/clustershell-bootstrap/src/lib.rs`: Gateway lifecycle orchestration (`deploy_gateway`, `deploy_gateway_with_logs`, `gateway_handle`, `check_existing_deployment`).
- `crates/clustershell-bootstrap/src/docker.rs`: Docker API wrappers (per-gateway network, volume, container, image operations).
- `crates/clustershell-bootstrap/src/image.rs`: Remote image registry pull with XOR-obfuscated distribution credentials.
- `crates/clustershell-bootstrap/src/runtime.rs`: In-container operations via `docker exec` (health polling, stale node cleanup, deployment restart).
- `crates/clustershell-bootstrap/src/metadata.rs`: Gateway metadata creation, storage, and active gateway tracking.
- `crates/clustershell-bootstrap/src/mtls.rs`: Gateway TLS detection and CLI mTLS bundle extraction.
- `crates/clustershell-bootstrap/src/push.rs`: Local development image push into k3s containerd.
- `crates/clustershell-bootstrap/src/paths.rs`: XDG path resolution.
- `crates/clustershell-bootstrap/src/constants.rs`: Shared constants (image name, container/volume/network naming).
...
All gateway lifecycle commands live under `clustershell gateway`:

| Command | Description |
|---|---|
| `clustershell gateway start [--name NAME] [--remote user@host] [--ssh-key PATH]` | Provision or update a gateway |
| `clustershell gateway stop [--name NAME] [--remote user@host]` | Stop the container (preserves state) |
| `clustershell gateway destroy [--name NAME] [--remote user@host]` | Destroy container, attached volumes, per-gateway network, and metadata |
| `clustershell gateway info [--name NAME]` | Show deployment details (endpoint, SSH host) |
| `clustershell status` | Show gateway health via gRPC/HTTP |
| `clustershell doctor logs [--name NAME] [--remote user@host] [--tail N]` | Fetch gateway container logs |
| `clustershell doctor exec [--name NAME] [--remote user@host] -- <command>` | Run a command inside the gateway container |
| `clustershell gateway select <name>` | Set the active gateway |
| `clustershell gateway select` | Open an interactive chooser on a TTY, or list all gateways in non-interactive mode |

The `--name` flag defaults to `"clustershell"`. When omitted on commands that accept it, the CLI resolves the active gateway via: `--gateway` flag, then `CLUSTERSHELL_GATEWAY` env, then `~/.config/clustershell/active_gateway` file.
...
invokes `clustershell gateway start` with explicit flags such as `--recreate`,
...
For `mise run cluster`, `.env` acts as local source-of-truth for `GATEWAY_NAME`, `GATEWAY_PORT`, and `CLUSTERSHELL_GATEWAY`. Missing keys are appended; existing values are preserved. If `GATEWAY_PORT` is missing, the task selects a free local port and persists it.
...
sequenceDiagram
  ...
  participant C as clustershell-cli
  participant B as clustershell-bootstrap
  participant L as Local Docker daemon
  participant R as Remote Docker daemon (SSH)

  U->>C: clustershell gateway start --remote user@host
  ...
  B->>R: poll for secret clustershell-cli-client (90 attempts, 2s apart)
...
flowchart LR
  subgraph WS[User workstation]
    NAV[clustershell-cli]
    MTLS[mTLS bundle ca.crt, tls.crt, tls.key]
  end

  subgraph HOST[Target host]
    DOCKER[Docker daemon]
    K3S[clustershell-cluster-NAME single k3s container]
...
`deploy_gateway(DeployOptions)` in `crates/clustershell-bootstrap/src/lib.rs` chooses execution mode:
...
1. If `CLUSTERSHELL_CLUSTER_IMAGE` is set and non-empty, use it verbatim.
2. Otherwise, use the published distribution image base (`<distribution-registry>/clustershell/cluster`) with its default tag behavior.
...
- **Remote deploy**: `pull_remote_image()` queries the remote daemon's architecture via `Docker::version()`, pulls the matching platform variant from the distribution registry (with XOR-decoded credentials), and tags the pulled image to the expected local ref (for example `clustershell/cluster:dev` when an explicit local tag is requested).

### 3) Runtime infrastructure

For the target daemon (local or remote):

1. **Ensure bridge network** `clustershell-cluster-{name}` (attachable, bridge driver) via `ensure_network()`. Each gateway gets its own isolated Docker network.
2. **Ensure volume** `clustershell-cluster-{name}` via `ensure_volume()`.
...
4. **Ensure container** `clustershell-cluster-{name}` via `ensure_container()`:
   ...
   - Privileged mode.
   - Volume bind mount: `clustershell-cluster-{name}:/var/lib/rancher/k3s`.
    - Network: `clustershell-cluster-{name}` (per-gateway bridge network).
   ...
   - Port mappings:
      ...
      |---|---|---|
      | 30051/tcp | configurable (default 8080) | ClusterShell service NodePort (mTLS) |
...
2. **Push local images** (optional, local deploy only): If `CLUSTERSHELL_PUSH_IMAGES` is set, the comma-separated image refs are exported from the local Docker daemon as a single tar, uploaded into the container via `docker put_archive`, and imported into containerd via `ctr images import` in the `k8s.io` namespace. After import, `kubectl rollout restart deployment/clustershell clustershell` is run, followed by `kubectl rollout status --timeout=180s` to wait for completion. See `crates/clustershell-bootstrap/src/push.rs`.
...
TLS is always required. `fetch_and_store_cli_mtls()` polls for Kubernetes secret `clustershell-cli-client` in namespace `clustershell` (90 attempts, 2 seconds apart, 3 min total). Each attempt checks the container is still running. The secret's base64-encoded `ca.crt`, `tls.crt`, and `tls.key` fields are decoded and stored.

Storage location: `~/.config/clustershell/gateways/{name}/mtls/`
...
Metadata location: `~/.config/clustershell/gateways/{name}_metadata.json`

Note: metadata is stored at the `gateways/` level (not nested inside `{name}/` like mTLS).

After deploy, the CLI calls `save_active_gateway(name)`, writing the gateway name to `~/.config/clustershell/active_gateway`. Subsequent commands that don't specify `--gateway` or `CLUSTERSHELL_GATEWAY` resolve to this active gateway.
...
4. Kubernetes manifests: `deploy/kube/manifests/*.yaml` -> `/opt/clustershell/manifests/`

Bundled manifests include:
- `clustershell-helmchart.yaml` (ClusterShell Helm chart auto-deploy)
...
Copies bundled manifests from `/opt/clustershell/manifests/` to `/var/lib/rancher/k3s/server/manifests/`. This is needed because the volume mount on `/var/lib/rancher/k3s` overwrites any files baked into that path at image build time.

### Image configuration overrides

When environment variables are set, the entrypoint modifies the HelmChart manifest at `/var/lib/rancher/k3s/server/manifests/clustershell-helmchart.yaml`:
...
2. **ClusterShell StatefulSet**: Checks that `statefulset/clustershell` in namespace `clustershell` exists and has 1 ready replica.
3. **Gateway**: Checks that `gateway/clustershell-gateway` in namespace `clustershell` has the `Programmed` condition.
4. **mTLS secret** (conditional): If `NAV_GATEWAY_TLS_ENABLED` is true (or inferred from the HelmChart manifest using the same two-path detection logic as the bootstrap code), checks that secret `clustershell-cli-client` exists with non-empty `ca.crt`, `tls.crt`, and `tls.key` data.

## GPU Enablement

GPU support is part of the single-node gateway bootstrap path rather than a separate architecture.

- `clustershell gateway start --gpu` threads a boolean deploy option through `crates/clustershell-cli`, `crates/clustershell-bootstrap`, and `crates/clustershell-bootstrap/src/docker.rs`.
...
- `deploy/docker/cluster-entrypoint.sh` checks `GPU_ENABLED=true` and copies GPU-only manifests from `/opt/clustershell/gpu-manifests/` into k3s's manifests directory.
...
- The ClusterShell Helm chart grants the gateway service account cluster-scoped read access to `node.k8s.io/runtimeclasses` and core `nodes` so GPU sandbox admission can verify both the `nvidia` `RuntimeClass` and allocatable GPU capacity before creating a sandbox.
...
flowchart LR
  ...
  Tag --> OK[Image available as clustershell/cluster:TAG]
```
...
- The host port (configurable via `--port`, default 8080) maps to container port 30051 (ClusterShell service NodePort).
...
When `clustershell sandbox create` cannot connect to a gateway (connection refused, DNS error, missing default TLS certs), the CLI offers to bootstrap one automatically:

1. `should_attempt_bootstrap()` in `crates/clustershell-cli/src/bootstrap.rs` checks the error type. It returns `true` for connectivity errors and missing default TLS materials, but `false` for TLS handshake/auth errors.
2. If running in a terminal, the user is prompted to confirm.
3. `run_bootstrap()` deploys a gateway named `"clustershell"`, sets it as active, and returns fresh `TlsOptions` pointing to the newly-written mTLS certs.
...
| `REGISTRY_HOST` | Distribution registry host (or `CLUSTERSHELL_REGISTRY_HOST` override) | Always |
| `REGISTRY_INSECURE` | `"true"` or `"false"` | Always |
| `IMAGE_REPO_BASE` | `{registry_host}/{namespace}` (or `IMAGE_REPO_BASE`/`CLUSTERSHELL_IMAGE_REPO_BASE` override) | Always |
| `REGISTRY_ENDPOINT` | Custom endpoint URL | When `CLUSTERSHELL_REGISTRY_ENDPOINT` is set |
...
| Variable | Effect |
|---|---|
| `CLUSTERSHELL_CLUSTER_IMAGE` | Overrides entire image ref if set and non-empty |
| `IMAGE_TAG` | Sets image tag (default: `"dev"`) when `CLUSTERSHELL_CLUSTER_IMAGE` is not set |
...
| `CLUSTERSHELL_PUSH_IMAGES` | Comma-separated image refs to push into the gateway's containerd (local deploy only) |
| `CLUSTERSHELL_REGISTRY_HOST` | Override the distribution registry host |
| `CLUSTERSHELL_REGISTRY_NAMESPACE` | Override the registry namespace (default: `"clustershell"`) |
| `IMAGE_REPO_BASE` / `CLUSTERSHELL_IMAGE_REPO_BASE` | Override the image repository base path |
| `CLUSTERSHELL_REGISTRY_INSECURE` | Use HTTP instead of HTTPS for registry mirror |
| `CLUSTERSHELL_REGISTRY_ENDPOINT` | Custom registry mirror endpoint |
| `CLUSTERSHELL_REGISTRY_USERNAME` | Override registry auth username |
| `CLUSTERSHELL_REGISTRY_PASSWORD` | Override registry auth password |
| `CLUSTERSHELL_GATEWAY` | Set the active gateway name for CLI commands |

## File System Layout

Artifacts stored under `$XDG_CONFIG_HOME/clustershell/` (default `~/.config/clustershell/`):
...
- `crates/clustershell-bootstrap/src/lib.rs` -- public API, deploy orchestration
- `crates/clustershell-bootstrap/src/docker.rs` -- Docker API wrappers
- `crates/clustershell-bootstrap/src/image.rs` -- registry pull, XOR credentials
- `crates/clustershell-bootstrap/src/runtime.rs` -- exec, health polling, stale node cleanup
- `crates/clustershell-bootstrap/src/metadata.rs` -- metadata CRUD, active gateway, SSH resolution
- `crates/clustershell-bootstrap/src/mtls.rs` -- TLS detection, secret extraction, atomic write
- `crates/clustershell-bootstrap/src/push.rs` -- local image push into k3s containerd
- `crates/clustershell-bootstrap/src/constants.rs` -- naming conventions
- `crates/clustershell-bootstrap/src/paths.rs` -- XDG path helpers
- `crates/clustershell-cli/src/main.rs` -- CLI command definitions
- `crates/clustershell-cli/src/run.rs` -- CLI command implementations
- `crates/clustershell-cli/src/bootstrap.rs` -- auto-bootstrap from sandbox create
...
- `deploy/docker/cluster-healthcheck.sh` -- Docker HEALTHCHECK script
- `deploy/kube/manifests/clustershell-helmchart.yaml` -- ClusterShell Helm chart manifest
