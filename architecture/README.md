...
```bash
pip install <package>
clustershell sandbox create --remote user@host -- claude
```
...
The inference routing system transparently intercepts AI inference API calls from sandboxed agents and routes them to configured backends. Routing happens locally within the sandbox -- the proxy intercepts connections to `inference.local`, and the embedded `clustershell-router` forwards requests directly to the backend without traversing the gateway at request time.

**How it works end-to-end:**

1. An operator configures cluster-level inference via `clustershell cluster inference set --provider <name> --model <id>`. This stores a reference to the named provider and model on the gateway.
2. When a sandbox starts, the supervisor fetches an inference bundle from the gateway via the `GetInferenceBundle` RPC. The gateway resolves the stored provider reference into a complete route: endpoint URL, API key, supported protocols, provider type, and auth metadata. The sandbox refreshes this bundle eagerly in the background every 5 seconds by default (override with `CLUSTERSHELL_ROUTE_REFRESH_INTERVAL_SECS`).
...
- Provider-specific behavior (auth header style, default headers, supported protocols) is centralized in `InferenceProviderProfile` definitions in `clustershell-core`. Supported inference provider types are openai, anthropic, and nvidia.
- Cluster inference is managed via CLI (`clustershell cluster inference set/get`).
...
|---|---|---|
| Proxy inference interception | `crates/clustershell-sandbox/src/proxy.rs` | Intercepts `inference.local` CONNECT requests, TLS-terminates, dispatches to router |
| Inference pattern detection | `crates/clustershell-sandbox/src/l7/inference.rs` | Matches HTTP method + path against known inference API patterns |
| Local inference router | `crates/clustershell-router/src/lib.rs` | Selects a compatible route by protocol and proxies to the backend |
| Provider profiles | `crates/clustershell-core/src/inference.rs` | Centralized auth, headers, protocols, and endpoint defaults per provider type |
| Gateway inference service | `crates/clustershell-server/src/inference.rs` | Stores cluster inference config, resolves bundles with credentials from provider records |
...
Policies are not intended to be hand-edited by end users in normal operation. They are associated with sandboxes at creation time and fetched by the sandbox supervisor at startup via gRPC. For development and testing, policies can also be loaded from local files. A gateway-global policy can override all sandbox policies via `clustershell policy set --global`.
...
- **Gateway management** (`clustershell gateway`): Deploy, stop, destroy, and inspect clusters. Supports both local and remote (SSH) targets.
- **Sandbox management** (`clustershell sandbox`): Create sandboxes (with optional file upload and provider auto-discovery), connect to sandboxes via SSH, and delete sandboxes.
- **Top-level commands**: `clustershell status` (cluster health), `clustershell logs` (sandbox logs), `clustershell forward` (port forwarding), `clustershell policy` (sandbox policy management), `clustershell settings` (effective sandbox settings and global/sandbox key updates).
- **Provider management** (`clustershell provider`): Create, update, list, and delete external service credentials.
- **Inference management** (`clustershell cluster inference`): Configure cluster-level inference by specifying a provider and model. The gateway resolves endpoint and credential details from the named provider record.

The CLI resolves which gateway to operate on through a priority chain: explicit `--gateway` flag, then the `CLUSTERSHELL_GATEWAY` environment variable, then the active gateway set by `clustershell gateway select`. Gateway names are exposed to shell completion from local metadata, and `clustershell gateway select` opens an interactive chooser on a TTY while falling back to a printed list in non-interactive use. The CLI supports TLS client certificates for mutual authentication with the gateway.
...
```bash
clustershell sandbox create --remote user@hostname -- claude
```
...
and then runs `clustershell gateway start` there. It defaults to secure gateway
...
```bash
clustershell sandbox connect <sandbox-name>
```
