...
graph TD
    ...
    NAV["ClusterShellServer<br/>(ClusterShell service)"]
    ...
    MUX -->|"other"| HTTP
    GRPC_ROUTER -->|"/clustershell.inference.v1.Inference/*"| INF
...
| Module | File | Purpose |
|--------|------|---------|
| Entry point | `crates/clustershell-server/src/main.rs` | CLI argument parsing, config assembly, tracing setup, calls `run_server` |
| Gateway runtime | `crates/clustershell-server/src/lib.rs` | `ServerState` struct, `run_server()` accept loop |
| Protocol mux | `crates/clustershell-server/src/multiplex.rs` | `MultiplexService`, `MultiplexedService`, `GrpcRouter`, `BoxBody` |
| gRPC: ClusterShell | `crates/clustershell-server/src/grpc.rs` | `ClusterShellService` -- sandbox CRUD, provider CRUD, watch, exec, SSH sessions, policy delivery |
| gRPC: Inference | `crates/clustershell-server/src/inference.rs` | `InferenceService` -- cluster inference config (set/get) and sandbox inference bundle delivery |
| HTTP | `crates/clustershell-server/src/http.rs` | Health endpoints, merged with SSH tunnel router |
| Browser auth | `crates/clustershell-server/src/auth.rs` | Cloudflare browser login relay at `/auth/connect` |
| SSH tunnel | `crates/clustershell-server/src/ssh_tunnel.rs` | HTTP CONNECT handler at `/connect/ssh` |
| WS tunnel | `crates/clustershell-server/src/ws_tunnel.rs` | WebSocket tunnel handler at `/_ws_tunnel` for Cloudflare-fronted clients |
| TLS | `crates/clustershell-server/src/tls.rs` | `TlsAcceptor` wrapping rustls with ALPN |
| Persistence | `crates/clustershell-server/src/persistence/mod.rs` | `Store` enum (SQLite/Postgres), generic object CRUD, protobuf codec |
| Persistence: SQLite | `crates/clustershell-server/src/persistence/sqlite.rs` | `SqliteStore` with sqlx |
| Persistence: Postgres | `crates/clustershell-server/src/persistence/postgres.rs` | `PostgresStore` with sqlx |
| Sandbox K8s | `crates/clustershell-server/src/sandbox/mod.rs` | `SandboxClient`, CRD creation/deletion, Kubernetes watcher, phase derivation |
| Sandbox index | `crates/clustershell-server/src/sandbox_index.rs` | `SandboxIndex` -- in-memory name/pod-to-id correlation |
| Watch bus | `crates/clustershell-server/src/sandbox_watch.rs` | `SandboxWatchBus`, `PlatformEventBus`, Kubernetes event tailer |
| Tracing bus | `crates/clustershell-server/src/tracing_bus.rs` | `TracingLogBus` -- captures tracing events keyed by `sandbox_id` |

Proto definitions consumed by the gateway:

| Proto file | Package | Defines |
|------------|---------|---------|
| `proto/clustershell.proto` | `clustershell.v1` | `ClusterShell` service, sandbox/provider/SSH/watch messages |
| `proto/inference.proto` | `clustershell.inference.v1` | `Inference` service: `SetClusterInference`, `GetClusterInference`, `GetInferenceBundle` |
| `proto/datamodel.proto` | `clustershell.datamodel.v1` | `Sandbox`, `SandboxSpec`, `SandboxStatus`, `Provider`, `SandboxPhase` |
| `proto/sandbox.proto` | `clustershell.sandbox.v1` | `SandboxPolicy`, `NetworkPolicyRule`, `SettingValue`, `EffectiveSetting`, `SettingScope`, `PolicySource`, `GetSandboxSettingsRequest/Response`, `GetGatewaySettingsRequest/Response` |

## Startup Sequence

The gateway boots in `main()` (`crates/clustershell-server/src/main.rs`) and proceeds through these steps:
...
4. **Build `Config`** -- Assembles a `clustershell_core::Config` from the parsed arguments.
5. **Call `run_server()`** (`crates/clustershell-server/src/lib.rs`):
...
|------|---------|---------|-------------|
| `--port` | `CLUSTERSHELL_SERVER_PORT` | `8080` | TCP listen port (binds `0.0.0.0`) |
| `--log-level` | `CLUSTERSHELL_LOG_LEVEL` | `info` | Tracing log level filter |
| `--tls-cert` | `CLUSTERSHELL_TLS_CERT` | None | Path to PEM certificate file |
| `--tls-key` | `CLUSTERSHELL_TLS_KEY` | None | Path to PEM private key file |
| `--tls-client-ca` | `CLUSTERSHELL_TLS_CLIENT_CA` | None | Path to PEM CA cert for mTLS client verification |
| `--disable-tls` | `CLUSTERSHELL_DISABLE_TLS` | `false` | Listen on plaintext HTTP behind a trusted reverse proxy or tunnel |
| `--disable-gateway-auth` | `CLUSTERSHELL_DISABLE_GATEWAY_AUTH` | `false` | Keep TLS enabled but allow no-certificate clients and rely on application-layer auth |
| `--client-tls-secret-name` | `CLUSTERSHELL_CLIENT_TLS_SECRET_NAME` | None | K8s secret name to mount into sandbox pods for mTLS |
| `--db-url` | `CLUSTERSHELL_DB_URL` | *required* | Database URL (`sqlite:...` or `postgres://...`). The Helm chart defaults to `sqlite:/var/clustershell/clustershell.db` (persistent volume). In-memory SQLite (`sqlite::memory:?cache=shared`) works for ephemeral/test environments but data is lost on restart. |
| `--sandbox-namespace` | `CLUSTERSHELL_SANDBOX_NAMESPACE` | `default` | Kubernetes namespace for sandbox CRDs |
| `--sandbox-image` | `CLUSTERSHELL_SANDBOX_IMAGE` | None | Default container image for sandbox pods |
| `--grpc-endpoint` | `CLUSTERSHELL_GRPC_ENDPOINT` | None | gRPC endpoint reachable from within the cluster (for sandbox callbacks) |
| `--ssh-gateway-host` | `CLUSTERSHELL_SSH_GATEWAY_HOST` | `127.0.0.1` | Public hostname returned in SSH session responses |
| `--ssh-gateway-port` | `CLUSTERSHELL_SSH_GATEWAY_PORT` | `8080` | Public port returned in SSH session responses |
| `--ssh-connect-path` | `CLUSTERSHELL_SSH_CONNECT_PATH` | `/connect/ssh` | HTTP path for SSH CONNECT/upgrade |
| `--sandbox-ssh-port` | `CLUSTERSHELL_SANDBOX_SSH_PORT` | `2222` | SSH listen port inside sandbox pods |
| `--ssh-handshake-secret` | `CLUSTERSHELL_SSH_HANDSHAKE_SECRET` | None | Shared HMAC-SHA256 secret for gateway-to-sandbox handshake |
| `--ssh-handshake-skew-secs` | `CLUSTERSHELL_SSH_HANDSHAKE_SKEW_SECS` | `300` | Allowed clock skew (seconds) for SSH handshake timestamps |

## Shared State

All handlers share an `Arc<ServerState>` (`crates/clustershell-server/src/lib.rs`):
...
`MultiplexService::serve()` (`crates/clustershell-server/src/multiplex.rs`) creates per-connection service instances:
...
`GrpcRouter` (`crates/clustershell-server/src/multiplex.rs`) further routes gRPC requests by URI path prefix:

- Paths starting with `/clustershell.inference.v1.Inference/` go to `InferenceServer`.
- All other gRPC paths go to `ClusterShellServer`.
...
When TLS is enabled (`crates/clustershell-server/src/tls.rs`):
...
- Certificates are generated at cluster bootstrap time by the `clustershell-bootstrap` crate using `rcgen`, not by a Helm Job. The bootstrap reconciles three K8s secrets: `clustershell-server-tls` (server cert+key), `clustershell-server-client-ca` (CA cert), and `clustershell-client-tls` (client cert+key+CA, shared by CLI and sandbox pods).
...
- **Redeploy behavior**: On redeploy, existing cluster TLS secrets are loaded and reused if they are complete and valid PEM. If secrets are missing, incomplete, or malformed, fresh PKI is generated. If rotation occurs and the clustershell workload is already running, the bootstrap performs a rollout restart and waits for completion before persisting CLI-side credentials.

## gRPC Services

### ClusterShell Service

Defined in `proto/clustershell.proto`, implemented in `crates/clustershell-server/src/grpc.rs` as `ClusterShellService`.
...
Defined in `proto/inference.proto`, implemented in `crates/clustershell-server/src/inference.rs` as `InferenceService`.
...
The trait method delegates to `resolve_inference_bundle(store)` (`crates/clustershell-server/src/inference.rs`), which takes `&Store` instead of `&self`. This extraction decouples bundle resolution from `ServerState`, enabling direct unit testing against an in-memory SQLite store without constructing a full server.
...
Managed route resolution in `resolve_managed_cluster_route()` (`crates/clustershell-server/src/inference.rs`):
...
5. Resolve the provider into a `ResolvedProviderRoute` via `resolve_provider_route()`:
   - Look up the `InferenceProviderProfile` for the provider's type via `clustershell_core::inference::profile_for()`. Supported types: `openai`, `anthropic`, `nvidia`.
...
The HTTP router (`crates/clustershell-server/src/http.rs`) merges two sub-routers:
...
The `WatchSandbox` RPC (`crates/clustershell-server/src/grpc.rs`) provides a multiplexed server-streaming response that can include sandbox status snapshots, gateway log lines, and platform events.
...
The `ExecSandbox` RPC (`crates/clustershell-server/src/grpc.rs`) executes a command inside a sandbox pod over SSH and streams stdout/stderr/exit back to the client.
...
The SSH tunnel endpoint (`crates/clustershell-server/src/ssh_tunnel.rs`) allows external SSH clients to reach sandbox pods through the gateway using HTTP CONNECT upgrades.
...
The `Store` enum (`crates/clustershell-server/src/persistence/mod.rs`) dispatches to either `SqliteStore` or `PostgresStore` based on the database URL prefix:
...
Both backends auto-run migrations on connect from `crates/clustershell-server/migrations/{sqlite,postgres}/`.
...
The gateway runs as a Kubernetes **StatefulSet** with a `volumeClaimTemplate` that provisions a 1Gi `ReadWriteOnce` PersistentVolumeClaim mounted at `/var/clustershell`. On k3s clusters this uses the built-in `local-path-provisioner` StorageClass (the cluster default). The SQLite database file at `/var/clustershell/clustershell.db` survives pod restarts and rescheduling.

The Helm chart template is at `deploy/helm/clustershell/templates/statefulset.yaml`.
...
`SandboxClient` (`crates/clustershell-server/src/sandbox/mod.rs`) manages `agents.x-k8s.io/v1alpha1/Sandbox` CRDs.

- **Create**: Translates a `Sandbox` proto into a Kubernetes `DynamicObject` with labels (`clustershell.ai/sandbox-id`, `clustershell.ai/managed-by: clustershell`) and a spec that includes the pod template, environment variables, and gateway-required env vars (`CLUSTERSHELL_SANDBOX_ID`, `CLUSTERSHELL_ENDPOINT`, `CLUSTERSHELL_SSH_LISTEN_ADDR`, etc.).
...
`spawn_sandbox_watcher()` (`crates/clustershell-server/src/sandbox/mod.rs`) runs a Kubernetes watcher on `Sandbox` CRDs and processes three event types:
...
`spawn_kube_event_tailer()` (`crates/clustershell-server/src/sandbox_watch.rs`) watches all Kubernetes `Event` objects in the sandbox namespace and correlates them to sandbox IDs using `SandboxIndex`:
...
`SandboxIndex` (`crates/clustershell-server/src/sandbox_index.rs`) maintains two in-memory maps protected by an `RwLock`:
