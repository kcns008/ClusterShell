...
All inference execution happens locally inside the sandbox via the `clustershell-router` crate. The gateway is control-plane only: it stores configuration and delivers resolved route bundles to sandboxes over gRPC.
...
sequenceDiagram
    ...
    participant Router as clustershell-router
...
File: `crates/clustershell-core/src/inference.rs`
...
File: `crates/clustershell-server/src/inference.rs`
...
- `crates/clustershell-sandbox/src/proxy.rs` -- proxy interception, inference context, request routing
- `crates/clustershell-sandbox/src/l7/inference.rs` -- pattern detection, HTTP parsing, response formatting
- `crates/clustershell-sandbox/src/lib.rs` -- inference context initialization, route refresh
- `crates/clustershell-sandbox/src/grpc_client.rs` -- `fetch_inference_bundle()`

In cluster mode, the sandbox starts a background refresh loop as soon as the inference context is created. The loop polls the gateway every 5 seconds by default (`CLUSTERSHELL_ROUTE_REFRESH_INTERVAL_SECS` override) and uses the bundle revision hash to skip no-op cache writes.
...
File: `crates/clustershell-sandbox/src/l7/inference.rs` -- `default_patterns()` and `detect_inference_pattern()`
...
`bundle_to_resolved_routes()` in `lib.rs` converts proto `ResolvedRoute` messages to router `ResolvedRoute` structs. Auth header style and default headers are derived from `provider_type` using `clustershell_core::inference::auth_for_provider_type()`.

## Router Behavior

Files:

- `crates/clustershell-router/src/lib.rs` -- `Router`, `proxy_with_candidates()`, `proxy_with_candidates_streaming()`
- `crates/clustershell-router/src/backend.rs` -- `proxy_to_backend()`, `proxy_to_backend_streaming()`, URL construction
- `crates/clustershell-router/src/config.rs` -- `RouteConfig`, `ResolvedRoute`, YAML loading
...
File: `crates/clustershell-router/src/mock.rs`

Routes with `mock://` scheme endpoints return canned responses without making HTTP requests. Mock responses are protocol-aware (OpenAI chat completion, OpenAI completion, Anthropic messages, or generic JSON). Mock routes include an `x-clustershell-mock: true` response header.
...
File: `crates/clustershell-router/src/config.rs`
...
- `clustershell inference set --provider <name> --model <id>` -- configures user-facing cluster inference
- `clustershell inference set --system --provider <name> --model <id>` -- configures system inference
- `clustershell inference get` -- displays both user and system inference configuration
- `clustershell inference get --system` -- displays only the system inference configuration
...
- `crates/clustershell-providers/src/lib.rs` -- `ProviderRegistry`, `ProviderPlugin` trait
- `crates/clustershell-providers/src/providers/openai.rs` -- `OpenaiProvider`
- `crates/clustershell-providers/src/providers/anthropic.rs` -- `AnthropicProvider`
- `crates/clustershell-providers/src/providers/nvidia.rs` -- `NvidiaProvider`

Provider discovery and inference routing are separate concerns:

- `ProviderPlugin` (in `clustershell-providers`) handles credential *discovery* -- scanning environment variables to find API keys.
- `InferenceProviderProfile` (in `clustershell-core`) handles how to *use* discovered credentials to make inference API calls.
