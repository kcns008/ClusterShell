...
graph TD
    CLI["CLI / TUI"]
    GW["Gateway<br/>(clustershell-server)"]
...
**File:** `crates/clustershell-core/src/settings.rs`
...
**File:** `crates/clustershell-server/src/grpc.rs`
...
This allows operators to push a single policy that applies to all sandboxes via `clustershell policy set --global --policy FILE`.
...
The blocking logic is implemented by `require_no_global_policy()` in `crates/clustershell-server/src/grpc.rs`, which checks for the `policy` key in global settings and returns `FailedPrecondition` if present.
...
Deleting the global policy (`clustershell policy delete --global`) removes the `policy` key from global settings and restores sandbox-level policy control.
...
**File:** `crates/clustershell-sandbox/src/lib.rs` (`log_setting_changes`)
...
**File:** `crates/clustershell-cli/src/main.rs` (`SettingsCommands`), `crates/clustershell-cli/src/run.rs`
...
# Skip confirmation
clustershell settings set --global --key log_level --value info --yes
```
...
clustershell settings delete --global --key log_level --yes
```
...
The `--wait` flag is rejected for global policy updates with: `"--wait is not supported for global policies; global policies are effective immediately"`. See `crates/clustershell-cli/src/main.rs`.
...
```bash
clustershell policy list --global
clustershell policy list --global --limit 10
```
...
clustershell policy get --global

# Specific version
clustershell policy get --global --rev 3

# Full policy payload as YAML
clustershell policy get --global --full
```
...
**File:** `crates/clustershell-tui/src/`

### Dashboard: Global Policy Indicator

**File:** `crates/clustershell-tui/src/ui/dashboard.rs`
...
**File:** `crates/clustershell-tui/src/ui/sandbox_detail.rs`
...
End-to-end trace for `clustershell settings set --global --key log_level --value debug --yes`:

1. **CLI** (`crates/clustershell-cli/src/run.rs` -- `gateway_setting_set()`):
...
2. **Gateway** (`crates/clustershell-server/src/grpc.rs` -- `update_settings()`):
...
End-to-end trace for `clustershell policy set --global --policy policy.yaml --yes`:

1. **CLI** (`crates/clustershell-cli/src/main.rs`, `crates/clustershell-cli/src/run.rs` -- `sandbox_policy_set_global()`):
...
- [Sandbox Architecture](sandbox.md) -- Poll loop, `CachedClusterShellClient`, OPA reload lifecycle
