...
graph TD
    subgraph PKI["PKI (generated at bootstrap)"]
        CA["clustershell-ca<br/>(self-signed root)"]
        SERVER_CERT["clustershell-server cert<br/>(signed by CA)"]
        CLIENT_CERT["clustershell-client cert<br/>(signed by CA, shared)"]
        CA --> SERVER_CERT
        CA --> CLIENT_CERT
    end

    subgraph CLUSTER["Kubernetes Cluster"]
        S1["clustershell-server-tls<br/>Secret (server cert+key)"]
        S2["clustershell-server-client-ca<br/>Secret (CA cert)"]
        S3["clustershell-client-tls<br/>Secret (client cert+key+CA)"]
    ...
    subgraph HOST["User's Machine"]
        CLI["CLI"]
        MTLS_DIR["~/.config/clustershell/<br/>clusters/&lt;name&gt;/mtls/"]
    end
...
The PKI is a single-tier CA hierarchy generated by the `clustershell-bootstrap` crate using `rcgen`. All certificates are created in a single pass at cluster bootstrap time.

```
clustershell-ca  (Self-signed Root CA, O=clustershell, CN=clustershell-ca)
├── clustershell-server  (Leaf cert, CN=clustershell-server)
│   SANs: clustershell, clustershell.clustershell.svc,
│          clustershell.clustershell.svc.cluster.local,
│          localhost, host.docker.internal, 127.0.0.1
│          + extra SANs for remote deployments
│
└── clustershell-client  (Leaf cert, CN=clustershell-client)
...
See `crates/clustershell-bootstrap/src/pki.rs:35` for the `generate_pki()` implementation and `crates/clustershell-bootstrap/src/pki.rs:18` for the default SAN list.

## Kubernetes Secret Distribution

The PKI bundle is distributed as three Kubernetes secrets in the `clustershell` namespace:

| Secret Name | Type | Contents | Consumed By |
|---|---|---|---|
| `clustershell-server-tls` | `kubernetes.io/tls` | `tls.crt` (server cert), `tls.key` (server key) | Gateway StatefulSet |
| `clustershell-server-client-ca` | `Opaque` | `ca.crt` (CA cert) | Gateway StatefulSet (client verification) |
| `clustershell-client-tls` | `Opaque` | `tls.crt` (client cert), `tls.key` (client key), `ca.crt` (CA cert) | Sandbox pods, CLI (via local filesystem) |

Secret names are defined as constants in `crates/clustershell-bootstrap/src/constants.rs:6-10`.

### Gateway Mounts

The Helm StatefulSet (`deploy/helm/clustershell/templates/statefulset.yaml`) mounts:

| Volume | Mount Path | Source Secret |
|---|---|---|
| `tls-cert` | `/etc/clustershell-tls/server/` (read-only) | `clustershell-server-tls` |
| `tls-client-ca` | `/etc/clustershell-tls/client-ca/` (read-only) | `clustershell-server-client-ca` |

Environment variables point the gateway binary to these paths:

```
CLUSTERSHELL_TLS_CERT=/etc/clustershell-tls/server/tls.crt
CLUSTERSHELL_TLS_KEY=/etc/clustershell-tls/server/tls.key
CLUSTERSHELL_TLS_CLIENT_CA=/etc/clustershell-tls/client-ca/ca.crt
```

### Sandbox Pod Mounts

When the gateway creates a sandbox pod (`crates/clustershell-server/src/sandbox/mod.rs:681`), it injects:

- A volume backed by the `clustershell-client-tls` secret.
- A read-only mount at `/etc/clustershell-tls/client/` on the agent container.
- Environment variables for the sandbox gRPC client:

```
CLUSTERSHELL_TLS_CA=/etc/clustershell-tls/client/ca.crt
CLUSTERSHELL_TLS_CERT=/etc/clustershell-tls/client/tls.crt
CLUSTERSHELL_TLS_KEY=/etc/clustershell-tls/client/tls.key
CLUSTERSHELL_ENDPOINT=https://clustershell.clustershell.svc.cluster.local:8080
```
...
```
$XDG_CONFIG_HOME/clustershell/gateways/<gateway-name>/mtls/
├── ca.crt
├── tls.crt
└── tls.key
```

Files are written atomically using a temp-dir -> validate -> rename strategy with backup and rollback on failure. See `crates/clustershell-bootstrap/src/mtls.rs:10`.

## PKI Bootstrap Sequence

PKI provisioning occurs during `deploy_cluster_with_logs()` (`crates/clustershell-bootstrap/src/lib.rs:284`). The full sequence:
...
4. **`reconcile_pki()` called** (`crates/clustershell-bootstrap/src/lib.rs:515`):
   1. Wait for the `clustershell` namespace to exist (created by the Helm controller).
...
5. **Workload restart on rotation** -- if `rotated=true` and the clustershell StatefulSet already exists, the bootstrap performs `kubectl rollout restart` and waits for completion. This ensures the server picks up new TLS secrets before the CLI writes its local copy.
...
sequenceDiagram
    ...
    CLI->>K8s: Wait for clustershell namespace
...
`TlsAcceptor::from_files()` (`crates/clustershell-server/src/tls.rs:27`) constructs the `rustls::ServerConfig`:
...
Sandbox pods connect back to the gateway at startup to fetch their policy and provider credentials. The gRPC client (`crates/clustershell-sandbox/src/grpc_client.rs:18`) reads three environment variables to configure mTLS:
...
- **Secret**: shared via `CLUSTERSHELL_SSH_HANDSHAKE_SECRET` env var, set on both the gateway and sandbox.
- **Clock skew tolerance**: configurable via `CLUSTERSHELL_SSH_HANDSHAKE_SKEW_SECS` (default 300 seconds).
...
| Container | `30051` | Hardcoded in `crates/clustershell-bootstrap/src/docker.rs` |
| k3s NodePort | `30051` | `deploy/helm/clustershell/values.yaml` (`service.nodePort`) |
| k3s Service | `8080` | `deploy/helm/clustershell/values.yaml` (`service.port`) |
| Server bind | `8080` | `--port` flag / `CLUSTERSHELL_SERVER_PORT` env var |
...
- **Individual sandbox identity at the TLS layer**: all sandboxes share one client certificate (`CN=clustershell-client`). Post-TLS identification uses the `x-sandbox-id` gRPC metadata header, which is trusted because it arrives over an mTLS-authenticated channel.
...
The gateway container runs with a hardened security context (`deploy/helm/clustershell/values.yaml:25`):
...
- A malicious actor with direct access to Kubernetes secrets in the `clustershell` namespace.
...
1. **Ephemeral sandbox CA**: a per-sandbox CA (`CN=ClusterShell Sandbox CA, O=ClusterShell`) is generated at sandbox startup. This CA is completely independent of the cluster mTLS CA.
