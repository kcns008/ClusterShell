# ClusterShell System Architecture
...
graph TB
    ...
    subgraph UserMachine["User's Machine"]
        CLI["ClusterShell CLI<br/>(clustershell)"]
        TUI["ClusterShell TUI<br/>(clustershell term)"]
        SDK["Python SDK<br/>(clustershell)"]
        LocalConfig["~/.config/clustershell/<br/>clusters, mTLS certs,<br/>active_cluster"]
    end

    %% ============================================================
    %% KUBERNETES CLUSTER (single Docker container)
    %% ============================================================
    subgraph Cluster["ClusterShell Cluster Container (Docker)"]
        ...
        subgraph NSNamespace["clustershell namespace"]

            subgraph GatewayPod["Gateway StatefulSet"]
                Gateway["clustershell-server<br/>:8080<br/>(gRPC + HTTP, mTLS)"]
                SQLite[("SQLite DB<br/>/var/clustershell/<br/>clustershell.db")]
            ...
            subgraph SandboxPod["Sandbox Pod (1 per sandbox)"]
                ...
                subgraph Supervisor["Sandbox Supervisor<br/>(privileged user)"]
                    ...
                    InferenceRouter["Inference Router<br/>(clustershell-router)"]
...
|-------|----------|---------|
| Blue | User-side components | ClusterShell CLI, ClusterShell TUI, Python SDK |
| Orange | Gateway / Control plane | clustershell-server, watch bus, log bus |
