...
ClusterShell uses a first-class `Provider` entity to represent external tool credentials and
...
The gRPC surface is defined in `proto/clustershell.proto`:
...
- `crates/clustershell-cli`
  - `clustershell provider ...` command handlers,
  - sandbox provider requirement resolution in `sandbox create`.
- `crates/clustershell-server` (gateway)
...
Each provider has its own module under `crates/clustershell-providers/src/providers/`.

### Trait Definition

`ProviderPlugin` (`crates/clustershell-providers/src/lib.rs`):
...
`clustershell provider create --type <type> --name <name> [--from-existing] [--credential k=v]... [--config k=v]...`
...
- `clustershell provider get <name>`
- `clustershell provider list`
- `clustershell provider update <name> ...`
- `clustershell provider delete <name> [<name>...]`

### Sandbox Create

`clustershell sandbox create --provider gitlab -- claude`

Resolution logic (CLI side, `crates/clustershell-cli/src/run.rs`):
...
Gateway-side `create_sandbox()` (`crates/clustershell-server/src/grpc.rs`):
...
`resolve_provider_environment()` (`crates/clustershell-server/src/grpc.rs`) builds the
...
The sandbox pod runs `clustershell-sandbox` (`crates/clustershell-sandbox/src/main.rs`). On
startup it receives `CLUSTERSHELL_SANDBOX_ID` and `CLUSTERSHELL_ENDPOINT` as environment
variables (injected into the pod spec by the gateway's Kubernetes sandbox creation code).

In `run_sandbox()` (`crates/clustershell-sandbox/src/lib.rs`):
...
- a child-visible env map with placeholder values such as
  `clustershell:resolve:env:ANTHROPIC_API_KEY`, and
...
**1. Entrypoint process** (`crates/clustershell-sandbox/src/process.rs`):

```rust
let mut cmd = Command::new(program);
cmd.args(args)
    .env("CLUSTERSHELL_SANDBOX", "1");
...
`CLUSTERSHELL_SSH_HANDSHAKE_SECRET` so the handshake secret does not leak into the agent
entrypoint process.
...
**2. SSH shell sessions** (`crates/clustershell-sandbox/src/ssh.rs`):

When a user connects via `clustershell sandbox connect`, a PTY shell is spawned:

```rust
let mut cmd = Command::new(shell);
cmd.env("CLUSTERSHELL_SANDBOX", "1")
    .env("HOME", "/sandbox")
    .env("USER", "sandbox")
    .env("TERM", term);
...
header (for example `Authorization: Bearer clustershell:resolve:env:ANTHROPIC_API_KEY`), the
...
```
CLI: clustershell sandbox create -- claude
  |
  ...
  +-- Sends CreateSandboxRequest to gateway
        ...
        Gateway: create_sandbox()
          ...
          +-- Creates K8s Sandbox CRD (no credentials in pod spec)
                |
                K8s: pod starts clustershell-sandbox binary
                  +-- CLUSTERSHELL_SANDBOX_ID and CLUSTERSHELL_ENDPOINT set in pod env
                  |
                    Sandbox supervisor: run_sandbox()
                      ...
                      |     +-- child env: {ANTHROPIC_API_KEY: "clustershell:resolve:env:ANTHROPIC_API_KEY"}
                      |     +-- supervisor registry: {"clustershell:resolve:env:ANTHROPIC_API_KEY": "sk-..."}
                      +-- Spawns entrypoint with placeholder env
                      +-- SSH server holds placeholder env
                      |     +-- Each SSH shell: cmd.env("ANTHROPIC_API_KEY", "clustershell:resolve:env:ANTHROPIC_API_KEY")
...
- `CLUSTERSHELL_SSH_HANDSHAKE_SECRET` is required by the supervisor/SSH server path but is
  explicitly kept out of spawned sandbox child-process environments.

## Test Strategy

- Per-provider unit tests in each provider module.
- Shared normalization/command-detection tests in `crates/clustershell-providers/src/lib.rs`.
...
- `resolve_provider_environment` unit tests in `crates/clustershell-server/src/grpc.rs`.
