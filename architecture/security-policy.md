...
| ---------------- | ------------------------ | ------------------------------------------------ |
| `--policy-rules` | `CLUSTERSHELL_POLICY_RULES` | Path to `.rego` file containing evaluation rules |
| `--policy-data`  | `CLUSTERSHELL_POLICY_DATA`  | Path to YAML file containing policy data         |

The YAML data file is preprocessed before loading into the OPA engine: L7 policies are validated, and `access` presets are expanded into explicit `rules` arrays. See `crates/clustershell-sandbox/src/opa.rs` -- `preprocess_yaml_data()`.
...
clustershell-sandbox \
  --sandbox-id abc123 \
  --clustershell-endpoint https://clustershell:8080 \
  -- /bin/bash
```

| Flag                     | Environment Variable   | Description                  |
| ------------------------ | ---------------------- | ---------------------------- |
| `--sandbox-id`           | `CLUSTERSHELL_SANDBOX_ID` | Sandbox ID for policy lookup |
| `--clustershell-endpoint`   | `CLUSTERSHELL_ENDPOINT`   | Gateway gRPC endpoint        |

The gateway returns a `SandboxPolicy` protobuf message (defined in `proto/sandbox.proto`). The sandbox supervisor converts this proto into JSON, validates L7 config, expands presets, and loads it into the OPA engine using baked-in Rego rules (`sandbox-policy.rego` compiled via `include_str!`). See `crates/clustershell-sandbox/src/opa.rs` -- `OpaEngine::from_proto()`.
...
flowchart TD
    ...
    CHECK -->|No| OPENSHELL{gRPC mode?<br/>--sandbox-id +<br/>--clustershell-endpoint}
...
File mode takes precedence. If both `--policy-rules`/`--policy-data` and `--sandbox-id`/`--clustershell-endpoint` are provided, file mode is used. See `crates/clustershell-sandbox/src/lib.rs` -- `load_policy()`.
...
| **Dynamic** | `network_policies` | Evaluated at runtime by the OPA engine on every proxy CONNECT request and L7 rule check. The OPA engine can be atomically replaced. | Yes -- via `clustershell policy set` |

Attempting to change a static field in an update request returns an `INVALID_ARGUMENT` error with a message indicating which field cannot be modified. See `crates/clustershell-server/src/grpc.rs` -- `validate_static_fields_unchanged()`.
...
sequenceDiagram
    ...
    participant GW as Gateway (clustershell-server)
    participant DB as Persistence (SQLite/Postgres)
    participant SB as Sandbox (clustershell-sandbox)
...
- **Version 1** is the policy from the sandbox's `spec.policy` at creation time. It is backfilled lazily on the first `GetSandboxSettings` call if no explicit revision exists in the policy history table. See `crates/clustershell-server/src/grpc.rs` -- `get_sandbox_settings()`.
...
In gRPC mode, the sandbox spawns a background task that periodically polls the gateway for policy updates. See `crates/clustershell-sandbox/src/lib.rs` -- `run_policy_poll_loop()`.

| Parameter | Default | Override |
|-----------|---------|----------|
| Poll interval | 10 seconds | `CLUSTERSHELL_POLICY_POLL_INTERVAL_SECS` environment variable |

The poll loop:

1. Connects a reusable gRPC client (`CachedClusterShellClient`) to avoid per-poll TLS handshake overhead.
...
See `crates/clustershell-sandbox/src/grpc_client.rs` -- `CachedClusterShellClient`.
...
The `clustershell policy` subcommand group manages live policy updates:

```bash
# Push a new policy to a running sandbox
clustershell policy set <sandbox-name> --policy updated-policy.yaml

# Push and wait for the sandbox to load it (with 60s timeout)
clustershell policy set <sandbox-name> --policy updated-policy.yaml --wait

# Push and wait with a custom timeout
clustershell policy set <sandbox-name> --policy updated-policy.yaml --wait --timeout 120

# Set a gateway-global policy (overrides all sandbox policies)
clustershell policy set --global --policy policy.yaml --yes

# Delete the gateway-global policy (restores sandbox-level control)
clustershell policy delete --global --yes

# View the current active policy and its status
clustershell policy get <sandbox-name>

# Inspect a specific revision
clustershell policy get <sandbox-name> --rev 3

# Print the full policy as YAML (round-trips with --policy input format)
clustershell policy get <sandbox-name> --full

# Combine: inspect a specific revision's full policy
clustershell policy get <sandbox-name> --rev 2 --full

# List policy revision history
clustershell policy list <sandbox-name> --limit 20
```
...
When `--full` is specified, the server includes the deserialized `SandboxPolicy` protobuf in the `SandboxPolicyRevision.policy` field (see `crates/clustershell-server/src/grpc.rs` -- `policy_record_to_revision()` with `include_policy: true`). The CLI converts this proto back to YAML via `policy_to_yaml()`, which uses a `BTreeMap` for `network_policies` to produce deterministic key ordering. See `crates/clustershell-cli/src/run.rs` -- `policy_to_yaml()`, `policy_get()`.

See `crates/clustershell-cli/src/main.rs` -- `PolicyCommands` enum, `crates/clustershell-cli/src/run.rs` -- `policy_set()`, `policy_get()`, `policy_list()`.
...
**Filesystem preparation**: Before the child process spawns, the supervisor creates any `read_write` directories that do not exist and sets their ownership to `process.run_as_user`:`process.run_as_group` via `chown()`. See `crates/clustershell-sandbox/src/lib.rs` -- `prepare_filesystem()`.

**Working directory**: When `include_workdir` is `true` and a `--workdir` is specified, the working directory path is appended to `read_write` if not already present. See `crates/clustershell-sandbox/src/sandbox/linux/landlock.rs` -- `apply()`.

**TLS directory**: When network proxy mode is active with TLS termination enabled, the directory `/etc/clustershell-tls` is automatically appended to `read_only` so sandbox processes can read the ephemeral CA certificate files.
...
See `crates/clustershell-sandbox/src/sandbox/linux/landlock.rs` -- `compat_level()`.
...
This happens before Landlock and seccomp are applied because `initgroups` needs access to `/etc/group` and `/etc/passwd`, which Landlock may subsequently block. The post-condition checks (steps 3, 5, 6) are async-signal-safe and add negligible overhead while guarding against hypothetical kernel-level defects. See `crates/clustershell-sandbox/src/process.rs` -- `drop_privileges()`.
...
See `crates/clustershell-sandbox/src/l7/mod.rs` -- `expand_access_presets()`.
...
See `crates/clustershell-sandbox/src/l7/mod.rs` -- `validate_l7_policies()` for validation, `sandbox-policy.rego` -- `endpoint_allowed` for the Rego glob matching rule.

**Rego implementation**: The Rego rules detect host wildcards via `contains(endpoint.host, "*")` and dispatch to `glob.match(lower(endpoint.host), ["."], lower(network.host))`. Exact-match hosts use a separate, faster `lower(endpoint.host) == lower(network.host)` rule. See `crates/clustershell-sandbox/data/sandbox-policy.rego`.
...
This normalization happens in `crates/clustershell-sandbox/src/opa.rs` -- `normalize_endpoint_ports()` (YAML path) and `proto_to_opa_data_json()` (proto path).
...
**YAML serialization**: When serializing policy back to YAML (e.g., `nav policy get --full`), a single-element `ports` array is emitted as the compact `port: N` scalar form. Multi-element arrays are emitted as `ports: [N, M]`. See `crates/clustershell-policy/src/lib.rs` -- `from_proto()`.
...
Inference routing to `inference.local` is handled by the proxy's `InferenceContext`, not by the OPA policy engine or an `inference` block in the policy YAML. The proxy intercepts HTTPS CONNECT requests to `inference.local` and routes matching inference API requests (e.g., `POST /v1/chat/completions`, `POST /v1/messages`) through the sandbox-local `clustershell-router`. See [Inference Routing](inference-routing.md) for details on route configuration and the router architecture.

The proxy always runs in proxy mode so that `inference.local` is addressable from within the sandbox's network namespace. Inference route sources are configured separately from policy: via `--inference-routes` (file mode) or fetched from the gateway's inference bundle (cluster mode). See `crates/clustershell-sandbox/src/proxy.rs` -- `InferenceContext`, `crates/clustershell-sandbox/src/l7/inference.rs`.
...
The sandbox always runs in **proxy mode**. Both file mode and gRPC mode set `NetworkMode::Proxy` unconditionally. This ensures all egress is evaluated by OPA and the virtual hostname `inference.local` is always addressable for inference routing. See `crates/clustershell-sandbox/src/lib.rs` -- `load_policy()`, `crates/clustershell-sandbox/src/policy.rs` -- `TryFrom<ProtoSandboxPolicy>`.
...
**Implementation path**: After L4 CONNECT is allowed, the proxy calls `query_l7_config()` which evaluates the Rego rule `data.clustershell.sandbox.matched_endpoint_config`. This rule only matches endpoints that have a `protocol` field set (see `sandbox-policy.rego` line `ep.protocol`). If a config is returned, the proxy enters `relay_with_inspection()` instead of `copy_bidirectional()`. See `crates/clustershell-sandbox/src/proxy.rs` -- `handle_tcp_connection()`.

**Validation requirement**: When `protocol` is set, either `rules` or `access` must also be present. An endpoint with `protocol` but no rules/access is rejected at validation time because it would deny all traffic (no allow rules means nothing matches). See `crates/clustershell-sandbox/src/l7/mod.rs` -- `validate_l7_policies()`.
...
7. Rewrites the request: absolute-form → origin-form (`GET /path HTTP/1.1`), strips hop-by-hop headers, adds `Via: 1.1 clustershell-sandbox` and `Connection: close`
...
**Implementation**: See `crates/clustershell-sandbox/src/proxy.rs` -- `handle_forward_proxy()`, `parse_proxy_uri()`, `rewrite_forward_request()`.
...
- The sandbox supervisor generates an ephemeral CA at startup (`SandboxCa::generate()`) and writes it to `/etc/clustershell-tls/`.
...
- A combined CA bundle (system CAs + sandbox CA) is written to `/etc/clustershell-tls/ca-bundle.pem` so `SSL_CERT_FILE` replaces the default trust store while still trusting real CAs.

**Certificate caching**: Per-hostname leaf certificates are cached (up to 256 entries, then the entire cache is cleared). See `crates/clustershell-sandbox/src/l7/tls.rs` -- `CertCache`.
...
The response includes an `X-ClusterShell-Policy` header and `Connection: close`. See `crates/clustershell-sandbox/src/l7/rest.rs` -- `send_deny_response()`.
...
The seccomp filter uses a default-allow policy (`SeccompAction::Allow`) with specific `socket()` syscall rules that return `EPERM` when the first argument (domain) matches a blocked value. See `crates/clustershell-sandbox/src/sandbox/linux/seccomp.rs`.
...
The child process enters the namespace via `setns(fd, CLONE_NEWNET)` in `pre_exec`. This provides hard network isolation -- even if a process ignores proxy environment variables, it can only reach the host veth IP, where the proxy listens. See `crates/clustershell-sandbox/src/sandbox/linux/netns.rs`.
...
See `crates/clustershell-sandbox/src/procfs.rs`, `crates/clustershell-sandbox/src/identity.rs`.
...
These errors are returned by the gateway's `UpdateSandboxPolicy` handler and reject the update before it is persisted. See `crates/clustershell-server/src/grpc.rs`.
...
See `crates/clustershell-sandbox/src/l7/mod.rs` -- `validate_l7_policies()`.
...
Functions in `crates/clustershell-sandbox/src/proxy.rs` implement the SSRF checks:
...
The conversion is performed in `crates/clustershell-sandbox/src/opa.rs` -- `proto_to_opa_data_json()`.
...
This ordering is intentional: privilege dropping needs `/etc/group` and `/etc/passwd` access, which Landlock may subsequently restrict. Network namespace entry must happen before any network operations. See `crates/clustershell-sandbox/src/process.rs` -- `spawn_impl()`.
...
The filter applies to both one-shot mode (`GetSandboxLogs` RPC) and streaming mode (`--tail`, via `WatchSandbox` RPC). In both cases, the server evaluates `source_matches()` before sending each log line to the client. See `crates/clustershell-server/src/grpc.rs` -- `source_matches()`, `get_sandbox_logs()`.
...
The filter is applied server-side via `level_matches()` in both one-shot and streaming modes. See `crates/clustershell-server/src/grpc.rs` -- `level_matches()`.
...
An empty `sources`/`log_sources` list means no source filtering (all sources pass). An empty `min_level`/`log_min_level` string means no level filtering (all levels pass). See `proto/clustershell.proto`.
