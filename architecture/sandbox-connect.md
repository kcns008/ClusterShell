...
**File**: `crates/clustershell-cli/src/ssh.rs`
...
- ClusterShell-managed SSH config helpers -- install a single `Include` entry in
  `~/.ssh/config` and maintain generated `Host clustershell-<name>` blocks in a
  separate ClusterShell-owned config file for editor workflows

These are re-exported from `crates/clustershell-cli/src/run.rs` for backward compatibility.

### CLI `ssh-proxy` subcommand

**File**: `crates/clustershell-cli/src/main.rs` (line ~139, `Commands::SshProxy`)
...
**Files**: `proto/clustershell.proto`, `crates/clustershell-server/src/grpc.rs`
...
**File**: `crates/clustershell-server/src/ssh_tunnel.rs`
...
**File**: `crates/clustershell-server/src/multiplex.rs`

The gateway runs a single listener that multiplexes gRPC and HTTP on the same port. `MultiplexedService` routes based on the `content-type` header: requests with `application/grpc` go to the gRPC router; all others (including HTTP CONNECT) go to the HTTP router. The HTTP router (`crates/clustershell-server/src/http.rs`) merges health endpoints with the SSH tunnel router.
...
**File**: `crates/clustershell-server/src/grpc.rs` (functions `stream_exec_over_ssh`, `start_single_use_ssh_proxy`, `run_exec_with_russh`)
...
1. `crates/clustershell-cli/src/main.rs` -- `SandboxCommands::Connect { name }` dispatches to `run::sandbox_connect()`
2. `crates/clustershell-cli/src/ssh.rs` -- `sandbox_connect()` calls `ssh_session_config()`:
   - Resolves sandbox name to ID via `GetSandbox` gRPC
   - Creates an SSH session via `CreateSshSession` gRPC
   - Builds a `ProxyCommand` string: `<clustershell-exe> ssh-proxy --gateway <url> --sandbox-id <id> --token <token>`
...
6. `crates/clustershell-cli/src/ssh.rs` -- `sandbox_ssh_proxy()`:
...
When `clustershell sandbox create` launches a `--no-keep` command or shell, it keeps the CLI process alive instead of `exec()`-ing into SSH so it can delete the sandbox after SSH exits. The default create flow, along with `--forward`, keeps the sandbox running.

### Port Forwarding (`forward start`)

`clustershell forward start <port> <name>` opens a local SSH tunnel so connections to `127.0.0.1:<port>`
...
- With `-d`/`--background`, SSH forks after auth and the CLI exits. The PID is
  tracked in `~/.config/clustershell/forwards/<name>-<port>.pid` along with sandbox id metadata.
- `clustershell forward stop <port> <name>` validates PID ownership and then kills a background forward.
- `clustershell forward list` shows all tracked forwards.
- `clustershell forward stop` and `clustershell forward list` are local operations and do not require
  resolving an active cluster.
- `clustershell sandbox create --forward <port>` starts a background forward before connect/exec, including
  when no trailing command is provided.
- `clustershell sandbox delete` auto-stops any active forwards for the deleted sandbox.

#### TUI

The TUI (`crates/clustershell-tui/`) supports port forwarding through the create sandbox modal. Users
...
request. PID tracking uses the same `~/.config/clustershell/forwards/` directory as the CLI.

#### Shared forward module

**File**: `crates/clustershell-core/src/forward.rs`

Port forwarding PID management and SSH utility functions are shared between the CLI and TUI:

- `forward_dir()` -- returns `~/.config/clustershell/forwards/`, creating it if needed
...
The sandbox SSH server (`crates/clustershell-sandbox/src/ssh.rs`) implements
...
**Files**: `crates/clustershell-cli/src/ssh.rs`, `crates/clustershell-cli/src/run.rs`
...
#### `clustershell sandbox upload` / `clustershell sandbox download`

Standalone commands support bidirectional file transfer:

```bash
# Push local files up to sandbox
clustershell sandbox upload <name> <local-path> [<sandbox-path>]

# Pull sandbox files down to local
clustershell sandbox download <name> <sandbox-path> [<local-path>]
```
...
**File**: `crates/clustershell-sandbox/src/ssh.rs` -- `verify_preface()`
...
Both the gateway (`crates/clustershell-server/src/ssh_tunnel.rs` -- `build_preface()`) and the gRPC exec path (`crates/clustershell-server/src/grpc.rs` -- `build_preface()`) use identical logic:
...
`run_ssh_server()` in `crates/clustershell-sandbox/src/ssh.rs`:
...
3. Configures the shell command with environment variables:
   - `CLUSTERSHELL_SANDBOX=1`, `HOME=/sandbox`, `USER=sandbox`, `TERM=<from pty request>`
...
**File**: `crates/clustershell-server/src/ssh_tunnel.rs` (gateway), `crates/clustershell-server/src/grpc.rs` (exec)
...
**Proto**: `proto/clustershell.proto` -- `CreateSshSessionRequest` / `CreateSshSessionResponse`
...
**Proto**: `proto/clustershell.proto` -- `SshSession` message
...
**Proto**: `proto/clustershell.proto` -- `ExecSandboxRequest` / `ExecSandboxEvent`
...
**File**: `crates/clustershell-core/src/forward.rs` -- `resolve_ssh_gateway()`
...
This function is shared between the CLI and TUI via the `clustershell-core::forward` module.
...
The NSSH1 handshake secret (`CLUSTERSHELL_SSH_HANDSHAKE_SECRET`) is required. Both the server and sandbox will refuse to start if the secret is empty or unset. For cluster deployments the secret is auto-generated by the entrypoint script (`deploy/docker/cluster-entrypoint.sh`) via `openssl rand -hex 32` and injected into the Helm values.
...
**File**: `crates/clustershell-core/src/config.rs` -- `Config` struct
...
|--------------------------------------|-------------|
| `CLUSTERSHELL_SSH_LISTEN_ADDR`          | Address for the embedded SSH server to bind |
| `CLUSTERSHELL_SSH_HANDSHAKE_SECRET`     | Shared secret for NSSH1 handshake validation |
| `CLUSTERSHELL_SSH_HANDSHAKE_SKEW_SECS`  | Allowed clock skew for handshake timestamp |

### CLI TLS options

| Flag / Env Var              | Description |
|-----------------------------|-------------|
| `--tls-ca` / `CLUSTERSHELL_TLS_CA`       | CA certificate for gateway verification |
| `--tls-cert` / `CLUSTERSHELL_TLS_CERT`   | Client certificate for mTLS |
| `--tls-key` / `CLUSTERSHELL_TLS_KEY`     | Client private key for mTLS |
