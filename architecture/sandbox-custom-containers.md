...
Users can run `clustershell sandbox create --from <source>` to launch a sandbox with a custom container image while keeping the `clustershell-sandbox` process supervisor in control.
...
The supervisor binary (`clustershell-sandbox`) is **always side-loaded** from the k3s node filesystem via a read-only `hostPath` volume. It is never baked into sandbox images. This applies to all sandbox pods — whether using the default community base image, a custom image, or a user-built Dockerfile.

```mermaid
flowchart TB
    subgraph node["K3s Node"]
        bin["/opt/clustershell/bin/clustershell-sandbox
    ...
    subgraph pod["Pod"]
        subgraph agent["Agent Container"]
            ...
            Command: /opt/clustershell/bin/clustershell-sandbox
            Volume: /opt/clustershell/bin (ro hostPath)
            Env: CLUSTERSHELL_SANDBOX_ID, CLUSTERSHELL_ENDPOINT, ...
...
1. Adds a `hostPath` volume named `clustershell-supervisor-bin` pointing to `/opt/clustershell/bin` on the node.
2. Mounts it read-only at `/opt/clustershell/bin` in the agent container.
3. Overrides the agent container's `command` to `/opt/clustershell/bin/clustershell-sandbox`.
...
```bash
clustershell sandbox create --from myimage:latest -- echo "hello from custom container"
```
...
```bash
clustershell sandbox create --from ./Dockerfile -- echo "built and running"
clustershell sandbox create --from ./my-sandbox/  # directory with Dockerfile
```

## Supervisor Behavior in Custom Images

The `clustershell-sandbox` supervisor adapts to arbitrary environments:

- **Log file fallback**: Attempts to open `/var/log/clustershell.log` for append; silently falls back to stdout-only logging if the path is not writable.
- **Command resolution**: Executes the command from CLI args, then the `CLUSTERSHELL_SANDBOX_COMMAND` env var (set to `sleep infinity` by the server), then `/bin/bash` as a last resort.
...
| `CLUSTERSHELL_COMMUNITY_REGISTRY` env var | Allows organizations to host their own community sandbox registry |
...
| Command override | Ensures `clustershell-sandbox` is the entrypoint regardless of the image's default CMD |
| Clear `run_as_user/group` for custom images | Prevents startup failure when the image lacks the default `sandbox` user |
| Non-fatal log file init | `/var/log/clustershell.log` may be unwritable in arbitrary images; falls back to stdout |
...
- The supervisor binary must be present on the k3s node at `/opt/clustershell/bin/clustershell-sandbox` (embedded in the cluster image at build time)
