...
This example demonstrates ClusterShell's inference interception and routing.
A sandbox process sends inference traffic to `inference.local`, and
ClusterShell intercepts and reroutes it to the configured backend.
...
Requires a running ClusterShell gateway and `NVIDIA_API_KEY` set in your shell.

```bash
# 1. Create a provider using your NVIDIA credentials
clustershell provider create --name nvidia --type nvidia --credential NVIDIA_API_KEY

# 2. Configure inference routing
clustershell inference set --provider nvidia --model meta/llama-3.1-8b-instruct
...
Run the sandbox binary directly with a route file — no ClusterShell cluster needed:
...
#### 1. Start a ClusterShell cluster
...
clustershell cluster inference set \
...
```bash
clustershell cluster inference get
```
...
The script targets `https://inference.local/v1` directly. ClusterShell
...
```bash
clustershell sandbox connect inference-demo
# Inside the sandbox:
python examples/inference/inference.py
```
...
```bash
clustershell sandbox delete inference-demo
```

## Customizing Routes

Edit `routes.yaml` to change which backend endpoint/model standalone mode uses.
In cluster mode, use `clustershell cluster inference set` instead.
