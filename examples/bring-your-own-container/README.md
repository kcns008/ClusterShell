...
ClusterShell replaces it with the sandbox supervisor (which manages SSH access,
...
# {"message": "hello from ClusterShell sandbox!"}
...
ClusterShell handles all the wiring automatically.  You build a standard
Linux container image — no ClusterShell-specific dependencies or
configuration required.  When you create a sandbox with `--from`,
ClusterShell ensures that sandboxing (network policy, filesystem isolation,
