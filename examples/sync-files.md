...
Move code, data, and artifacts between your local machine and a ClusterShell
sandbox using `clustershell sandbox upload` and `clustershell sandbox download`.
...
```bash
clustershell sandbox upload my-sandbox .
```
...
```bash
clustershell sandbox upload my-sandbox ./config.yaml /sandbox/config.yaml
```
...
```bash
clustershell sandbox download my-sandbox /sandbox/output ./output
```

Pull results to the current directory:

```bash
clustershell sandbox download my-sandbox /sandbox/results
```
...
```bash
clustershell sandbox create --sync -- python main.py
```
...
clustershell sandbox create --name dev --sync

# Make local changes, then push them
clustershell sandbox upload dev ./src /sandbox/src

# Run tests inside the sandbox
clustershell sandbox connect dev
# (inside sandbox) pytest

# Pull test artifacts back
clustershell sandbox download dev /sandbox/coverage ./coverage
```
