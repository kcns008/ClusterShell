...
cleanup() {
    ...
    printf '\n'
    step "Cleaning up"
    clustershell sandbox delete "$SANDBOX_NAME" 2>/dev/null || true
}
trap cleanup EXIT
...
run clustershell sandbox create \
    --name "$SANDBOX_NAME" \
    --keep \
    --no-auto-providers \
    --no-tty \
...
clustershell sandbox ssh-config "$SANDBOX_NAME" > "$SSH_CONFIG"
...
sleep 2
printf "  ${BOLD}\$ clustershell logs ${SANDBOX_NAME} --since 1m -n 10${RESET}\n"
clustershell logs "$SANDBOX_NAME" --since 1m -n 10 2>&1 \
...
run clustershell policy set "$SANDBOX_NAME" \
    --policy "$POLICY_FILE" \
    --wait
...
sleep 2
printf "  ${BOLD}\$ clustershell logs ${SANDBOX_NAME} --level warn --since 1m -n 10${RESET}\n"
clustershell logs "$SANDBOX_NAME" --level warn --since 1m -n 10 2>&1 \
