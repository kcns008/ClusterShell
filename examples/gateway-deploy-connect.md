...
Deploy a ClusterShell gateway, verify it is reachable, and run your first
...
- ClusterShell CLI installed (`clustershell`)
...
```bash
clustershell sandbox create --name hello -- echo "it works"
```

### 4. Clean up

```bash
clustershell sandbox delete hello
...
```bash
clustershell status
clustershell sandbox create --name remote-test -- echo "running on remote host"
clustershell sandbox connect remote-test
```
...
```bash
clustershell sandbox delete remote-test
clustershell gateway destroy
```
...
```bash
clustershell gateway select my-other-cluster
```
...
1. Pulls the ClusterShell cluster image and provisions a container.
...
```bash
docker logs clustershell-cluster-clustershell
```
