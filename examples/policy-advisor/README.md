...
A capture-the-flag challenge that walks you through ClusterShell's policy
...
- A running ClusterShell gateway (`clustershell gateway start` or a remote gateway)
- The `clustershell` CLI installed
...
- **Flush interval** -- the denial aggregator flushes every 10 seconds by
  default.  Set `CLUSTERSHELL_DENIAL_FLUSH_INTERVAL_SECS=5` in the sandbox
  environment for faster feedback during the demo.
- **CLI alternative** -- you can approve drafts from the CLI instead of the
  TUI:
  ```bash
   clustershell rule get advisor-ctf                    # list pending
   clustershell rule approve advisor-ctf --chunk-id ID  # approve one
   clustershell rule approve-all advisor-ctf             # approve all
  ```
...
```bash
clustershell sandbox delete advisor-ctf
```
