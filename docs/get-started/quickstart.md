...
title:
  page: Quickstart
  nav: Quickstart
description: Install the ClusterShell CLI and create your first sandboxed AI agent in two commands.
topics:
- Generative AI
- Cybersecurity
tags:
...
```console
$ clustershell sandbox create -- opencode
```
...
Use the `--from` flag to pull other ClusterShell sandbox images from the [NVIDIA Container Registry](https://registry.nvidia.com/).
For example, to pull the `base` image, run the following command:

```console
$ clustershell sandbox create --from base
```
...
Running `clustershell sandbox create` without a gateway auto-bootstraps a local one.
...
:::{note}
Deploy an ClusterShell gateway on Brev by clicking **Deploy** on the [ClusterShell Launchable](https://brev.nvidia.com/launchable/deploy/now?launchableID=env-3Ap3tL55zq4a8kew1AuW0FpSLsg).
:::
...
```console
$ clustershell gateway add https://<your-port-8080-url>.brevlab.com
$ clustershell status
```
...
After `clustershell status` shows the gateway as healthy, all subsequent commands route through the SSH tunnel.
