...
Static fields are set at sandbox creation time. Changing them requires destroying and recreating the sandbox. Dynamic fields can be updated on a running sandbox with `clustershell policy set` and take effect without restarting.
...
|---|---|---|---|---|
| `compatibility` | string | No | `best_effort`, `hard_requirement` | How ClusterShell handles kernel ABI differences. `best_effort` uses the highest Landlock ABI the host kernel supports. `hard_requirement` fails if the required ABI is unavailable. |
