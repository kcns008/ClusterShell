...
The CLI loads three PEM files from `~/.config/clustershell/gateways/<name>/mtls/`:
...
**Registration flow** (`clustershell gateway add https://gateway.example.com`):
...
**Re-authentication**: If the token expires, run `clustershell gateway login` to open the browser flow again and update the stored token.
...
All gateway credentials and metadata are stored under `~/.config/clustershell/`:
