...
| Value | Description |
|---|---|
| Provider record | The credential backend ClusterShell uses to authenticate with the upstream model host. |
...
Create a provider that holds the backend credentials you want ClusterShell to use.
...
```console
$ clustershell provider create --name nvidia-prod --type nvidia --from-existing
```
...
$ clustershell provider create \
    ...
    --config OPENAI_BASE_URL=http://host.clustershell.internal:11434/v1
```

Use `--config OPENAI_BASE_URL` to point to any OpenAI-compatible server running where the gateway runs. For host-backed local inference, use `host.clustershell.internal` or the host's LAN IP. Avoid `127.0.0.1` and `localhost`. Set `OPENAI_API_KEY` to a dummy value if the server does not require authentication.
...
```console
$ clustershell provider create --name anthropic-prod --type anthropic --from-existing
```
...
```console
$ clustershell inference update --model nvidia/nemotron-3-nano-30b-a3b
```

Or switch providers without repeating the current model:

```console
$ clustershell inference update --provider openai-prod
```
...
When the upstream runs on the same machine as the gateway, bind it to `0.0.0.0` and point the provider at `host.clustershell.internal` or the host's LAN IP. `127.0.0.1` and `localhost` usually fail because the request originates from the gateway or sandbox runtime, not from your shell.

If the gateway runs on a remote host or behind a cloud deployment, `host.clustershell.internal` points to that remote machine, not to your laptop. A locally running Ollama or vLLM process is not reachable from a remote gateway unless you add your own tunnel or shared network path. Ollama also supports cloud-hosted models that do not require local hardware.

### Verify the Endpoint from a Sandbox

`clustershell inference set` and `clustershell inference update` verify the resolved upstream endpoint by default before saving the configuration. If the endpoint is not live yet, retry with `--no-verify` to persist the route without the probe.

`clustershell inference get` confirms the current saved configuration. To confirm end-to-end connectivity from a sandbox, run:
