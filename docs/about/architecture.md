---
title:
  page: How ClusterShell Works
  nav: How It Works
description: ClusterShell architecture overview covering the gateway, sandbox, policy engine, and privacy router.
topics:
- Generative AI
- Cybersecurity
tags:
...
# How ClusterShell Works

ClusterShell runs inside a Docker container. Each sandbox is an isolated environment managed through the gateway. Four components work together to keep agents secure.

```{figure} architecture.svg
:alt: ClusterShell architecture diagram showing the component layout
:align: center
...
3. If the target is `https://inference.local`, the proxy handles it as managed inference before policy evaluation. ClusterShell strips sandbox-supplied credentials, injects the configured backend credentials, and forwards the request to the managed model endpoint.
...
ClusterShell can run locally, on a remote host, or behind a cloud proxy. The architecture is identical in all cases — only the Docker container location and authentication mode change.

| Mode | Description | Command |
|---|---|---|
| **Local** | The gateway runs inside Docker on your workstation. The CLI provisions it automatically on first use. | `clustershell gateway start` |
| **Remote** | The gateway runs on a remote host via SSH. Only Docker is required on the remote machine. | `clustershell gateway start --remote user@host` |
| **Cloud** | A gateway already running behind a reverse proxy (e.g. Cloudflare Access). Register and authenticate via browser. | `clustershell gateway add https://gateway.example.com` |

You can register multiple gateways and switch between them with `clustershell gateway select`. For the full deployment and management workflow, refer to the [Gateways](../sandboxes/manage-gateways.md) section.
...
- To learn how ClusterShell enforces isolation across all protection layers, refer to [Sandboxes](../sandboxes/index.md).
