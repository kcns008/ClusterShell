...
title:
  page: Providers
  nav: Providers
description: Create and manage credential providers that inject API keys and tokens into ClusterShell sandboxes.
topics:
- Generative AI
- Cybersecurity
tags:
...
AI agents typically need credentials to access external services: an API key for the AI model provider, a token for GitHub or GitLab, and so on. ClusterShell manages these credentials as first-class entities called *providers*.
...
```console
$ clustershell provider create --name my-claude --type claude --from-existing
```
...
```console
$ clustershell provider create --name my-api --type generic --credential API_KEY=sk-abc123
```
...
```console
$ clustershell provider create --name my-api --type generic --credential API_KEY
```
...
```console
$ clustershell provider get my-claude
```

Update a provider's credentials:

```console
$ clustershell provider update my-claude --type claude --from-existing
```

Delete a provider:

```console
$ clustershell provider delete my-claude
```
...
```console
$ clustershell sandbox create --provider my-claude --provider my-github -- claude
```
...
When the trailing command in `clustershell sandbox create` is a recognized tool name (`claude`, `codex`, or `opencode`), the CLI auto-creates the required
...
| Ollama (local) | `ollama` | `openai` | `http://host.clustershell.internal:11434/v1` | `OPENAI_API_KEY` |
| LM Studio (local) | `lmstudio` | `openai` | `http://host.clustershell.internal:1234/v1` | `OPENAI_API_KEY` |
