...
title:
  ...
  nav: Policies
description: Apply, iterate, and debug sandbox network policies with hot-reload on running ClusterShell sandboxes.
topics:
- Generative AI
- Cybersecurity
tags:
...
Dynamic sections can be updated on a running sandbox with `clustershell policy set` and take effect without restarting.
...
```console
$ clustershell sandbox create --policy ./my-policy.yaml -- claude
```

`clustershell sandbox create` keeps the sandbox running after the initial command exits, which is useful when you plan to iterate on the policy. Add `--no-keep` if you want the sandbox deleted automatically instead.

To avoid passing `--policy` every time, set a default policy with an environment variable:

```console
$ export CLUSTERSHELL_SANDBOX_POLICY=./my-policy.yaml
$ clustershell sandbox create -- claude
```

The CLI uses the policy from `CLUSTERSHELL_SANDBOX_POLICY` whenever `--policy` is not explicitly provided.
...
1. Create the sandbox with your initial policy by following [Apply a Custom Policy](#apply-a-custom-policy) above (or set `CLUSTERSHELL_SANDBOX_POLICY`).

2. Monitor denials. Each log entry shows host, port, binary, and reason. Alternatively, use `clustershell term` for a live dashboard.

   ```console
   $ clustershell logs <name> --tail --source sandbox
   ```

3. Pull the current policy. Strip the metadata header (Version, Hash, Status) before reusing the file.

   ```console
   $ clustershell policy get <name> --full > current-policy.yaml
   ```
...
5. Push the updated policy. Exit codes: 0 = loaded, 1 = validation failed, 124 = timeout.
   ...
   ```console
   $ clustershell policy set <name> --policy current-policy.yaml --wait
   ```
...
```console
$ clustershell policy set --global --policy ./global-policy.yaml
```
...
```console
$ clustershell settings get <name>
```

## Debug Denied Requests

Check `clustershell logs <name> --tail --source sandbox` for the denied host, path, and binary.
...
Add these blocks to the `network_policies` section of your sandbox policy. Apply with `clustershell policy set <name> --policy <file> --wait`.
