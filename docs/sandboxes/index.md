...
description: Understand gateways, gateway types, sandbox lifecycle, supported agents, built-in default policy, and network access rules in ClusterShell.
topics:
- Generative AI
- Cybersecurity
tags:
...
| **Cloud** | Behind a reverse proxy (for example, Cloudflare Access) | Individual users accessing ClusterShell behind a cloud VM. Cloud gateways are not yet intended for shared team access. |
...
:::{tip}
You do not need to deploy a gateway manually. Running `clustershell sandbox create` without a gateway auto-bootstraps a local one for you.
:::
...
| Error | Something went wrong during provisioning or execution. Check logs with `clustershell logs` for details. |
| Deleting | The sandbox is being torn down. The system releases resources and purges credentials. |

## Sandbox Policies

ClusterShell ships a built-in policy that covers common agent workflows out of the box.
