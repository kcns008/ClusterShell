...
The gateway is the control plane for ClusterShell. All control-plane traffic between the CLI and running sandboxes flows through the gateway.
...
:::{tip}
You do not need to deploy a gateway manually. If you run `clustershell sandbox create` without a gateway, the CLI auto-bootstraps a local gateway for you.
:::

To use a different port or name:

```console
$ clustershell gateway start --port 9090
$ clustershell gateway start --name dev-local
```
...
```console
$ clustershell gateway start --remote user@hostname
```
...
```console
$ clustershell gateway start --remote user@hostname --ssh-key ~/.ssh/my_key
```
...
```console
$ clustershell gateway start --remote <username>@<spark-ssid>.local
```
:::

## Register an Existing Gateway

Use `clustershell gateway add` to register a gateway that is already running.
...
```console
$ clustershell gateway add https://gateway.example.com
```
...
```console
$ clustershell gateway add https://gateway.example.com --name production
```

If the token expires later, re-authenticate with:

```console
$ clustershell gateway login
```
...
```console
$ clustershell gateway add https://remote-host:8080 --remote user@remote-host
```

Or use the `ssh://` scheme to combine the SSH destination and gateway port:

```console
$ clustershell gateway add ssh://user@remote-host:8080
```
...
```console
$ clustershell gateway add https://127.0.0.1:8080 --local
```
...
```console
$ clustershell gateway select
```

Switch the active gateway:

```console
$ clustershell gateway select my-remote-cluster
```

Override the active gateway for a single command with `-g`:

```console
$ clustershell status -g my-other-cluster
```
...
$ clustershell gateway info --name my-remote-cluster
```
...
| `--registry-username` | Username for registry authentication. Defaults to `__token__` when `--registry-token` is set. Only needed for private registries. Also configurable with `CLUSTERSHELL_REGISTRY_USERNAME`. |
| `--registry-token` | Authentication token for pulling container images. For GHCR, a GitHub PAT with `read:packages` scope. Only needed for private registries. Also configurable with `CLUSTERSHELL_REGISTRY_TOKEN`. |
...
```console
$ clustershell gateway stop --name my-gateway
$ clustershell gateway destroy --name my-gateway
```
...
```console
$ clustershell doctor logs
$ clustershell doctor logs --tail              # stream live
$ clustershell doctor logs --lines 50          # last 50 lines
```
...
```console
$ clustershell gateway start --recreate
```
