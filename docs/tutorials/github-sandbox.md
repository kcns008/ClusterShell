...
- A working ClusterShell installation. Complete the {doc}`Quickstart </get-started/quickstart>` before proceeding.
...
- An [Anthropic account](https://console.anthropic.com/) with access to Claude Code. ClusterShell provides the sandbox runtime, not the agent. You must authenticate with your own account.
...
- **Terminal 1**: The sandbox terminal. You create the sandbox in this terminal by running `clustershell sandbox create` and interact with Claude Code inside it.
- **Terminal 2**: A terminal outside the sandbox on your machine. You use this terminal for viewing the sandbox logs with `clustershell term` and applying an updated policy with `clustershell policy set`.
...
$ clustershell provider create --name my-github --type github --from-existing
$ clustershell sandbox create --provider my-github -- claude
```

`clustershell sandbox create` keeps the sandbox running after Claude Code exits, so you can apply policy updates later without recreating the environment. Add `--no-keep` if you want the sandbox deleted automatically instead.
...
To find the name of running sandboxes, run `clustershell sandbox list` in terminal 2.
...
In terminal 2, launch the ClusterShell terminal:
...
```console
$ clustershell policy set <sandbox-name> --policy /tmp/sandbox-policy-update.yaml --wait
```
...
The push completes successfully. The `clustershell term` dashboard now shows `l7_decision=allow` entries for `api.github.com` and `github.com` where it previously showed denials.
...
```console
$ clustershell sandbox delete <sandbox-name>
```
