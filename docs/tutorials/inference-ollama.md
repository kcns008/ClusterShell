...
description: Run local and cloud models inside an ClusterShell sandbox using the Ollama community sandbox, or route sandbox requests to a host-level Ollama server.
topics:
- Generative AI
- Cybersecurity
tags:
...
This tutorial covers two ways to use Ollama with ClusterShell:
...
```console
$ clustershell sandbox create --from ollama
```
...
```console
$ clustershell sandbox create --from ollama -e OLLAMA_UPDATE=1
```
...
ClusterShell injects `host.clustershell.internal` so sandboxes and the gateway can reach the host machine. You can also use the host's LAN IP.

### Step 4: Set Inference Routing

```console
$ clustershell inference set --provider ollama --model qwen3.5:0.8b
```
...
- **`OPENAI_BASE_URL` wrong** — Use `http://host.clustershell.internal:11434/v1`, not `localhost` or `127.0.0.1`.
...
$ clustershell status
$ clustershell inference get
$ clustershell provider get ollama
```
