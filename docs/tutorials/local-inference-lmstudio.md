...
This tutorial describes how to configure ClusterShell to route inference requests to a local LM Studio server.
...
- Expose a local inference server to ClusterShell sandboxes.
- Verify end-to-end inference from inside a sandbox.

## Prerequisites

First, complete ClusterShell installation and follow the {doc}`/get-started/quickstart`.
...
LM Studio will listen to `127.0.0.1:1234` by default. For use with ClusterShell, you'll need to configure LM Studio to listen on all interfaces (`0.0.0.0`).
...
Add LM Studio as an OpenAI-compatible provider through `host.clustershell.internal`:
...
$ clustershell provider create \
    ...
    --config OPENAI_BASE_URL=http://host.clustershell.internal:1234/v1
```
...
$ clustershell provider create \
    ...
    --config ANTHROPIC_BASE_URL=http://host.clustershell.internal:1234
```
...
```console
$ clustershell inference set --provider lmstudio --model qwen/qwen3.5-2b
```

If the command succeeds, ClusterShell has verified that the upstream is reachable and accepts the expected OpenAI-compatible request shape.
...
```console
$ clustershell inference set --provider lmstudio-anthropic --model qwen/qwen3.5-2b
```

If the command succeeds, ClusterShell has verified that the upstream is reachable and accepts the expected Anthropic-compatible request shape.

::::

:::::

The active `inference.local` route is gateway-scoped, so only one provider and model pair is active at a time. Re-run `clustershell inference set` whenever you want to switch between OpenAI-compatible and Anthropic-compatible clients.
...
- `OPENAI_BASE_URL` uses `http://host.clustershell.internal:1234/v1` when you use an `openai` provider
- `ANTHROPIC_BASE_URL` uses `http://host.clustershell.internal:1234` when you use an `anthropic` provider
...
$ clustershell status
$ clustershell inference get
$ clustershell provider get lmstudio
$ clustershell provider get lmstudio-anthropic
```
