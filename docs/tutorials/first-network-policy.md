...
description: See how ClusterShell network policies work by creating a sandbox, observing default-deny in action, and applying a fine-grained L7 read-only rule.
topics:
- Generative AI
- Cybersecurity
tags:
...
This tutorial shows how ClusterShell's network policy system works in under five minutes. You create a sandbox, watch a request get blocked by the default-deny policy, apply a fine-grained L7 rule, and verify that reads are allowed while writes are blocked, all without restarting anything.
...
- A working ClusterShell installation. Complete the {doc}`/get-started/quickstart` before proceeding.
- Docker Desktop running on your machine.

:::{tip}
To run every step of this tutorial, you can also use the automated demo script at the [examples/sandbox-policy-quickstart](https://github.com/kcns008/ClusterShell/blob/main/examples/sandbox-policy-quickstart) directory in the NVIDIA ClusterShell repository. It runs the full walkthrough in under a minute but without any user interaction.
...
```console
$ clustershell sandbox create --name demo --keep --no-auto-providers
```
...
```console
$ clustershell logs demo --since 5m
```
...
```console
$ clustershell policy set demo --policy github-readonly.yaml --wait
```
...
```console
$ clustershell logs demo --level warn --since 5m
```
...
```console
$ clustershell sandbox delete demo
```
