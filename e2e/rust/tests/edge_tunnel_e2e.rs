...
//!
//! Prerequisites:
//! - A running clustershell gateway deployed with `--plaintext`
//! - The gateway's HTTP endpoint accessible (no TLS)
//! - The `clustershell` binary (built automatically from the workspace)
//!
//! These tests exercise the full CLI → WS tunnel → gRPC flow.
//!
//! Environment variables:
//! - `CLUSTERSHELL_GATEWAY`: Name of the active gateway (standard e2e var)
//!
//! The cluster must have been deployed with `clustershell gateway start --plaintext`
...
/// Run `clustershell <args>` using the system's configured gateway.
...
/// Run `clustershell <args>` with a custom config directory so the CLI reads
...
fn seed_edge_cluster_config(
    ...
    let clustershell_dir = config_dir.join("clustershell");
    let clusters_dir = clustershell_dir.join("clusters");

    // Write active_cluster file.
    std::fs::create_dir_all(&clustershell_dir).expect("create clustershell config dir");
    std::fs::write(clustershell_dir.join("active_cluster"), cluster_name)
...
/// `clustershell status` should report a healthy gateway when connected to a
...
async fn plaintext_cluster_status_reports_healthy() {
    ...
    assert_eq!(
        code, 0,
        "clustershell status should exit 0 against plaintext cluster:\n{clean}"
    );
...
/// dummy token, then runs `clustershell status` against the live plaintext
/// gateway.
///
...
async fn ws_tunnel_status_through_edge_proxy() {
    ...
    if !endpoint.starts_with("http://") {
        eprintln!(
            "Skipping ws_tunnel test: gateway endpoint is not plaintext HTTP: {endpoint}\n\
             Deploy with `clustershell gateway start --plaintext` for this test."
        );
        return;
    }
    ...
    assert_eq!(
        code, 0,
        "clustershell status through WS tunnel should exit 0:\n{clean}"
    );
    assert!(
        clean.to_lowercase().contains("healthy")
            || clean.to_lowercase().contains("running")
