...
use clustershell_e2e::harness::binary::{clustershell_cmd, clustershell_tty_cmd};
use clustershell_e2e::harness::output::{extract_field, strip_ansi};
...
async fn sandbox_list_names() -> Vec<String> {
    ...
    let output = cmd.output().await.expect("spawn clustershell sandbox list");
...
async fn sandbox_create_keeps_sandbox_after_tty_command_by_default() {
    let mut cmd = clustershell_tty_cmd(&["sandbox", "create", "--", "echo", "OK"]);
    cmd.stdout(Stdio::piped())
        .stderr(Stdio::piped());

    let output = cmd.output().await.expect("spawn clustershell sandbox create");
    let stdout = String::from_utf8_lossy(&output.stdout).to_string();
    let stderr = String::from_utf8_lossy(&output.stderr).to_string();
    let combined = normalize_output(&format!("{stdout}{stderr}"));
...
async fn sandbox_create_with_no_keep_cleans_up_after_tty_command() {
    let mut cmd = clustershell_tty_cmd(&["sandbox", "create", "--no-keep", "--", "echo", "OK"]);
