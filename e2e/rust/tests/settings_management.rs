...
use clustershell_e2e::harness::binary::{clustershell_bin, clustershell_cmd};
...
impl Drop for GlobalSettingCleanup {
    fn drop(&mut self) {
        let _ = std::process::Command::new(clustershell_bin())
            .args([
                "settings",
                "delete",
                "--global",
...
async fn run_cli(args: &[&str]) -> CliResult {
    ...
    let output = cmd.output().await.expect("spawn clustershell command");
