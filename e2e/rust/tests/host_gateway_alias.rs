...
use clustershell_e2e::harness::port::find_free_port;
...
async fn run_cli(args: &[&str]) -> Result<String, String> {
    ...
    let output = cmd
        .output()
        .await
        .map_err(|e| format!("failed to spawn clustershell {}: {e}", args.join(" ")))?;
...
network_policies:
  host_echo:
    name: host_echo
    endpoints:
      - host: host.clustershell.internal
        port: {port}
        allowed_ips:
          - "172.0.0.0/8"
    binaries:
...
#[tokio::test]
async fn sandbox_reaches_host_clustershell_internal_via_host_gateway_alias() {
    ...
    let guard = SandboxGuard::create(&[
        ...
        "--max-time",
        "15",
        &format!("http://host.clustershell.internal:{}/", server.port),
    ])
    .await
    .expect("sandbox create with host.clustershell.internal echo request");
...
#[tokio::test]
async fn sandbox_inference_local_routes_to_host_clustershell_internal() {
    ...
    let create_output = create_openai_provider(
        ...
        &format!("http://host.clustershell.internal:{}/v1", server.port),
    )
    .await
...
async fn inference_set_supports_no_verify_for_unreachable_endpoint() {
    ...
    create_openai_provider(
        ...
        "http://host.clustershell.internal:9/v1",
    )
    .await
