...
/// Run `clustershell <args>` with a given tmpdir as config (for persisting state
...
/// `clustershell gateway start --help` must show `--plaintext`.
#[tokio::test]
...
/// `clustershell gateway --help` must list `add` and `login` subcommands.
#[tokio::test]
...
/// `clustershell gateway add --help` must show the endpoint arg and gateway-type flags.
#[tokio::test]
...
/// `clustershell gateway login --help` is recognized.
#[tokio::test]
...
/// `clustershell gateway add <endpoint>` (cloud gateway) should:
