...
/// Run `clustershell <args>` in an isolated environment where Docker is
...
async fn run_without_docker(args: &[&str]) -> (String, i32, std::time::Duration) {
    ...
    cmd.args(args)
        ...
        .env("DOCKER_HOST", "unix:///tmp/clustershell-e2e-nonexistent.sock")
...
/// `clustershell gateway start` with no Docker should fail within seconds
...
/// `clustershell gateway start --recreate` should also fail fast when
...
/// `clustershell sandbox create` triggers auto-bootstrap when no gateway
...
/// `clustershell doctor check` with Docker unavailable should fail fast
...
/// `clustershell doctor check` output should include the check label
...
/// `clustershell doctor check` with Docker unavailable should include
...
/// When Docker IS available, `clustershell doctor check` should pass and
/// report the version.
///
