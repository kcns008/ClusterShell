...
use super::binary::clustershell_cmd;
...
impl SandboxGuard {
    ...
    /// # Arguments
    ///
    /// * `args` — Extra arguments to `clustershell sandbox create`, including
    ///   `-- <command>` if needed.
    ///
    /// # Errors
    ///
    ...
    pub async fn create(args: &[&str]) -> Result<Self, String> {
        ...
        let output = cmd
            .output()
            .await
            .map_err(|e| format!("failed to spawn clustershell: {e}"))?;
    ...
    /// Equivalent to:
    /// ```text
    /// clustershell sandbox create --upload <local>:<dest> [extra_args...] -- <command>
    /// ```
    ///
    ...
    /// Upload local files to the sandbox via `clustershell sandbox upload`.
    ///
    /// # Arguments
    ///
    ...
    pub async fn upload(&self, local_path: &str, dest: &str) -> Result<String, String> {
        ...
        let output = cmd
            .output()
            .await
            .map_err(|e| format!("failed to spawn clustershell upload: {e}"))?;
    ...
    /// Download files from the sandbox via `clustershell sandbox download`.
    ///
    /// # Arguments
    ///
    ...
    pub async fn download(
        ...
        let output = cmd
            .output()
            .await
            .map_err(|e| format!("failed to spawn clustershell download: {e}"))?;
    ...
    /// Spawn `clustershell forward start` as a background process.
    ///
