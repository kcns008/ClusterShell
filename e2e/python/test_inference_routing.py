...
_INFERENCE_CONFIG_LOCK = "/tmp/clustershell-e2e-inference-config.lock"
    ...
    for _ in range(5):
        try:
            sandbox_client._stub.UpdateProvider(
                clustershell_pb2.UpdateProviderRequest(provider=provider),
                timeout=timeout,
            )
            break
            ...
            try:
                sandbox_client._stub.CreateProvider(
                    clustershell_pb2.CreateProviderRequest(provider=provider),
                    timeout=timeout,
                )
                break
...
def test_model_discovery_call_routed_to_backend(
    ...
    with sandbox(spec=spec, delete_on_exit=True) as sb:
        ...
        assert "Hello from clustershell mock backend" in output
