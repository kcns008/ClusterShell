...
from clustershell._proto import datamodel_pb2, clustershell_pb2, sandbox_pb2
...
def provider(
    ...
    *,
    ...
    stub.CreateProvider(
        clustershell_pb2.CreateProviderRequest(
...
def _delete_provider(stub: object, name: str) -> None:
    ...
    try:
        stub.DeleteProvider(clustershell_pb2.DeleteProviderRequest(name=name))
...
def test_provider_credentials_available_as_env_vars(
    ...
    with provider(
        ...
        with sandbox(spec=spec, delete_on_exit=True) as sb:
            ...
            assert value == "clustershell:resolve:env:ANTHROPIC_API_KEY"
...
def test_generic_provider_credentials_available_as_env_vars(
    ...
    with provider(
        ...
        with sandbox(spec=spec, delete_on_exit=True) as sb:
            ...
            assert (
                result.stdout.strip()
                == "clustershell:resolve:env:CUSTOM_SERVICE_TOKEN|clustershell:resolve:env:CUSTOM_SERVICE_URL"
            )
...
def test_nvidia_provider_injects_nvidia_api_key_env_var(
    ...
    with provider(
        ...
        with sandbox(spec=spec, delete_on_exit=True) as sb:
            ...
            assert result.exit_code == 0, result.stderr
            assert result.stdout.strip() == "clustershell:resolve:env:NVIDIA_API_KEY"
...
def test_ssh_handshake_secret_not_visible_in_exec_environment(
    ...
    def read_handshake_secret() -> str:
        ...
        return os.environ.get("CLUSTERSHELL_SSH_HANDSHAKE_SECRET", "NOT_SET")
    ...
    with provider(
        ...
        with sandbox(spec=spec, delete_on_exit=True) as sb:
            fetched = sandbox_client._stub.GetSandbox(
                clustershell_pb2.GetSandboxRequest(name=sb.sandbox.name)
            )
...
def test_update_provider_preserves_unset_credentials_and_config(
    ...
    try:
        ...
        stub.UpdateProvider(
            clustershell_pb2.UpdateProviderRequest(
                provider=datamodel_pb2.Provider(
                    name=name,
                    type="",
        ...
        got = stub.GetProvider(clustershell_pb2.GetProviderRequest(name=name))
        p = got.provider
