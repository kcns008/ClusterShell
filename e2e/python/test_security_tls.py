...
These tests verify that the ClusterShell server correctly requires valid client
certificates signed by the cluster CA.  Only callers presenting the provisioned
mTLS client cert should be able to reach the ClusterShell gRPC API; all other
...
from clustershell._proto import clustershell_pb2, clustershell_pb2_grpc
...
def _resolve_cluster_name() -> str:
    env_cluster = os.environ.get("CLUSTERSHELL_GATEWAY")
    if env_cluster:
        return env_cluster
...
def _cluster_metadata(cluster_name: str) -> dict:
    metadata_path = (
        _xdg_config_home() / "clustershell" / "gateways" / cluster_name / "metadata.json"
    )
    return json.loads(metadata_path.read_text())


def _mtls_dir(cluster_name: str) -> pathlib.Path:
    return _xdg_config_home() / "clustershell" / "gateways" / cluster_name / "mtls"
...
def server_endpoint(cluster_name: str) -> tuple[str, int, str]:
    """Return (host, port, scheme) for the ClusterShell server."""
...
class TestServerMtlsEnforcement:
    ...
    def test_authenticated_client_succeeds(
        ...
        try:
            stub = clustershell_pb2_grpc.ClusterShellStub(channel)
            response = stub.Health(clustershell_pb2.HealthRequest(), timeout=10)
            assert response.status == clustershell_pb2.SERVICE_STATUS_HEALTHY
        finally:
            channel.close()
    ...
    def test_no_client_cert_rejected(
        ...
        try:
            ...
            with pytest.raises(grpc.RpcError) as exc_info:
                stub.Health(clustershell_pb2.HealthRequest(), timeout=10)
