...
from clustershell._proto import datamodel_pb2, sandbox_pb2

if TYPE_CHECKING:
    from collections.abc import Callable

    from clustershell import Sandbox, SandboxClient
...
def _read_clustershell_log():
    """Return a closure that reads the clustershell log file(s).

    Since the sandbox uses a rolling file appender, logs are written to
    date-stamped files like ``/var/log/clustershell.YYYY-MM-DD.log`` instead
    of a single ``/var/log/clustershell.log``.  This helper globs for all
    ...
    def fn():
        ...
        logs = []
        for path in sorted(glob.glob("/var/log/clustershell*.log*")):
            try:
                with open(path) as f:
                    logs.append(f.read())
...
def test_l4_log_fields(
    ...
    with sandbox(spec=spec, delete_on_exit=True) as sb:
        ...
        log_result = sb.exec_python(_read_clustershell_log())
...
def test_l7_tls_ca_trust_store_injected(
        ...
        return _json.dumps(
            {
                ...
                "ca_cert_exists": os.path.exists("/etc/clustershell-tls/clustershell-ca.pem"),
                "bundle_exists": os.path.exists("/etc/clustershell-tls/ca-bundle.pem"),
            }
        )
    ...
    with sandbox(spec=spec, delete_on_exit=True) as sb:
        ...
        assert env["ca_cert_exists"], "clustershell-ca.pem should exist"
        assert env["bundle_exists"], "ca-bundle.pem should exist"
        assert "clustershell-tls" in env["SSL_CERT_FILE"]
        assert "clustershell-tls" in env["NODE_EXTRA_CA_CERTS"]
...
def test_l7_tls_deny_response_format(
    ...
    with sandbox(spec=spec, delete_on_exit=True) as sb:
        ...
        assert "X-ClusterShell-Policy" in resp["headers"]
...
def test_live_policy_update_and_logs(
    ...
    from clustershell._proto import clustershell_pb2, sandbox_pb2
    ...
    with sandbox(spec=spec, delete_on_exit=True) as sb:
        ...
        status_resp = stub.GetSandboxPolicyStatus(
            clustershell_pb2.GetSandboxPolicyStatusRequest(name=sandbox_name, version=0)
        )
        ...
        update_resp = stub.UpdateConfig(
            clustershell_pb2.UpdateConfigRequest(
                name=sandbox_name,
                policy=policy_a,
            )
        )
        ...
        while time.time() < deadline:
            status_resp = stub.GetSandboxPolicyStatus(
                clustershell_pb2.GetSandboxPolicyStatusRequest(
                    name=sandbox_name, version=new_version
                )
            )
            status = status_resp.revision.status
            if status == clustershell_pb2.POLICY_STATUS_LOADED:
                loaded = True
                break
            if status == clustershell_pb2.POLICY_STATUS_FAILED:
                pytest.fail(
        ...
        list_resp = stub.ListSandboxPolicies(
            clustershell_pb2.ListSandboxPoliciesRequest(name=sandbox_name, limit=10)
        )
        ...
        loaded_count = sum(
            ...
            if r.status == clustershell_pb2.POLICY_STATUS_LOADED
        )
        ...
        get_resp = stub.GetSandbox(clustershell_pb2.GetSandboxRequest(name=sandbox_name))
        sandbox_id = get_resp.sandbox.id

        logs_resp = stub.GetSandboxLogs(
            clustershell_pb2.GetSandboxLogsRequest(sandbox_id=sandbox_id, lines=500)
        )
...
def test_forward_proxy_log_fields(
    ...
    with sandbox(spec=spec, delete_on_exit=True) as sb:
        ...
        # Read the log
        result = sb.exec_python(_read_clustershell_log())
        assert result.exit_code == 0, result.stderr
        log = result.stdout
...
def _verify_sandbox_functional():
    ...
    def fn():
        ...
        # Can read clustershell log (rolling appender writes date-stamped files)
        import glob

        checks["var_log"] = len(glob.glob("/var/log/clustershell*.log*")) > 0
...
def test_baseline_enrichment_missing_filesystem_policy(
    ...
    with sandbox(spec=spec, delete_on_exit=True) as sb:
        ...
        assert checks["var_log"] is True, "ClusterShell log not accessible"
