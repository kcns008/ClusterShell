...
function run_install
    ...
    set -g INSTALL_OUTPUT (CLUSTERSHELL_INSTALL_DIR="$INSTALL_DIR" \
        SHELL="/usr/bin/fish" \
...
function test_binary_installed
    ...
    if test -f "$INSTALL_DIR/clustershell"
        pass "clustershell binary exists at $INSTALL_DIR/clustershell"
    else
        fail "clustershell binary exists" "not found at $INSTALL_DIR/clustershell"
    end
end
...
function test_binary_executable
    ...
    if test -x "$INSTALL_DIR/clustershell"
        pass "clustershell binary is executable"
    else
        fail "clustershell binary is executable" "$INSTALL_DIR/clustershell is not executable"
    end
end
...
function test_binary_runs
    ...
    set -l version_output ("$INSTALL_DIR/clustershell" --version 2>/dev/null)
    if test $status -eq 0
        pass "clustershell --version succeeds: $version_output"
    else
        fail "clustershell --version succeeds" "exit code: $status"
    end
end
...
printf 'Installing clustershell...\n'
run_install
printf 'Done.\n\n'
