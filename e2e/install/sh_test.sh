...
test_binary_installed() {
  ...
  if [ -f "$INSTALL_DIR/clustershell" ]; then
...
test_binary_executable() {
  ...
  if [ -x "$INSTALL_DIR/clustershell" ]; then
...
test_binary_runs() {
  ...
  if _version="$("$INSTALL_DIR/clustershell" --version 2>/dev/null)"; then
    pass "clustershell --version succeeds: $_version"
  else
    fail "clustershell --version succeeds" "exit code: $?"
  fi
}
