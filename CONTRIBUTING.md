...
ClusterShell is built agent-first. We design systems and use agents to implement them. Your agent is your first collaborator — point it at this repo before opening issues, asking questions, or submitting code.
...
ClusterShell is agent-first, not agent-only. The distinction matters:
...
1. Open a [Vouch Request](https://github.com/kcns008/ClusterShell/discussions/new?category=vouch-request) discussion.
...
Issues labeled [`good-first-issue`](https://github.com/kcns008/ClusterShell/issues?q=is%3Aissue+is%3Aopen+label%3Agood-first-issue) are scoped, well-documented, and friendly to new contributors. Start there. If you need guidance, comment on the issue.
...
2. Load the relevant skill - `debug-clustershell-cluster` for cluster problems, `debug-inference` for inference setup problems, `clustershell-cli` for usage questions, `generate-sandbox-policy` for policy help.
...
- An infrastructure problem that the `debug-clustershell-cluster` skill could not resolve.
...
- Configuration problems - your agent can diagnose these with `clustershell-cli`, `debug-clustershell-cluster`, and `debug-inference`.
...
| --------------- | ------------------------- | --------------------------------------------------------------------------------------------------- |
| Getting Started | `clustershell-cli`           | CLI usage, sandbox lifecycle, provider management, BYOC workflows                                   |
| Getting Started | `debug-clustershell-cluster` | Diagnose cluster startup failures and health issues                                                 |
...
## Building the `clustershell` CLI

Inside this repository, `clustershell` is a local shortcut script at `scripts/bin/clustershell`. The script will

1. Build `clustershell-cli` if needed.
2. Run the local debug CLI binary under `target/debug/clustershell`.

Because `mise` adds `scripts/bin` to `PATH` for this project, you can run `clustershell` directly from the repo.

```bash
clustershell --help
clustershell sandbox create -- codex
```
...
| Script    | What it does                                                                         |
| --------- | ------------------------------------------------------------------------------------ |
| `kubectl` | Runs `kubectl` inside the active gateway's k3s container via `clustershell doctor exec` |
| `k9s`     | Runs `k9s` inside the active gateway's k3s container via `clustershell doctor exec`     |

These work for both local and remote gateways (SSH is handled automatically). Examples:

```bash
kubectl get pods -A
kubectl logs -n clustershell statefulset/clustershell
k9s
k9s -n clustershell
```
...
```
feat(cli): add --verbose flag to clustershell run
