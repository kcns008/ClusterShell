...
///
/// Defined in `clustershell-core` so both `clustershell-router` (which applies it)
/// and `clustershell-server` / `clustershell-sandbox` (which resolve it from
...
///
/// This is separate from [`clustershell_providers::ProviderPlugin`] which handles
