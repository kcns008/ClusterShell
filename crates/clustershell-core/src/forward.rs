...
//!
//! Used by both the CLI (`clustershell-cli`) and the TUI (`clustershell-tui`) to
...
pub fn forward_pid_dir() -> Result<PathBuf> {
    Ok(xdg_config_dir()?.join("clustershell").join("forwards"))
}
...
/// hint:
///
/// - If an existing clustershell forward owns the port, suggest the stop command.
...
pub fn check_port_available(spec: &ForwardSpec) -> Result<()> {
    ...
    // Port is occupied.  Check if it belongs to a tracked clustershell forward.
        ...
        && let Some(fwd) = forwards.iter().find(|f| f.port == port && f.alive)
    {
        return Err(miette::miette!(
            "Port {port} is already forwarded to sandbox '{}'.\n\
             Stop it with: clustershell forward stop {port} {}",
            fwd.sandbox,
            fwd.sandbox,
        ));
    }
