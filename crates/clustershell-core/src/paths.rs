...
//!
//! All `ClusterShell` crates should use [`xdg_config_dir`] from this module instead
...
/// The top-level `ClusterShell` config directory: `$XDG_CONFIG_HOME/clustershell/`.
pub fn clustershell_config_dir() -> Result<PathBuf> {
    Ok(xdg_config_dir()?.join("clustershell"))
}
...
mod tests {
    ...
    fn xdg_config_dir_respects_env() {
        ...
        // we rely on the integration tests in clustershell-bootstrap for full
        // round-trip testing.
    ...
    #[test]
    fn clustershell_config_dir_appends_clustershell() {
        let dir = clustershell_config_dir().unwrap();
        assert!(
            dir.ends_with("clustershell"),
            "expected path ending with 'clustershell', got: {dir:?}"
        );
    }
