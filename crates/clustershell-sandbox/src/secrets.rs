...
const PLACEHOLDER_PREFIX: &str = "clustershell:resolve:env:";
...
mod tests {
    ...
    fn provider_env_is_replaced_with_placeholders() {
        ...
        assert_eq!(
            ...
            Some(&"clustershell:resolve:env:ANTHROPIC_API_KEY".to_string())
        );
        assert_eq!(
            resolver
                .as_ref()
                .and_then(|resolver| resolver
                    .resolve_placeholder("clustershell:resolve:env:ANTHROPIC_API_KEY")),
            Some("sk-test")
        );
    }
    ...
    fn rewrites_exact_placeholder_header_values() {
        ...
        assert_eq!(
            rewrite_header_line("x-api-key: clustershell:resolve:env:CUSTOM_TOKEN", &resolver),
    ...
    fn rewrites_http_header_blocks_and_preserves_body() {
        ...
        let raw = b"POST /v1 HTTP/1.1\r\nAuthorization: Bearer clustershell:resolve:env:CUSTOM_TOKEN\r\nContent-Length: 5\r\n\r\nhello";
    ...
    fn full_round_trip_child_env_to_rewritten_headers() {
        ...
        assert!(
            !rewritten.contains("clustershell:resolve:env:"),
