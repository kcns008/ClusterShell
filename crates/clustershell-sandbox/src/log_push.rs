...
//! Push sandbox tracing events to the ClusterShell server via gRPC.
//!
...
use clustershell_core::proto::{PushSandboxLogsRequest, SandboxLogLine};
...
/// Tracing layer that pushes log events to the ClusterShell server.
///
...
impl LogPushLayer {
    pub fn new(sandbox_id: String, tx: mpsc::Sender<SandboxLogLine>) -> Self {
        let max_level = std::env::var("CLUSTERSHELL_LOG_PUSH_LEVEL")
            .ok()
...
async fn run_push_loop(
    ...
    loop {
        ...
        // --- Connect ---
        let client = match CachedClusterShellClient::connect(&endpoint).await {
            Ok(c) => {
                if attempt > 1 {
                    eprintln!("clustershell: log push reconnected (attempt {attempt})");
                }
                backoff = INITIAL_BACKOFF;
                c
            }
            Err(e) => {
                eprintln!("clustershell: log push connect failed: {e}");
        ...
        tokio::spawn({
            ...
            async move {
                if let Err(e) = nav_client.push_sandbox_logs(stream).await {
                    eprintln!("clustershell: log push RPC failed: {e}");
                }
        ...
        if stream_broken {
            eprintln!("clustershell: log push stream lost, reconnecting...");
            backoff = INITIAL_BACKOFF;
