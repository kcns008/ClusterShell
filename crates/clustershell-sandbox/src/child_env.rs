...
mod tests {
    ...
    fn apply_tls_env_sets_node_and_bundle_paths() {
        ...
        let ca_cert_path = Path::new("/etc/clustershell-tls/clustershell-ca.pem");
        let combined_bundle_path = Path::new("/etc/clustershell-tls/ca-bundle.pem");
        ...
        assert!(stdout.contains("NODE_EXTRA_CA_CERTS=/etc/clustershell-tls/clustershell-ca.pem"));
        assert!(stdout.contains("SSL_CERT_FILE=/etc/clustershell-tls/ca-bundle.pem"));
    }
}
