...
pub struct InferenceContext {
    ...
    system_routes: Arc<tokio::sync::RwLock<Vec<clustershell_router::config::ResolvedRoute>>>,
}
...
impl InferenceContext {
    pub fn new(
        ...
        system_routes: Vec<clustershell_router::config::ResolvedRoute>,
    ) -> Self {
        Self {
            patterns,
            router,
    ...
    pub fn route_cache(
        &self,
    ) -> Arc<tokio::sync::RwLock<Vec<clustershell_router::config::ResolvedRoute>>> {
        self.routes.clone()
    }
    ...
    pub async fn system_inference(
        ...
        body: bytes::Bytes,
    ) -> Result<clustershell_router::ProxyResponse, clustershell_router::RouterError> {
...
/// inference API calls locally via `clustershell-router`.
        ...
        detect_inference_pattern(&request.method, &normalized_path, &ctx.patterns)
    {
        ...
        if routes.is_empty() {
            let body = serde_json::json!({
                ...
                "hint": "run: clustershell cluster inference set --help"
            });
...
fn router_error_to_http(err: &clustershell_router::RouterError) -> (u16, String) {
    use clustershell_router::RouterError;
    match err {
...
fn rewrite_forward_request(
    ...
    if !has_via {
        output.extend_from_slice(b"Via: 1.1 clustershell-sandbox\r\n");
    }
...
mod tests {
    ...
    fn router_error_route_not_found_maps_to_400() {
        let err = clustershell_router::RouterError::RouteNotFound("local".into());
    ...
    fn router_error_no_compatible_route_maps_to_400() {
        let err = clustershell_router::RouterError::NoCompatibleRoute("anthropic_messages".into());
        let (status, msg) = router_error_to_http(&err);
        assert_eq!(status, 400);
        assert!(
    ...
    fn router_error_unauthorized_maps_to_401() {
        let err = clustershell_router::RouterError::Unauthorized("bad token".into());
        let (status, msg) = router_error_to_http(&err);
    ...
    fn router_error_upstream_unavailable_maps_to_503() {
        let err = clustershell_router::RouterError::UpstreamUnavailable("connection refused".into());
        let (status, msg) = router_error_to_http(&err);
    ...
    fn router_error_upstream_protocol_maps_to_502() {
        let err = clustershell_router::RouterError::UpstreamProtocol("bad gateway".into());
        let (status, msg) = router_error_to_http(&err);
    ...
    fn router_error_internal_maps_to_502() {
        let err = clustershell_router::RouterError::Internal("unexpected".into());
        let (status, msg) = router_error_to_http(&err);
        assert_eq!(status, 502);
    ...
    fn test_rewrite_get_request() {
        ...
        assert!(result_str.contains("Via: 1.1 clustershell-sandbox"));
    }
    ...
    fn test_rewrite_resolves_placeholder_auth_headers() {
        ...
        let raw = b"GET http://host/p HTTP/1.1\r\nHost: host\r\nAuthorization: Bearer clustershell:resolve:env:ANTHROPIC_API_KEY\r\n\r\n";
        let result = rewrite_forward_request(raw, raw.len(), "/p", resolver.as_ref());
        let result_str = String::from_utf8_lossy(&result);
        assert!(result_str.contains("Authorization: Bearer sk-test"));
        assert!(!result_str.contains("clustershell:resolve:env:ANTHROPIC_API_KEY"));
    }
