...
const SSH_HANDSHAKE_SECRET_ENV: &str = "CLUSTERSHELL_SSH_HANDSHAKE_SECRET";
...
mod tests {
    ...
    async fn inject_provider_env_sets_placeholder_values() {
        ...
        let provider_env = std::iter::once((
            ...
            "clustershell:resolve:env:ANTHROPIC_API_KEY".to_string(),
        ))
        .collect();
        ...
        let output = cmd.output().await.expect("spawn env");
        let stdout = String::from_utf8(output.stdout).expect("utf8");
        assert!(stdout.contains("ANTHROPIC_API_KEY=clustershell:resolve:env:ANTHROPIC_API_KEY"));
    }
}
