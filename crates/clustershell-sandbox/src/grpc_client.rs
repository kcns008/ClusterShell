...
//! route bundles from ClusterShell server.
...
use clustershell_core::proto::{
    ...
    inference_client::InferenceClient, open_shell_client::ClusterShellClient,
};
use tonic::transport::{Certificate, Channel, ClientTlsConfig, Endpoint, Identity};
use tracing::debug;

/// Create a channel to the ClusterShell server.
///
/// When the endpoint uses `https://`, mTLS is configured using these env vars:
/// - `CLUSTERSHELL_TLS_CA` -- path to the CA certificate
/// - `CLUSTERSHELL_TLS_CERT` -- path to the client certificate
/// - `CLUSTERSHELL_TLS_KEY` -- path to the client private key
///
...
async fn connect_channel(endpoint: &str) -> Result<Channel> {
    ...
    if tls_enabled {
        let ca_path = std::env::var("CLUSTERSHELL_TLS_CA")
            .into_diagnostic()
            .wrap_err("CLUSTERSHELL_TLS_CA is required")?;
        let cert_path = std::env::var("CLUSTERSHELL_TLS_CERT")
            .into_diagnostic()
            .wrap_err("CLUSTERSHELL_TLS_CERT is required")?;
        let key_path = std::env::var("CLUSTERSHELL_TLS_KEY")
            .into_diagnostic()
            .wrap_err("CLUSTERSHELL_TLS_KEY is required")?;
    ...
    ep.connect()
        .await
        .into_diagnostic()
        .wrap_err("failed to connect to ClusterShell server")
}

/// Connect to the ClusterShell server (mTLS or plaintext based on endpoint scheme).
async fn connect(endpoint: &str) -> Result<ClusterShellClient<Channel>> {
    let channel = connect_channel(endpoint).await?;
    Ok(ClusterShellClient::new(channel))
}

/// Fetch sandbox policy from ClusterShell server via gRPC.
///
...
pub async fn fetch_policy(endpoint: &str, sandbox_id: &str) -> Result<Option<ProtoSandboxPolicy>> {
    debug!(endpoint = %endpoint, sandbox_id = %sandbox_id, "Connecting to ClusterShell server");
...
/// gateway stores the effective policy users see via `clustershell sandbox get`.
...
/// Fetch provider environment variables for a sandbox from ClusterShell server via gRPC.
///
...
/// A reusable gRPC client for the ClusterShell service.
///
...
/// Settings poll result returned by [`CachedClusterShellClient::poll_settings`].
pub struct SettingsPollResult {
    ...
    pub settings: std::collections::HashMap<String, clustershell_core::proto::EffectiveSetting>,
    /// When `policy_source` is `Global`, the version of the global policy revision.
    pub global_policy_version: u32,
}

impl CachedClusterShellClient {
    pub async fn connect(endpoint: &str) -> Result<Self> {
        debug!(endpoint = %endpoint, "Connecting clustershell gRPC client for policy polling");
        let channel = connect_channel(endpoint).await?;
    ...
    pub fn raw_client(&self) -> ClusterShellClient<Channel> {
        self.client.clone()
    }
    ...
    pub async fn submit_policy_analysis(
        ...
        proposed_chunks: Vec<clustershell_core::proto::PolicyChunk>,
        analysis_mode: &str,
    ) -> Result<()> {
        self.client
            .clone()
