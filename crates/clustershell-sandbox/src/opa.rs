...
impl OpaEngine {
    ...
    pub fn evaluate_network(&self, input: &NetworkInput) -> Result<PolicyDecision> {
        ...
        let allowed = engine
            .eval_rule("data.clustershell.sandbox.allow_network".into())
            .map_err(|e| miette::miette!("{e}"))?;
        let allowed = allowed == regorus::Value::from(true);

        let reason = engine
            .eval_rule("data.clustershell.sandbox.deny_reason".into())
            .map_err(|e| miette::miette!("{e}"))?;
        let reason = value_to_string(&reason);

        let matched = engine
            .eval_rule("data.clustershell.sandbox.matched_network_policy".into())
            .map_err(|e| miette::miette!("{e}"))?;
    ...
    pub fn evaluate_network_action(&self, input: &NetworkInput) -> Result<NetworkAction> {
        ...
        let action_val = engine
            .eval_rule("data.clustershell.sandbox.network_action".into())
            .map_err(|e| miette::miette!("{e}"))?;
    ...
    ///
    /// Designed for future gRPC hot-reload from the clustershell gateway.
    ...
    pub fn query_sandbox_config(&self) -> Result<SandboxConfig> {
        ...
        let fs_val = engine
            .eval_rule("data.clustershell.sandbox.filesystem_policy".into())
            .map_err(|e| miette::miette!("{e}"))?;
        let filesystem = parse_filesystem_policy(&fs_val);

        // Query landlock policy
        let ll_val = engine
            .eval_rule("data.clustershell.sandbox.landlock_policy".into())
            .map_err(|e| miette::miette!("{e}"))?;
        let landlock = parse_landlock_policy(&ll_val);

        // Query process policy
        let proc_val = engine
            .eval_rule("data.clustershell.sandbox.process_policy".into())
            .map_err(|e| miette::miette!("{e}"))?;
    ...
    pub fn query_endpoint_config(&self, input: &NetworkInput) -> Result<Option<regorus::Value>> {
        ...
        let val = engine
            .eval_rule("data.clustershell.sandbox.matched_endpoint_config".into())
            .map_err(|e| miette::miette!("{e}"))?;
...
mod tests {
    ...
    fn test_proto() -> ProtoSandboxPolicy {
        ...
        ProtoSandboxPolicy {
            ...
            landlock: Some(clustershell_core::proto::LandlockPolicy {
...
"#;
    ...
    fn eval_l7(engine: &OpaEngine, input: &serde_json::Value) -> bool {
        ...
        let val = eng
            .eval_rule("data.clustershell.sandbox.allow_request".into())
            .unwrap();
    ...
    fn l7_deny_reason_populated() {
        ...
        eng.set_input_json(&input.to_string()).unwrap();
        let val = eng
            .eval_rule("data.clustershell.sandbox.request_deny_reason".into())
            .unwrap();
