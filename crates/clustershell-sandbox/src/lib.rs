...
/// `CLUSTERSHELL_ROUTE_REFRESH_INTERVAL_SECS` environment variable.
...
fn route_refresh_interval_secs() -> u64 {
    match std::env::var("CLUSTERSHELL_ROUTE_REFRESH_INTERVAL_SECS") {
...
pub async fn run_sandbox(
    ...
    let clustershell_endpoint_for_proxy = clustershell_endpoint.clone();
    let sandbox_name_for_agg = sandbox.clone();
    let (policy, opa_engine) = load_policy(
        sandbox_id.clone(),
        sandbox,
        clustershell_endpoint.clone(),
        policy_rules,
        policy_data,
    )
    .await?;
    ...
    let provider_env = if let (Some(id), Some(endpoint)) = (&sandbox_id, &clustershell_endpoint) {
    ...
    let (tls_state, ca_file_paths) = if matches!(policy.network.mode, NetworkMode::Proxy) {
        match SandboxCa::generate() {
            Ok(ca) => {
                let tls_dir = std::path::Path::new("/etc/clustershell-tls");
                match write_ca_files(&ca, tls_dir) {
                    Ok(paths) => {
                        // /etc/clustershell-tls is subsumed by the /etc baseline
    ...
    let (_proxy, denial_rx, bypass_denial_tx) = if matches!(policy.network.mode, NetworkMode::Proxy)
    {
        ...
        let inference_ctx = build_inference_context(
            sandbox_id.as_deref(),
            clustershell_endpoint_for_proxy.as_deref(),
    ...
    // Zombie reaper — clustershell-sandbox may run as PID 1 in containers and
    ...
    if let Some(listen_addr) = ssh_listen_addr {
        ...
        let secret = ssh_handshake_secret
            ...
            .ok_or_else(|| {
                miette::miette!(
                    "CLUSTERSHELL_SSH_HANDSHAKE_SECRET is required when SSH is enabled.\n\
                     Set --ssh-handshake-secret or the CLUSTERSHELL_SSH_HANDSHAKE_SECRET env var."
                )
            })?;
    ...
    if let (Some(id), Some(endpoint), Some(engine)) =
        (&sandbox_id, &clustershell_endpoint, &opa_engine)
    {
        let poll_id = id.clone();
        let poll_endpoint = endpoint.clone();
        let poll_engine = engine.clone();
        let poll_interval_secs: u64 = std::env::var("CLUSTERSHELL_POLICY_POLL_INTERVAL_SECS")
            .ok()
        ...
        if let Some(rx) = denial_rx {
            ...
            let flush_interval_secs: u64 = std::env::var("CLUSTERSHELL_DENIAL_FLUSH_INTERVAL_SECS")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(10);
...
async fn build_inference_context(
    ...
    use clustershell_router::Router;
    use clustershell_router::config::RouterConfig;

    let source = infer_route_source(sandbox_id, clustershell_endpoint, inference_routes);
    ...
    let routes = match source {
        ...
        InferenceRouteSource::Cluster => {
            let (Some(_id), Some(endpoint)) = (sandbox_id, clustershell_endpoint) else {
                return Ok(None);
            };
    ...
    if matches!(source, InferenceRouteSource::Cluster)
        && let (Some(_id), Some(endpoint)) = (sandbox_id, clustershell_endpoint)
    {
        spawn_route_refresh(
            ctx.route_cache(),
...
fn partition_routes(
    routes: Vec<clustershell_router::config::ResolvedRoute>,
) -> (
    Vec<clustershell_router::config::ResolvedRoute>,
    Vec<clustershell_router::config::ResolvedRoute>,
) {
...
pub(crate) fn bundle_to_resolved_routes(
    bundle: &clustershell_core::proto::GetInferenceBundleResponse,
) -> Vec<clustershell_router::config::ResolvedRoute> {
    bundle
        .routes
        .iter()
        .map(|r| {
            let (auth, default_headers) =
                clustershell_core::inference::auth_for_provider_type(&r.provider_type);
            clustershell_router::config::ResolvedRoute {
                name: r.name.clone(),
...
pub(crate) fn spawn_route_refresh(
    user_cache: Arc<tokio::sync::RwLock<Vec<clustershell_router::config::ResolvedRoute>>>,
    system_cache: Arc<tokio::sync::RwLock<Vec<clustershell_router::config::ResolvedRoute>>>,
    endpoint: String,
    interval_secs: u64,
...
/// Python venv, and clustershell logs.
...
fn enrich_proto_baseline_paths(proto: &mut clustershell_core::proto::SandboxPolicy) -> bool {
    ...
    let fs = proto
        .filesystem
        .get_or_insert_with(|| clustershell_core::proto::FilesystemPolicy {
...
/// 2. If `sandbox_id` and `clustershell_endpoint` are provided, fetch via gRPC
...
async fn load_policy(
    ...
    if let (Some(id), Some(endpoint)) = (&sandbox_id, &clustershell_endpoint) {
        info!(
            sandbox_id = %id,
            endpoint = %endpoint,
                ...
                let sandbox = sandbox.as_deref().ok_or_else(|| {
                    miette::miette!(
                        "Cannot sync discovered policy: sandbox not available.\n\
                         Set CLUSTERSHELL_SANDBOX or --sandbox to enable policy sync."
                    )
                })?;
    ...
    Err(miette::miette!(
        "Sandbox policy required. Provide one of:\n\
         - --policy-rules and --policy-data (or CLUSTERSHELL_POLICY_RULES and CLUSTERSHELL_POLICY_DATA env vars)\n\
         - --sandbox-id and --clustershell-endpoint (or CLUSTERSHELL_SANDBOX_ID and CLUSTERSHELL_ENDPOINT env vars)"
    ))
}
...
fn discover_policy_from_disk_or_default() -> clustershell_core::proto::SandboxPolicy {
    let primary = std::path::Path::new(clustershell_policy::CONTAINER_POLICY_PATH);
    if primary.exists() {
        return discover_policy_from_path(primary);
    }
    let legacy = std::path::Path::new(clustershell_policy::LEGACY_CONTAINER_POLICY_PATH);
    if legacy.exists() {
        info!(
...
fn discover_policy_from_path(path: &std::path::Path) -> clustershell_core::proto::SandboxPolicy {
    use clustershell_policy::{
...
async fn flush_proposals_to_gateway(
    ...
    use crate::grpc_client::CachedClusterShellClient;
    use clustershell_core::proto::{DenialSummary, L7RequestSample};

    let client = CachedClusterShellClient::connect(endpoint).await?;
...
async fn run_policy_poll_loop(
    ...
    use crate::grpc_client::CachedClusterShellClient;
    use clustershell_core::proto::PolicySource;
    ...
    let mut current_settings: std::collections::HashMap<
        String,
        clustershell_core::proto::EffectiveSetting,
    > = std::collections::HashMap::new();
...
fn log_setting_changes(
    old: &std::collections::HashMap<String, clustershell_core::proto::EffectiveSetting>,
    new: &std::collections::HashMap<String, clustershell_core::proto::EffectiveSetting>,
) {
...
fn format_setting_value(es: &clustershell_core::proto::EffectiveSetting) -> String {
    use clustershell_core::proto::setting_value;
...
mod tests {
    ...
    fn bundle_to_resolved_routes_converts_all_fields() {
        let bundle = clustershell_core::proto::GetInferenceBundleResponse {
            routes: vec![
                clustershell_core::proto::ResolvedRoute {
        ...
        assert_eq!(
            routes[0].auth,
            clustershell_core::inference::AuthHeader::Bearer
        );
        assert_eq!(
            routes[0].protocols,
    ...
    fn bundle_to_resolved_routes_preserves_name_field() {
        let bundle = clustershell_core::proto::GetInferenceBundleResponse {
            routes: vec![clustershell_core::proto::ResolvedRoute {
    ...
    fn routes_segregated_by_name() {
        let routes = vec![
            clustershell_router::config::ResolvedRoute {
                ...
                protocols: vec!["openai_chat_completions".to_string()],
                auth: clustershell_core::inference::AuthHeader::Bearer,
                default_headers: vec![],
            },
            clustershell_router::config::ResolvedRoute {
                name: "sandbox-system".to_string(),
                ...
                auth: clustershell_core::inference::AuthHeader::Custom("x-api-key"),
                default_headers: vec![],
            },
        ];
...
"#,
    ...
    fn discover_policy_restrictive_default_blocks_network() {
        ...
        let proto = clustershell_policy::restrictive_default_policy();
    ...
    fn route_refresh_interval_uses_env_override() {
        ...
        with_vars(
            [("CLUSTERSHELL_ROUTE_REFRESH_INTERVAL_SECS", Some("9"))],
            || {
    ...
    fn route_refresh_interval_rejects_zero() {
        let _guard = ENV_LOCK.lock().unwrap();
        with_vars(
            [("CLUSTERSHELL_ROUTE_REFRESH_INTERVAL_SECS", Some("0"))],
            || {
                assert_eq!(
                    route_refresh_interval_secs(),
                    DEFAULT_ROUTE_REFRESH_INTERVAL_SECS
    ...
    fn route_refresh_interval_rejects_invalid_values() {
        let _guard = ENV_LOCK.lock().unwrap();
        with_vars(
            [("CLUSTERSHELL_ROUTE_REFRESH_INTERVAL_SECS", Some("abc"))],
            || {
                assert_eq!(
                    route_refresh_interval_secs(),
                    DEFAULT_ROUTE_REFRESH_INTERVAL_SECS
    ...
    async fn route_cache_preserves_content_when_not_written() {
        ...
        let routes = vec![clustershell_router::config::ResolvedRoute {
            name: "inference.local".to_string(),
