...
const DEFAULT_GATEWAY_NAME: &str = "clustershell";
...
///
/// Respects `$CLUSTERSHELL_GATEWAY` if set, otherwise falls back to the default.
...
pub async fn run_bootstrap(
    ...
    eprintln!();
    eprintln!("  The Gateway provides a secure control plane for ClusterShell. It streamlines");
    ...
    eprintln!(
        ...
        "clustershell status".bold(),
        "clustershell gateway stop".bold(),
    );
    eprintln!();
    ...
    let mut options = clustershell_bootstrap::DeployOptions::new(&gateway_name).with_recreate(true);
    if let Some(dest) = remote {
        let mut remote_opts = clustershell_bootstrap::RemoteOptions::new(dest);
    ...
    if let Ok(username) = std::env::var("CLUSTERSHELL_REGISTRY_USERNAME")
        && !username.trim().is_empty()
    {
        options = options.with_registry_username(username);
    }
    if let Ok(token) = std::env::var("CLUSTERSHELL_REGISTRY_TOKEN")
        && !token.trim().is_empty()
    {
    ...
    if let Ok(host) = std::env::var("CLUSTERSHELL_GATEWAY_HOST")
        && !host.trim().is_empty()
    {
    ...
    if let Err(err) = clustershell_bootstrap::save_active_gateway(&gateway_name) {
...
mod tests {
    ...
    fn missing_default_tls_files_triggers_bootstrap() {
        let err = report(
            "failed to read TLS CA from /home/user/.config/clustershell/clusters/clustershell/mtls/ca.crt: No such file or directory",
        );
        assert!(should_attempt_bootstrap(&err, &TlsOptions::default()));
    }
