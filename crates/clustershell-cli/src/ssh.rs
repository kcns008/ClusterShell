...
use clustershell_core::forward::{
    find_ssh_forward_pid, resolve_ssh_gateway, shell_escape, write_forward_pid,
};
use clustershell_core::proto::{CreateSshSessionRequest, GetSandboxRequest};
...
async fn ssh_session_config(
    ...
    let exe = std::env::current_exe()
        .into_diagnostic()
        .wrap_err("failed to resolve ClusterShell executable")?;
...
pub async fn sandbox_forward(
    server: &str,
    name: &str,
    spec: &clustershell_core::forward::ForwardSpec,
    background: bool,
    tls: &TlsOptions,
) -> Result<()> {
...
fn host_alias(name: &str) -> String {
    format!("clustershell-{name}")
}

fn render_ssh_config(gateway: &str, name: &str) -> String {
    let exe = std::env::current_exe().expect("failed to resolve ClusterShell executable");
...
fn clustershell_ssh_config_path() -> Result<PathBuf> {
    Ok(clustershell_core::paths::xdg_config_dir()?
        .join("clustershell")
        .join("ssh_config"))
}
...
fn ensure_clustershell_include(main_config: &Path, managed_config: &Path) -> Result<()> {
...
pub fn install_ssh_config(gateway: &str, name: &str) -> Result<PathBuf> {
    let managed_config = clustershell_ssh_config_path()?;
    let main_config = user_ssh_config_path()?;
    ensure_clustershell_include(&main_config, &managed_config)?;

    if let Some(parent) = managed_config.parent() {
        clustershell_core::paths::create_dir_restricted(parent)?;
    }
    ...
    fs::write(&managed_config, updated)
        .into_diagnostic()
        .wrap_err("failed to write ClusterShell SSH config")?;
    Ok(managed_config)
}
...
/// (`~/.config/clustershell/gateways/<name>/mtls/`).
...
mod tests {
    ...
    fn upsert_host_block_appends_when_missing() {
        ...
        let block = "Host clustershell-demo\n    User sandbox\n";
        let output = upsert_host_block(input, "clustershell-demo", block);
        assert!(output.contains("Host existing"));
        assert!(output.contains("Host clustershell-demo"));
        assert_eq!(output.matches("Host clustershell-demo").count(), 1);
    }

    #[test]
    fn upsert_host_block_replaces_existing_without_duplicates() {
        let input = "Host clustershell-demo\n    User old\n\nHost other\n    HostName other.example\n";
        let block = "Host clustershell-demo\n    User sandbox\n    LogLevel ERROR\n";
        let output = upsert_host_block(input, "clustershell-demo", block);
    ...
    fn install_ssh_config_adds_include_once_and_updates_managed_file() {
        ...
        let managed_path = install_ssh_config("clustershell", "demo").unwrap();
        install_ssh_config("clustershell", "demo").unwrap();
        ...
        assert_eq!(managed_contents.matches("Host clustershell-demo").count(), 1);
    ...
    fn launch_editor_returns_friendly_error_when_binary_missing() {
        let err = launch_editor_command(
            "clustershell-test-missing-binary",
            "Test Editor",
            "ssh-remote+clustershell-demo",
        )
        .unwrap_err();
        let text = format!("{err}");
        assert!(text.contains("clustershell-test-missing-binary is not installed or not on PATH"));
    }
    ...
    fn foreground_forward_started_message_custom_bind_addr() {
        let spec = clustershell_core::forward::ForwardSpec::parse("0.0.0.0:3000").unwrap();
        let message = foreground_forward_started_message("demo", &spec);
