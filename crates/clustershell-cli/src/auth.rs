...
pub async fn browser_auth_flow(gateway_endpoint: &str) -> Result<String> {
    ...
    let no_browser = std::env::var("CLUSTERSHELL_NO_BROWSER")
    ...
    if no_browser {
        eprintln!("Browser opening suppressed (CLUSTERSHELL_NO_BROWSER is set).");
        ...
        () = tokio::time::sleep(AUTH_TIMEOUT) => {
            return Err(miette::miette!(
                "authentication timed out after {} seconds.\n\
                 Try again with: clustershell gateway login",
