...
use clustershell_core::settings::{self, SettingValueKind};
use clustershell_providers::{
...
pub use clustershell_core::forward::{
...
fn ready_false_condition_message(
    status: Option<&clustershell_core::proto::SandboxStatus>,
) -> Option<String> {
    ...
    get_gateway_metadata(name).ok_or_else(|| {
        miette::miette!(
            "No gateway metadata found for '{name}'.\n\
              Deploy a gateway first with: clustershell gateway start --name {name}\n\
    ...
    if gateways.is_empty() || !interactive {
        ...
        if !gateways.is_empty() {
            ...
            eprintln!(
                ...
                "clustershell gateway select <name>".dimmed()
            );
        }
        return Ok(());
    }
...
pub async fn gateway_add(
    ...
    if get_gateway_metadata(name).is_some() {
        return Err(miette::miette!(
            "Gateway '{}' already exists.\n\
             Remove it first with: clustershell gateway destroy --name {}\n\
    ...
    if remote.is_some() || local {
        ...
        clustershell_bootstrap::extract_and_store_pki(name, remote_opts.as_ref(), endpoint_port)
            .await?;
        ...
        match crate::auth::browser_auth_flow(&endpoint).await {
            Ok(token) => {
                clustershell_bootstrap::edge_token::store_edge_token(name, &token)?;
            ...
            Err(e) => {
                ...
                eprintln!(
                    ...
                    "clustershell gateway login".dimmed(),
                );
            }
        }
    }
...
pub async fn gateway_login(name: &str) -> Result<()> {
    let metadata = clustershell_bootstrap::load_gateway_metadata(name).map_err(|_| {
        miette::miette!(
            "Unknown gateway '{name}'.\n\
             List available gateways: clustershell gateway select"
        )
    })?;
...
pub(crate) async fn deploy_gateway_with_panel(
    ...
    name: &str,
    location: &str,
) -> Result<clustershell_bootstrap::GatewayHandle> {
    ...
    if interactive {
        ...
        let result = clustershell_bootstrap::deploy_gateway_with_logs(options, move |line| {
        ...
        match result {
            ...
            Err(err) => {
                ...
                let container_logs = clustershell_bootstrap::fetch_gateway_logs(name, 80).await;
                ...
                let diagnosis =
                    clustershell_bootstrap::errors::diagnose_failure(name, &err_str, logs_opt)
                        .unwrap_or_else(|| {
                            clustershell_bootstrap::errors::generic_failure_diagnosis(name)
                        });
        ...
        let result = clustershell_bootstrap::deploy_gateway_with_logs(options, |line| {
            if let Some(status) = line.strip_prefix("[status] ") {
                eprintln!("  {status}");
...
pub(crate) fn print_deploy_summary(name: &str, handle: &clustershell_bootstrap::GatewayHandle) {
    eprintln!();
...
fn print_failure_diagnosis(diagnosis: &clustershell_bootstrap::errors::GatewayFailureDiagnosis) {
    eprintln!();
...
pub async fn gateway_admin_deploy(
    ...
    if openshift {
        eprintln!();
        eprintln!(
            "{} Deploying ClusterShell to OpenShift cluster...",
            "→".cyan().bold()
        );
        eprintln!();
        eprintln!(" Using namespace: {}", namespace.bold());
        if let Some(kc) = kubeconfig {
            eprintln!(" Using kubeconfig: {}", kc.dimmed());
        }
        if let Some(ctx) = context {
            eprintln!(" Using context: {}", ctx.bold());
        }
        eprintln!();

        let mut openshift_opts = clustershell_bootstrap::openshift::OpenShiftOptions::new()
            .with_namespace(namespace)
            .with_disable_tls(disable_tls)
            .with_disable_gateway_auth(disable_gateway_auth);

        if let Some(kc) = kubeconfig {
            openshift_opts = openshift_opts.with_kubeconfig(kc);
        }

        if let Some(ctx) = context {
            openshift_opts = openshift_opts.with_context(ctx);
        }
        ...
        match clustershell_bootstrap::openshift::deploy_to_openshift(&openshift_opts, &image).await {
            Ok(endpoint) => {
                eprintln!(
                    "{} ClusterShell deployed successfully to OpenShift!",
                    "✓".green().bold()
                );
                eprintln!();
                eprintln!(" Gateway endpoint: {}", endpoint.bold());
                eprintln!();
                eprintln!(" To get started:");
                eprintln!("   clustershell sandbox create -- <agent>");
                eprintln!();
            }
            Err(e) => {
                return Err(miette::miette!(
                    "Failed to deploy to OpenShift: {}\n\n\
                    Make sure you have 'oc' or 'kubectl' configured and can connect to your OpenShift cluster.",
                    e
                ));
            }
        }
    ...
    if let Some(existing) =
        clustershell_bootstrap::check_existing_deployment(name, remote_opts.as_ref()).await?
    {
        if !should_recreate {
    ...
    match resolve_gateway_control_target(name, remote_override) {
        ...
        GatewayControlTarget::ExternalRegistration => Err(miette::miette!(
            "Gateway '{name}' is an external registration, not a managed Docker gateway.\n\
             `clustershell gateway stop` is only supported for local or SSH-managed gateways."
        )),
    }
}

fn remove_gateway_registration(name: &str) {
    if let Err(err) = clustershell_bootstrap::edge_token::remove_edge_token(name) {
...
pub async fn gateway_admin_stop(
    ...
    let handle = clustershell_bootstrap::gateway_handle(name, remote_opts.as_ref()).await?;
    handle.stop().await?;
...
/// logs from the `clustershell-cluster-{name}` container.
pub async fn doctor_logs(
    ...
    clustershell_bootstrap::gateway_container_logs(remote_opts.as_ref(), name, lines, tail, stdout)
        .await
}
...
pub async fn doctor_check() -> Result<()> {
    ...
    match clustershell_bootstrap::check_docker_available().await {
        Ok(preflight) => {
...
pub async fn sandbox_create_with_bootstrap(
    ...
    providers: &[String],
    policy: Option<&str>,
    forward: Option<clustershell_core::forward::ForwardSpec>,
    command: &[String],
...
pub async fn sandbox_create(
    ...
    if editor.is_some() && !command.is_empty() {
        return Err(miette::miette!(
            "--editor cannot be used with a trailing command; use `clustershell sandbox connect <name> --editor ...` after the sandbox is ready"
        ));
    }
    ...
    if let Some(ref spec) = forward {
        clustershell_core::forward::check_port_available(spec)?;
    }
    ...
    let (mut client, effective_server, effective_tls) = match grpc_client(server, tls).await {
        ...
        Err(err) => {
            ...
            if !crate::bootstrap::confirm_bootstrap(bootstrap_override)? {
                ...
                eprintln!(
                    "    {} && {}",
                    format!("clustershell gateway destroy --name {gateway_name}").cyan(),
                    "clustershell gateway start".cyan(),
                );
                eprintln!();
                return Err(err);
            }
    ...
    loop {
        ...
        match evt.payload {
            Some(clustershell_core::proto::sandbox_stream_event::Payload::Sandbox(s)) => {
            ...
            Some(clustershell_core::proto::sandbox_stream_event::Payload::Log(line)) => {
                // Detect gateway readiness from log messages.
                if !saw_gateway_ready && line.message.contains("listening") {
                    saw_gateway_ready = true;
                }
            }
            Some(clustershell_core::proto::sandbox_stream_event::Payload::Event(ev)) => {
            ...
            Some(clustershell_core::proto::sandbox_stream_event::Payload::Warning(w)) => {
                if let Some(d) = display.as_mut() {
            ...
            Some(clustershell_core::proto::sandbox_stream_event::Payload::DraftPolicyUpdate(_)) => {
    ...
    match final_phase {
        SandboxPhase::Ready => {
            ...
            if let Some(ref spec) = forward {
                ...
                eprintln!(
                    "  Stop with: clustershell forward stop {} {sandbox_name}",
                    spec.port,
                );
            }
...
fn resolve_from(value: &str) -> Result<ResolvedSource> {
    ...
    let prefix = std::env::var("CLUSTERSHELL_COMMUNITY_REGISTRY")
...
async fn build_from_dockerfile(
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_secs();
    let tag = format!("clustershell/sandbox-from:{timestamp}");
    ...
    clustershell_bootstrap::build::build_and_push_image(
        dockerfile,
        &tag,
        context,
        gateway_name,
...
///
/// Resolution order: `--policy` flag > `CLUSTERSHELL_SANDBOX_POLICY` env var.
...
fn load_sandbox_policy(cli_path: Option<&str>) -> Result<Option<SandboxPolicy>> {
    clustershell_policy::load_sandbox_policy(cli_path)
}
...
fn print_sandbox_policy(policy: &SandboxPolicy) {
    ...
    println!();
    if let Ok(yaml_str) = clustershell_policy::serialize_sandbox_policy(policy) {
        ...
        if known_names.contains(name) {
            ...
            return Err(miette::miette!(
                "provider '{name}' not found and '{name}' is not a recognized provider type. \
                 Create it first with `clustershell provider create --type <type> --name {name}`"
            ));
        }
    }
    ...
    if auto_providers_override.is_none() && !std::io::stdin().is_terminal() {
        return Err(miette::miette!(
            "missing required provider '{provider_type}'. Create it first with \
             `clustershell provider create --type {provider_type} --name {provider_type} --from-existing`, \
...
pub async fn sandbox_settings_get(
    ...
    let policy_source =
        if response.policy_source == clustershell_core::proto::PolicySource::Global as i32 {
            "global"
        } else {
            "sandbox"
        };
...
fn settings_to_json_sandbox(
    name: &str,
    response: &clustershell_core::proto::GetSandboxConfigResponse,
) -> serde_json::Value {
    let policy_source =
        if response.policy_source == clustershell_core::proto::PolicySource::Global as i32 {
            "global"
...
fn settings_to_json_global(
    response: &clustershell_core::proto::GetGatewayConfigResponse,
) -> serde_json::Value {
    let mut settings = serde_json::Map::new();
    let mut keys: Vec<_> = response.settings.keys().cloned().collect();
    keys.sort();
    ...
    if let Some(rev) = inner.revision {
        ...
        if full {
            if let Some(ref policy) = rev.policy {
                println!("---");
                let yaml_str = clustershell_policy::serialize_sandbox_policy(policy)
...
fn print_policy_revision_table(revisions: &[clustershell_core::proto::SandboxPolicyRevision]) {
    println!(
        "{:<8} {:<14} {:<12} {:<24} ERROR",
...
pub async fn sandbox_logs(
    ...
    if tail {
        ...
        while let Some(event) = stream.next().await {
            ...
            if let Some(clustershell_core::proto::sandbox_stream_event::Payload::Log(log)) =
                event.payload
            {
                print_log_line(&log);
            }
...
fn print_log_line(log: &clustershell_core::proto::SandboxLogLine) {
...
fn format_endpoints(rule: &clustershell_core::proto::NetworkPolicyRule) -> String {
    rule.endpoints
        .iter()
        .map(|e| {
            if e.port > 0 {
...
mod tests {
    ...
    use clustershell_bootstrap::{load_active_gateway, store_gateway_metadata};
    use std::fs;
    ...
    use clustershell_bootstrap::GatewayMetadata;
    use clustershell_core::proto::{SandboxCondition, SandboxStatus};
    ...
    fn parse_cli_setting_value_parses_bool_aliases() {
        ...
        assert_eq!(
            yes_value.value,
            Some(clustershell_core::proto::setting_value::Value::BoolValue(true))
        );

        let zero_value = parse_cli_setting_value("dummy_bool", "0").expect("parse 0");
        assert_eq!(
            zero_value.value,
            Some(clustershell_core::proto::setting_value::Value::BoolValue(
                false
            ))
        );
    }
    ...
    fn parse_cli_setting_value_parses_int_key() {
        ...
        assert_eq!(
            int_value.value,
            Some(clustershell_core::proto::setting_value::Value::IntValue(42))
        );
    }
    ...
    fn sandbox_should_persist_when_forward_is_requested() {
        let spec = clustershell_core::forward::ForwardSpec::new(8080);
