...
use clustershell_bootstrap::{list_gateways, load_active_gateway, load_gateway_metadata};
use clustershell_core::proto::open_shell_client::ClusterShellClient;
use clustershell_core::proto::{ListProvidersRequest, ListSandboxesRequest};
...
fn resolve_active_gateway() -> Option<(String, String)> {
    let name = std::env::var("CLUSTERSHELL_GATEWAY")
        .ok()
...
async fn completion_grpc_client(
    server: &str,
    gateway_name: &str,
) -> Option<ClusterShellClient<Channel>> {
    ...
    Some(ClusterShellClient::new(channel))
}
...
mod tests {
    ...
    use clustershell_bootstrap::{GatewayMetadata, store_gateway_metadata};
