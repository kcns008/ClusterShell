...
//! `ClusterShell` CLI - command-line interface for `ClusterShell`.
...
use clustershell_cli::completers;
use clustershell_cli::run;
use clustershell_cli::tls::TlsOptions;
...
/// 3. `CLUSTERSHELL_GATEWAY` environment variable
/// 4. Active gateway from `~/.config/clustershell/active_gateway`
///
...
fn resolve_gateway(
    ...
    let name = gateway_flag
        .clone()
        .or_else(|| {
            std::env::var("CLUSTERSHELL_GATEWAY")
                .ok()
        ...
        .ok_or_else(|| {
            miette::miette!(
                "No active gateway.\n\
                 Set one with: clustershell gateway select <name>\n\
                 Or deploy a new gateway: clustershell gateway start"
            )
        })?;
    ...
    let metadata = load_gateway_metadata(&name).map_err(|_| {
        miette::miette!(
            "Unknown gateway '{name}'.\n\
             Deploy it first: clustershell gateway start --name {name}\n\
             Or list available gateways: clustershell gateway select"
        )
    })?;
...
{about-with-newline}
\x1b[1mUSAGE\x1b[0m
  clustershell <command> <subcommand> [flags]
...
\x1b[1mADDITIONAL COMMANDS\x1b[0m
  term:        Launch the ClusterShell interactive TUI
...
\x1b[1mEXAMPLES\x1b[0m
  ...
  $ clustershell gateway start
  $ clustershell logs my-sandbox

\x1b[1mLEARN MORE\x1b[0m
  Use `clustershell <command> --help` for more information about a command.
";
...
\x1b[1mEXAMPLES\x1b[0m
  $ clustershell sandbox create
  $ clustershell sandbox create --from python
...
\x1b[1mEXAMPLES\x1b[0m
  $ clustershell forward start 8080
  $ clustershell forward start 3000 my-sandbox
  $ clustershell forward stop 8080
...
\x1b[1mEXAMPLES\x1b[0m
  $ clustershell logs my-sandbox
  $ clustershell logs my-sandbox --tail
  $ clustershell logs --since 5m
  $ clustershell logs --source sandbox --level debug
";
...
\x1b[1mEXAMPLES\x1b[0m
  $ clustershell policy get my-sandbox
  $ clustershell policy set my-sandbox --policy policy.yaml
  $ clustershell policy set --global --policy policy.yaml
  $ clustershell policy delete --global
  $ clustershell policy list my-sandbox
";

const SETTINGS_EXAMPLES: &str = "\x1b[1mEXAMPLES\x1b[0m
  $ clustershell settings get my-sandbox
  $ clustershell settings get --global
  $ clustershell settings set my-sandbox --key log_level --value debug
  $ clustershell settings set --global --key log_level --value warn
  $ clustershell settings set --global --key dummy_bool --value yes
  $ clustershell settings set --global --key dummy_int --value 42
  $ clustershell settings delete --global --key log_level
";

const PROVIDER_EXAMPLES: &str = "\x1b[1mEXAMPLES\x1b[0m
  $ clustershell provider create --name openai --type openai --credential OPENAI_API_KEY
  $ clustershell provider create --name anthropic --type anthropic --from-existing
  $ clustershell provider list
  $ clustershell provider get openai
  $ clustershell provider delete openai
";
...
\x1b[1mEXAMPLES\x1b[0m
  $ clustershell gateway start
  $ clustershell gateway start --name my-gateway --port 9090
  $ clustershell gateway stop
  $ clustershell gateway select my-gateway
  $ clustershell gateway info
";

const INFERENCE_EXAMPLES: &str = "\x1b[1mEXAMPLES\x1b[0m
  $ clustershell inference set --provider openai --model gpt-4
  $ clustershell inference get
  $ clustershell inference update --model gpt-4-turbo
";
...
\x1b[1mAI AGENT USAGE\x1b[0m
  ...
  for debugging gateway clusters using `clustershell doctor logs` and
  `clustershell doctor exec`.
";

/// `ClusterShell` CLI - agent execution and management.
#[derive(Parser, Debug)]
#[command(name = "clustershell")]
#[command(author, version = clustershell_core::VERSION, about, long_about = None)]
...
struct Cli {
    ...
    #[arg(
        long,
        global = true,
        env = "CLUSTERSHELL_GATEWAY_ENDPOINT",
        help_heading = "GATEWAY FLAGS"
    )]
    gateway_endpoint: Option<String>,
...
enum Commands {
    ...
    /// `clustershell doctor llm.txt` for a full diagnostic prompt.
    ...
    // ADDITIONAL COMMANDS
    // ===================================================================
    /// Launch the `ClusterShell` interactive TUI.
    #[command(help_template = LEAF_HELP_TEMPLATE, next_help_heading = "FLAGS")]
    Term {
        /// Color theme for the TUI: auto, dark, or light.
        #[arg(long, default_value = "auto", env = "CLUSTERSHELL_THEME")]
        theme: clustershell_tui::ThemeMode,
    },
    ...
    ///   `clustershell ssh-proxy --gateway <url> --sandbox-id <id> --token <token>`
    ///
    /// **Name mode** (for use in `~/.ssh/config`):
    ///   `clustershell ssh-proxy --gateway <name> --name <sandbox-name>`
...
Generate shell completion scripts for ClusterShell CLI.
...
First, ensure that you install `bash-completion` using your package manager.
  ...
  clustershell completions bash > ~/.local/share/bash-completion/completions/clustershell

On macOS with Homebrew (install bash-completion first):

  mkdir -p $(brew --prefix)/etc/bash_completion.d
  clustershell completions bash > $(brew --prefix)/etc/bash_completion.d/clustershell.bash-completion

\x1b[1mFISH\x1b[0m

  mkdir -p ~/.config/fish/completions
  clustershell completions fish > ~/.config/fish/completions/clustershell.fish

\x1b[1mZSH\x1b[0m

  mkdir -p ~/.zfunc
  clustershell completions zsh > ~/.zfunc/_clustershell
...
\x1b[1mPOWERSHELL\x1b[0m
   ...
   clustershell completions powershell >> $PROFILE
...
fn normalize_completion_script(output: Vec<u8>, executable: &std::path::Path) -> Result<String> {
    ...
    Ok(script.replace(executable.to_string_lossy().as_ref(), "clustershell"))
}
...
impl From<CliEditor> for clustershell_cli::ssh::Editor {
...
enum GatewayCommands {
    ...
    Start {
        /// Gateway name.
        #[arg(long, default_value = "clustershell", env = "CLUSTERSHELL_GATEWAY")]
        name: String,
        ...
        #[arg(long, default_value_t = clustershell_bootstrap::DEFAULT_GATEWAY_PORT)]
        port: u16,
        ...
        #[arg(long, env = "CLUSTERSHELL_REGISTRY_USERNAME")]
        ...
        #[arg(long, env = "CLUSTERSHELL_REGISTRY_TOKEN")]
    ...
    /// Deploy to an existing OpenShift cluster instead of creating a new K3s cluster.
    ///
    /// When set, ClusterShell deploys its components into the OpenShift cluster
    /// specified by the current kubeconfig context instead of creating a new
    /// K3s cluster inside a Docker container. Requires OpenShift CLI (`oc`) to be
    /// installed and configured.
    #[arg(long)]
    openshift: bool,

    /// OpenShift namespace to deploy into (default: clustershell).
    #[arg(long, default_value = "clustershell")]
    namespace: String,

    /// Path to kubeconfig file for OpenShift deployment.
    #[arg(long, value_hint = ValueHint::FilePath)]
    kubeconfig: Option<String>,

    /// OpenShift context to use (default: current context).
    #[arg(long)]
    ...
    Stop {
        ...
        #[arg(long, env = "CLUSTERSHELL_GATEWAY", add = ArgValueCompleter::new(completers::complete_gateway_names))]
        name: Option<String>,
    ...
    ///
    /// Registers a gateway endpoint so it appears in `clustershell gateway select`.
    ///
...
enum DoctorCommands {
    ...
    Logs {
        /// Gateway name (defaults to active gateway).
        #[arg(long, env = "CLUSTERSHELL_GATEWAY")]
        name: Option<String>,
    ...
    /// Examples:
    ///   clustershell doctor exec -- kubectl get pods -A
    ///   clustershell doctor exec -- k9s
    ///   clustershell doctor exec -- sh
    #[command(help_template = LEAF_HELP_TEMPLATE, next_help_heading = "FLAGS")]
    Exec {
        /// Gateway name (defaults to active gateway).
        #[arg(long, env = "CLUSTERSHELL_GATEWAY")]
    ...
    /// diagnose gateway issues using `clustershell doctor logs` and
    /// `clustershell doctor exec`.
    ///
    /// Examples:
    ///   clustershell doctor llm.txt
    ///   clustershell doctor llm.txt | pbcopy
    ...
    /// your environment before running `clustershell gateway start`.
    ///
    /// Examples:
    ///   clustershell doctor check
...
enum SandboxCommands {
    ...
    Create {
        ...
        /// Keeps the sandbox alive and installs ClusterShell-managed SSH config.
        ...
        /// Overrides the built-in default and the `CLUSTERSHELL_SANDBOX_POLICY` env var.
        #[arg(long, value_hint = ValueHint::FilePath)]
    ...
    Connect {
        ...
        /// Installs ClusterShell-managed SSH config if needed.
        #[arg(long, value_enum)]
        editor: Option<CliEditor>,
    },
    ...
    match cli.command {
        ...
        GatewayCommands::Start {
            ...
            GatewayCommands::Stop {
                ...
                let name = name
                    ...
                    .unwrap_or_else(|| "clustershell".to_string());
            ...
            GatewayCommands::Login { name } => {
                let name = name
                    ...
                    .ok_or_else(|| {
                        miette::miette!(
                            "No active gateway.\n\
                             Specify a gateway name: clustershell gateway login <name>\n\
                             Or set one with: clustershell gateway select <name>"
                        )
                    })?;
        ...
        Some(Commands::Status) => {
            if let Ok(ctx) = resolve_gateway(&cli.gateway, &cli.gateway_endpoint) {
                ...
                println!(
                    ...
                    "clustershell gateway start".dimmed()
                );
            }
        }
        ...
        Some(Commands::Forward {
            ...
            ForwardCommands::Start {
                ...
                let spec = clustershell_core::forward::ForwardSpec::parse(&port)?;
                ...
                if background {
                    ...
                    eprintln!("  Stop with: clustershell forward stop {} {name}", spec.port);
                }
            }
        },
        ...
        Some(Commands::Sandbox {
            ...
            match command {
                SandboxCommands::Create {
                    ...
                    let forward = forward
                        .map(|s| clustershell_core::forward::ForwardSpec::parse(&s))
                        .transpose()?;
                    ...
                    match resolve_gateway(&cli.gateway, &cli.gateway_endpoint) {
                        Ok(ctx) => {
                            if remote.is_some() {
                                eprintln!(
                                    "{} --remote ignored: gateway '{}' is already active. \
                                     To redeploy, use: clustershell gateway start",
                                    "!".yellow(),
                                    ctx.name,
                                );
                                return Ok(());
                            ...
                            // "clustershell" gateway if the connection fails.
        ...
        Some(Commands::Term { theme }) => {
            ...
            let mut tls = tls.with_gateway_name(&ctx.name);
            apply_edge_auth(&mut tls, &ctx.name);
            let channel = clustershell_cli::tls::build_channel(&ctx.endpoint, &tls).await?;
            clustershell_tui::run(channel, &ctx.name, &ctx.endpoint, theme).await?;
        }
                ...
                (_, _, _, server_override, Some(g), Some(n)) => {
                    let endpoint = if let Some(srv) = server_override {
                        ...
                        let meta = load_gateway_metadata(&g).map_err(|_| {
                            miette::miette!(
                                "Unknown gateway '{g}'.\n\
                                  Deploy it first: clustershell gateway start --name {g}\n\
                                  Or list available gateways: clustershell gateway select"
                            )
                        })?;
...
mod tests {
    ...
    fn completions_engine_returns_candidates() {
        ...
        let args: Vec<OsString> = vec!["clustershell".into(), "".into()];
    ...
    fn completions_subcommand_appears_in_candidates() {
        let mut cmd = Cli::command();
        let args: Vec<OsString> = vec!["clustershell".into(), "comp".into()];
        let candidates = clap_complete::engine::complete(&mut cmd, args, 1, None)
            .expect("completion engine failed");
        let names: Vec<String> = candidates
            .iter()
    ...
    fn completions_other_path_flags_fall_back_to_path_candidates() {
        ...
        let cases: Vec<(Vec<&str>, usize, &str)> = vec![
            (
                vec!["clustershell", "gateway", "start", "--ssh-key", "id"],
                4,
                "id_rsa",
            ),
            (
                vec!["clustershell", "sandbox", "create", "--ssh-key", "id"],
                4,
                "id_rsa",
            ),
            (
                vec!["clustershell", "sandbox", "upload", "demo", "Do"],
                4,
                "Dockerfile",
            ),
            (
                vec!["clustershell", "sandbox", "create", "--from", "Do"],
                4,
                "Dockerfile",
            ),
            (
    ...
    fn gateway_completion_suggests_registered_gateways() {
        ...
        with_tmp_xdg(tmp.path(), || {
            ...
            for (raw_args, index) in [
                (vec!["clustershell", "--gateway", "a"], 2),
                (vec!["clustershell", "gateway", "select", "a"], 3),
                (vec!["clustershell", "gateway", "info", "--name", "a"], 4),
            ] {
                let mut cmd = Cli::command();
                let args: Vec<OsString> = raw_args.iter().copied().map(Into::into).collect();
    ...
    fn global_gateway_flag_still_parses_with_subcommands() {
        let cli = Cli::try_parse_from(["clustershell", "--gateway", "demo", "status"])
    ...
    fn hidden_aliases_still_parse() {
        let cli = Cli::try_parse_from(["clustershell", "lg", "sandbox-1"])
    ...
    #[test]
    fn completion_script_uses_clustershell_command_name() {
        let script = normalize_completion_script(
            b"/tmp/custom/clustershell -- \"${words[@]}\"\n#compdef clustershell\n".to_vec(),
            std::path::Path::new("/tmp/custom/clustershell"),
        )
        .expect("normalize completion script");

        assert!(script.contains("clustershell -- \"${words[@]}\""));
        assert!(!script.contains("/tmp/custom/clustershell"));
    }
    ...
    fn settings_get_global_parses() {
        let cli = Cli::try_parse_from(["clustershell", "settings", "get", "--global"])
    ...
    fn policy_delete_global_parses() {
        let cli = Cli::try_parse_from(["clustershell", "policy", "delete", "--global", "--yes"])
