...
use clustershell_core::proto::inference_client::InferenceClient;
...
pub type GrpcClient = ClusterShellClient<InterceptedService<Channel, EdgeAuthInterceptor>>;
...
fn tls_dir_for_gateway(name: &str) -> Option<PathBuf> {
    ...
    let base = xdg_config_dir().ok()?.join("clustershell").join("gateways");
...
fn default_tls_dir(server: &str) -> Option<PathBuf> {
    let mut name = std::env::var("CLUSTERSHELL_GATEWAY")
        .ok()
    ...
    let name = name.unwrap_or_else(|| "clustershell".to_string());
    let safe_name = sanitize_name(&name);
    let base = xdg_config_dir().ok()?.join("clustershell").join("gateways");
    Some(base.join(safe_name).join("mtls"))
}
...
fn xdg_config_dir() -> Result<PathBuf> {
    clustershell_core::paths::xdg_config_dir()
}
...
/// Build a gRPC [`ClusterShellClient`].
///
...
pub async fn grpc_client(server: &str, tls: &TlsOptions) -> Result<GrpcClient> {
    ...
    Ok(ClusterShellClient::with_interceptor(channel, interceptor))
}
