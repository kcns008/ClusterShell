...
use clustershell_bootstrap::{load_last_sandbox, save_last_sandbox};
use clustershell_cli::run;
use clustershell_cli::tls::TlsOptions;
use clustershell_core::proto::open_shell_server::{ClusterShell, ClusterShellServer};
...
impl ClusterShell for TestClusterShell {
    ...
    async fn revoke_ssh_session(
        &self,
        _request: tonic::Request<clustershell_core::proto::RevokeSshSessionRequest>,
    ) -> Result<Response<clustershell_core::proto::RevokeSshSessionResponse>, Status> {
        Ok(Response::new(
...
async fn run_server() -> TestServer {
    ...
    let clustershell = TestClusterShell::default();
    let svc_clustershell = clustershell.clone();

    tokio::spawn(async move {
        Server::builder()
            .tls_config(tls_config)
            .unwrap()
            .add_service(ClusterShellServer::new(svc_clustershell))
...
async fn sandbox_get_sends_correct_name() {
    ...
    let recorded = ts.clustershell.state.last_get_name.lock().await.clone();
    assert_eq!(
        recorded.as_deref(),
        Some("my-sandbox"),
