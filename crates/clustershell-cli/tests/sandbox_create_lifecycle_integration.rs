...
use clustershell_bootstrap::load_last_sandbox;
...
async fn deleted_names(server: &TestServer) -> Vec<Vec<String>> {
    server.clustershell.state.deleted_names.lock().await.clone()
}

fn test_tls(server: &TestServer) -> TlsOptions {
    server.tls.with_gateway_name("clustershell")
}
...
async fn sandbox_create_keeps_command_sessions_by_default() {
    ...
    assert_eq!(
        load_last_sandbox("clustershell").as_deref(),
        Some("default-command"),
...
async fn sandbox_create_deletes_command_sessions_with_no_keep() {
    ...
    assert_eq!(
        load_last_sandbox("clustershell"),
        None,
...
async fn sandbox_create_keeps_sandbox_with_forwarding() {
    ...
    run::sandbox_create(
        ...
        &[],
        None,
        Some(clustershell_core::forward::ForwardSpec::new(8080)),
        &["echo".to_string(), "OK".to_string()],
        Some(false),
        Some(false),
        Some(false),
