...
async fn explicit_provider_name_passes_through_when_it_exists() {
    ...
    ts.clustershell.seed_provider("nvidia", "nvidia").await;

    let mut client = clustershell_cli::tls::grpc_client(&ts.endpoint, &ts.tls)
        .await
        .expect("grpc client");
    ...
    let providers = ts.clustershell.state.providers.lock().await;
