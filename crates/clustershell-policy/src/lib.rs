...
//! Shared sandbox policy parsing and defaults for ClusterShell.
//!
...
/// 2. `CLUSTERSHELL_SANDBOX_POLICY` environment variable
///
...
pub fn load_sandbox_policy(cli_path: Option<&str>) -> Result<Option<SandboxPolicy>> {
    ...
    } else if let Ok(policy_path) = std::env::var("CLUSTERSHELL_SANDBOX_POLICY") {
...
pub const CONTAINER_POLICY_PATH: &str = "/etc/clustershell/policy.yaml";

/// Legacy path used before the navigator → clustershell rename.
///
...
"#;
    ...
    fn container_policy_path_is_expected() {
        assert_eq!(CONTAINER_POLICY_PATH, "/etc/clustershell/policy.yaml");
    }
