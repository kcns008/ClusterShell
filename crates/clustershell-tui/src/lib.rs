...
use clustershell_core::proto::open_shell_client::ClusterShellClient;
...
/// Launch the ClusterShell TUI.
///
/// `channel` must be a connected gRPC channel to the ClusterShell gateway.
...
pub async fn run(
    ...
    let client = ClusterShellClient::new(channel);
    let mut app = App::new(
        client,
...
fn refresh_gateway_list(app: &mut App) {
    if let Ok(gateways) = clustershell_bootstrap::list_gateways() {
...
async fn handle_gateway_switch(app: &mut App) {
    ...
    match connect_to_gateway(&name, &endpoint).await {
        Ok(channel) => {
            app.client = ClusterShellClient::new(channel);
...
fn gateway_mtls_dir(name: &str) -> Option<PathBuf> {
    let config_dir = clustershell_core::paths::xdg_config_dir().ok()?;
    Some(
        config_dir
            .join("clustershell")
            .join("gateways")
...
fn spawn_log_stream(app: &mut App, tx: mpsc::UnboundedSender<Event>) {
    ...
    let handle = tokio::spawn(async move {
        ...
        let req = clustershell_core::proto::GetSandboxLogsRequest {
        ...
        let req = clustershell_core::proto::WatchSandboxRequest {
            id: sandbox_id,
            follow_status: false,
            follow_logs: true,
            follow_events: false,
...
fn proto_to_log_line(log: clustershell_core::proto::SandboxLogLine) -> LogLine {
...
async fn handle_sandbox_delete(app: &mut App) {
    ...
    if let Ok(stopped) = clustershell_core::forward::stop_forwards_for_sandbox(&sandbox_name) {
        for port in &stopped {
            tracing::info!("stopped forward of port {port} for sandbox {sandbox_name}");
        }
    }

    let req = clustershell_core::proto::DeleteSandboxRequest { name: sandbox_name };
...
async fn fetch_sandbox_detail(app: &mut App) {
    ...
    let req = clustershell_core::proto::GetSandboxRequest {
    ...
    if let Some(id) = sandbox_id {
        let policy_req = clustershell_core::proto::GetSandboxConfigRequest { sandbox_id: id };
        ...
        .await
        {
            Ok(Ok(resp)) => {
                ...
                app.sandbox_policy_is_global =
                    inner.policy_source == clustershell_core::proto::PolicySource::Global as i32;
...
///
/// This replicates the `clustershell sandbox connect` flow but uses `Command::status()`
...
async fn handle_shell_connect(
    ...
    let session = {
        let req = clustershell_core::proto::CreateSshSessionRequest {
            sandbox_id: sandbox_id.clone(),
        };
...
// SSH utility functions are shared via clustershell_core::forward.
use clustershell_core::forward::{resolve_ssh_gateway, shell_escape};

/// Convert a `SandboxPolicy` proto into styled ratatui lines for the policy viewer.
fn render_policy_lines(
    policy: &clustershell_core::proto::SandboxPolicy,
    theme: &theme::Theme,
...
fn spawn_create_sandbox(app: &mut App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        ...
        let template = if has_custom_image {
            Some(clustershell_core::proto::SandboxTemplate {
                image,
                ..Default::default()
            })
        } else {
        ...
        let policy = if has_custom_image {
            Some(clustershell_policy::restrictive_default_policy())
        } else {
            None
        };

        let req = clustershell_core::proto::CreateSandboxRequest {
            name,
            spec: Some(clustershell_core::proto::SandboxSpec {
...
async fn start_port_forwards(
    client: &mut ClusterShellClient<Channel>,
    endpoint: &str,
    gateway_name: &str,
    sandbox_name: &str,
    sandbox_id: &str,
    specs: &[clustershell_core::forward::ForwardSpec],
) {
    // Create SSH session.
    let session = {
        let req = clustershell_core::proto::CreateSshSessionRequest {
    ...
    for spec in specs {
        ...
        match result {
            Ok(Ok(true)) => {
                if let Some(pid) = clustershell_core::forward::find_ssh_forward_pid(&sid, port_val) {
                    let _ = clustershell_core::forward::write_forward_pid(
...
fn spawn_create_provider(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        ...
        for attempt in 0..5u32 {
            ...
            let req = clustershell_core::proto::CreateProviderRequest {
                provider: Some(clustershell_core::proto::Provider {
                    id: String::new(),
...
fn spawn_get_provider(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        let req = clustershell_core::proto::GetProviderRequest { name };
...
fn spawn_update_provider(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        ...
        let req = clustershell_core::proto::UpdateProviderRequest {
            provider: Some(clustershell_core::proto::Provider {
                id: String::new(),
                name: name.clone(),
                r#type: ptype,
...
fn spawn_delete_provider(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        let req = clustershell_core::proto::DeleteProviderRequest { name };
...
fn spawn_draft_approve(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        let req = clustershell_core::proto::ApproveDraftChunkRequest { name, chunk_id };
...
fn spawn_draft_reject(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        let req = clustershell_core::proto::RejectDraftChunkRequest {
            name,
            chunk_id,
            reason: String::new(),
        };
...
fn spawn_draft_approve_all(
    app: &App,
    snapshot: Vec<clustershell_core::proto::PolicyChunk>,
    ...
    tokio::spawn(async move {
        ...
        for chunk in &snapshot {
            let req = clustershell_core::proto::ApproveDraftChunkRequest {
                name: name.clone(),
...
async fn refresh_providers(app: &mut App) {
    let req = clustershell_core::proto::ListProvidersRequest {
        limit: 100,
        offset: 0,
    };
...
async fn refresh_global_settings(app: &mut App) {
    let req = clustershell_core::proto::GetGatewayConfigRequest {};
    let result =
    ...
    let policy_req = clustershell_core::proto::ListSandboxPoliciesRequest {
        name: String::new(),
        limit: 1,
        offset: 0,
        global: true,
    ...
    .await
    {
        ...
        if let Some(latest) = revisions.first() {
            let status =
                clustershell_core::proto::PolicyStatus::try_from(latest.status).unwrap_or_default();
            app.global_policy_active = status == clustershell_core::proto::PolicyStatus::Loaded;
...
fn spawn_set_global_setting(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        ...
        use clustershell_core::proto::{SettingValue, UpdateConfigRequest, setting_value};

        let value = match kind {
            clustershell_core::settings::SettingValueKind::Bool => {
                match clustershell_core::settings::parse_bool_like(&raw) {
            ...
            clustershell_core::settings::SettingValueKind::Int => match raw.parse::<i64>() {
            ...
            clustershell_core::settings::SettingValueKind::String => {
...
fn spawn_delete_global_setting(app: &App, tx: mpsc::UnboundedSender<Event>) {
    ...
    tokio::spawn(async move {
        use clustershell_core::proto::UpdateConfigRequest;
...
async fn refresh_health(app: &mut App) {
    let req = clustershell_core::proto::HealthRequest {};
...
async fn refresh_sandboxes(app: &mut App) {
    let req = clustershell_core::proto::ListSandboxesRequest {
        limit: 100,
        offset: 0,
    };
    ...
    match result {
        ...
        Ok(Ok(resp)) => {
            ...
            let forwards = clustershell_core::forward::list_forwards().unwrap_or_default();
            app.sandbox_notes = sandboxes
                .iter()
                .map(|s| clustershell_core::forward::build_sandbox_notes(&s.name, &forwards))
                .collect();
...
async fn refresh_sandbox_policy(app: &mut App) {
    ...
    let policy_req = clustershell_core::proto::GetSandboxConfigRequest { sandbox_id };
...
async fn refresh_draft_chunks(app: &mut App) {
    ...
    let req = clustershell_core::proto::GetDraftPolicyRequest {
        name: sandbox_name,
