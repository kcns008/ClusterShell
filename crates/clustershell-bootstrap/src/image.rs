...
///
/// Set via `CLUSTERSHELL_IMAGE_TAG` env var during `cargo build`:
...
pub const DEFAULT_IMAGE_TAG: &str = match option_env!("CLUSTERSHELL_IMAGE_TAG") {
    Some(tag) => tag,
    None => "dev",
};
...
pub const DEFAULT_IMAGE_REPO_BASE: &str = "ghcr.io/nvidia/clustershell";

/// Default full gateway image path on GHCR (without tag).
pub const DEFAULT_GATEWAY_IMAGE: &str = "ghcr.io/nvidia/clustershell/cluster";
...
/// `clustershell/cluster:dev`) so that all downstream container creation logic works
/// without changes.
///
...
pub async fn pull_remote_image(
    ...
    // Determine the registry tag to pull.  If CLUSTERSHELL_CLUSTER_IMAGE is set
    ...
    // e.g., tag "ghcr.io/nvidia/clustershell/cluster:latest" as "clustershell/cluster:dev"
