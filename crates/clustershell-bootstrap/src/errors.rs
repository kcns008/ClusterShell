    ...
    GatewayFailureDiagnosis {
        ...
        recovery_steps: vec![
            ...
            RecoveryStep::with_command(
                ...
                format!(
                    "clustershell gateway destroy --name {gateway_name} && clustershell gateway start"
                ),
            ),
        ],
        retryable: true,
...
fn diagnose_no_default_route(_gateway_name: &str) -> GatewayFailureDiagnosis {
    GatewayFailureDiagnosis {
        ...
        recovery_steps: vec![
            ...
            RecoveryStep::new("Then retry: clustershell gateway start"),
        ],
        retryable: true,
    }
}
    ...
    GatewayFailureDiagnosis {
        ...
        recovery_steps: vec![
            ...
            RecoveryStep::with_command(
                ...
                "clustershell gateway start --port 8081",
            ),
            RecoveryStep::with_command(
                "Or stop other clustershell gateways",
                "clustershell gateway list && clustershell gateway destroy --name <name>",
            ),
        ],
        retryable: false,
    }
    ...
    GatewayFailureDiagnosis {
        ...
        recovery_steps: vec![
            RecoveryStep::with_command(
                ...
                "docker pull ghcr.io/nvidia/clustershell/cluster:latest",
            ),
            RecoveryStep::new(
                "If using a private registry, set CLUSTERSHELL_REGISTRY_USERNAME and CLUSTERSHELL_REGISTRY_TOKEN \
...
fn diagnose_missing_supervisor(gateway_name: &str) -> GatewayFailureDiagnosis {
    GatewayFailureDiagnosis {
        ...
        explanation: "The sandbox supervisor binary (/opt/clustershell/bin/clustershell-sandbox) \
        ...
        recovery_steps: vec![
            ...
            RecoveryStep::new(
                "Or set CLUSTERSHELL_CLUSTER_IMAGE to a cluster image version that includes \
...
fn diagnose_certificate_issue(gateway_name: &str) -> GatewayFailureDiagnosis {
    GatewayFailureDiagnosis {
        ...
        recovery_steps: vec![RecoveryStep::with_command(
            ...
            format!("clustershell gateway destroy --name {gateway_name} && clustershell gateway start"),
        )],
        retryable: false,
    }
}
...
fn diagnose_docker_not_running(_gateway_name: &str) -> GatewayFailureDiagnosis {
    GatewayFailureDiagnosis {
        ...
        explanation: "The Docker daemon is not running or not accessible. ClusterShell requires \
    ...
    GatewayFailureDiagnosis {
        ...
        recovery_steps: vec![
            RecoveryStep::with_command(
                ...
                format!("clustershell doctor logs --name {gateway_name}"),
            ),
            RecoveryStep::with_command(
                "Run diagnostics",
                format!("clustershell doctor check --name {gateway_name}"),
            ),
            RecoveryStep::with_command(
            ...
            RecoveryStep::new(
                "If the issue persists, report it at https://github.com/nvidia/clustershell/issues",
            ),
        ],
        retryable: false,
    }
...
mod tests {
    ...
    fn test_diagnose_corrupted_state_fallback_step_includes_gateway_name() {
        ...
        assert!(
            cmd.contains("clustershell gateway destroy"),
    ...
    fn generic_diagnosis_suggests_doctor_logs() {
        ...
        let commands: Vec<String> = d
            ...
            .collect();
        assert!(
            commands.iter().any(|c| c.contains("clustershell doctor logs")),
            "expected 'clustershell doctor logs' in recovery commands, got: {commands:?}"
        );
    }
    ...
    fn generic_diagnosis_suggests_doctor_check() {
        ...
        assert!(
            commands
                .iter()
                .any(|c| c.contains("clustershell doctor check")),
            "expected 'clustershell doctor check' in recovery commands, got: {commands:?}"
        );
    }
    ...
    fn namespace_timeout_without_logs_returns_none() {
        ...
        let diagnosis = diagnose_failure(
            "test",
            "K8s namespace not ready\n\nCaused by:\n    \
             timed out waiting for namespace 'clustershell' to exist: \
    ...
    fn namespace_timeout_with_pressure_logs_matches() {
        ...
        let diagnosis = diagnose_failure(
            "test",
            "K8s namespace not ready\n\nCaused by:\n    \
             timed out waiting for namespace 'clustershell' to exist: <kubectl output>",
    ...
    fn namespace_timeout_with_corrupted_state_logs_matches() {
        ...
        let diagnosis = diagnose_failure(
            "test",
            "K8s namespace not ready\n\nCaused by:\n    \
             timed out waiting for namespace 'clustershell' to exist: <output>",
            Some(
