...
use miette::Result;
use clustershell_core::paths::xdg_config_dir;
use std::path::PathBuf;

/// Path to the file that stores the active gateway name.
///
/// Location: `$XDG_CONFIG_HOME/clustershell/active_gateway`
pub fn active_gateway_path() -> Result<PathBuf> {
    Ok(xdg_config_dir()?.join("clustershell").join("active_gateway"))
}

/// Base directory for all gateway metadata files.
///
/// Location: `$XDG_CONFIG_HOME/clustershell/gateways/`
pub fn gateways_dir() -> Result<PathBuf> {
    Ok(xdg_config_dir()?.join("clustershell").join("gateways"))
}

/// Path to the file that stores the last-used sandbox name for a gateway.
///
/// Location: `$XDG_CONFIG_HOME/clustershell/gateways/<gateway>/last_sandbox`
...
mod tests {
    ...
    fn last_sandbox_path_layout() {
        ...
        assert!(
            path.ends_with("clustershell/gateways/my-gateway/last_sandbox"),
