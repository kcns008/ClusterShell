...
const REGISTRY_NAMESPACE_DEFAULT: &str = "clustershell";
    ...
    if !socket_exists && docker_host.is_none() {
        ...
        hints.push(
            "Install and start a Docker-compatible runtime. See the support matrix \
             in the ClusterShell docs for tested configurations."
                .to_string(),
        );
...
/// Find the running clustershell gateway container by image name.
///
/// Lists all running containers and returns the name of the one whose image
/// contains `clustershell/cluster`. When `port` is provided, only containers
...
pub async fn find_gateway_container(docker: &Docker, port: Option<u16>) -> Result<String> {
    ...
    let is_gateway_image = |c: &bollard::models::ContainerSummary| {
        c.image
            .as_deref()
            .is_some_and(|img| img.contains("clustershell/cluster"))
    };
    ...
    match matches.len() {
        0 => {
            let hint = if let Some(p) = port {
                format!(
                    "No clustershell gateway container found listening on port {p}.\n\
                     Is the gateway running? Check with: docker ps"
                )
            } else {
                "No clustershell gateway container found.\n\
                 Is the gateway running? Check with: docker ps"
                    .to_string()
            };
        ...
        _ => Err(miette::miette!(
            "Found multiple clustershell gateway containers: {}\n\
...
pub async fn ensure_container(
    ...
    let mut host_config = HostConfig {
        ...
        extra_hosts: Some(vec![
            ...
            "host.clustershell.internal:host-gateway".to_string(),
        ]),
        ..Default::default()
    };
    ...
    // k3s registries.yaml.
    let registry_host =
        env_non_empty("CLUSTERSHELL_REGISTRY_HOST").unwrap_or_else(|| DEFAULT_REGISTRY.to_string());
    let registry_namespace = env_non_empty("CLUSTERSHELL_REGISTRY_NAMESPACE")
        .unwrap_or_else(|| REGISTRY_NAMESPACE_DEFAULT.to_string());
    let image_repo_base = env_non_empty("IMAGE_REPO_BASE")
        .or_else(|| env_non_empty("CLUSTERSHELL_IMAGE_REPO_BASE"))
        .unwrap_or_else(|| {
    ...
    let registry_insecure = env_bool("CLUSTERSHELL_REGISTRY_INSECURE").unwrap_or(false);
    let registry_endpoint = env_non_empty("CLUSTERSHELL_REGISTRY_ENDPOINT");

    // Credential priority:
    // 1. CLUSTERSHELL_REGISTRY_USERNAME/PASSWORD env vars (power-user override)
    ...
    let effective_username = env_non_empty("CLUSTERSHELL_REGISTRY_USERNAME").or_else(|| {
        registry_username
            .filter(|u| !u.is_empty())
            .map(ToString::to_string)
    });
    let effective_password = env_non_empty("CLUSTERSHELL_REGISTRY_PASSWORD").or_else(|| {
        registry_token
            .filter(|t| !t.is_empty())
            .map(ToString::to_string)
    });
    ...
    // When CLUSTERSHELL_PUSH_IMAGES is set the entrypoint overrides the baked-in
    ...
    let push_mode = std::env::var("CLUSTERSHELL_PUSH_IMAGES")
        .ok()
    ...
    if push_mode {
        if let Ok(images) = std::env::var("CLUSTERSHELL_PUSH_IMAGES")
