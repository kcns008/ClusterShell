...
use crate::runtime::{
    clean_stale_nodes, exec_capture_with_exit, fetch_recent_logs, clustershell_workload_exists,
    restart_clustershell_deployment, wait_for_gateway_ready,
};
    ...
    F: FnMut(String) + Send + 'static,
{
        ...
        if recreate {
            ...
            return Err(miette::miette!(
                "Gateway '{name}' already exists (container_running={}).\n\
                 ...
                 clustershell gateway destroy --name {name}",
    ...
    let deploy_result: Result<GatewayMetadata> = async {
        ...
        let workload_existed_before_pki = clustershell_workload_exists(&target_docker, &name).await?;
        let (pki_bundle, rotated) = reconcile_pki(&target_docker, &name, &extra_sans, &log).await?;

        if rotated && workload_existed_before_pki {
            // If an clustershell workload is already running, it must be restarted so
            ...
            restart_clustershell_deployment(&target_docker, &name).await?;
        }
        ...
        if remote_opts.is_none()
            && let Ok(push_images_str) = std::env::var("CLUSTERSHELL_PUSH_IMAGES")
        {
...
/// `clustershell-cluster-{name}`, and writes them to the provided writer.
///
...
fn default_gateway_image_ref() -> String {
    if let Ok(image) = std::env::var("CLUSTERSHELL_CLUSTER_IMAGE")
        && !image.trim().is_empty()
    {
        return image;
    }
...
/// Create the three TLS K8s secrets required by the `ClusterShell` server and sandbox pods.
///
/// Secrets are created via `kubectl` exec'd inside the cluster container:
/// - `clustershell-server-tls` (kubernetes.io/tls): server cert + key
/// - `clustershell-server-client-ca` (Opaque): CA cert for verifying client certs
/// - `clustershell-client-tls` (Opaque): client cert + key + CA cert (shared by CLI & sandboxes)
async fn create_k8s_tls_secrets(
    ...
    // 1. clustershell-server-tls (kubernetes.io/tls)
    ...
    apply_secret(server_tls_manifest.to_string())
        .await
        .wrap_err("failed to create clustershell-server-tls secret")?;

    // 2. clustershell-server-client-ca (Opaque)
    ...
    apply_secret(client_ca_manifest.to_string())
        .await
        .wrap_err("failed to create clustershell-server-client-ca secret")?;

    // 3. clustershell-client-tls (Opaque) — shared by CLI and sandbox pods
    ...
    apply_secret(client_tls_manifest.to_string())
        .await
        .wrap_err("failed to create clustershell-client-tls secret")?;
    ...
    F: Fn(String) + Sync,
{
    ...
    log("[progress] Waiting for clustershell namespace".to_string());
    wait_for_namespace(docker, &cname, kubeconfig, "clustershell").await?;
...
async fn load_existing_pki_bundle(
    ...
    let read_secret_key = |secret: &str, key: &str| {
        ...
        async move {
            ...
            let cmd = format!(
                "KUBECONFIG={kubeconfig} kubectl get secret {secret} -n clustershell -o jsonpath='{jsonpath}' 2>/dev/null"
            );
            let (output, exit_code) = exec_capture_with_exit(
                &docker,
                &container_name,
...
///
/// The Helm controller creates the `clustershell` namespace when it processes
