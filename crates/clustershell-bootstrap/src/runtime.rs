...
/// binary (`/opt/clustershell/bin/clustershell-sandbox`) is missing from the node
    ...
    F: FnMut(String) + Send,
{
    ...
    for attempt in 0..attempts {
        ...
        // The health-check script verifies that /opt/clustershell/bin/clustershell-sandbox
            ...
            .any(|line| line.contains(MISSING_SUPERVISOR_MARKER))
        {
            result = Some(Err(miette::miette!(
                "The sandbox supervisor binary is missing from the cluster image.\n\
                 The file /opt/clustershell/bin/clustershell-sandbox was not found in the gateway \
                 ...
                 3. Then recreate the gateway: clustershell gateway destroy && clustershell gateway start\n\n{}",
                format_recent_logs(&recent_logs)
            )));
            break;
        }
...
/// Restart the clustershell workload so pods pick up updated images or secrets.
///
...
pub async fn restart_clustershell_deployment(docker: &Docker, name: &str) -> Result<()> {
    let cname = container_name(name);

    // Detect which workload kind exists in the cluster.
    let workload_kind = detect_clustershell_workload_kind(docker, &cname).await?;
    let workload_ref = format!("{workload_kind}/clustershell");
    ...
    let (restart_output, restart_exit) = exec_capture_with_exit(
        ...
        vec![
            ...
            "-c".to_string(),
            format!(
                "KUBECONFIG={KUBECONFIG_PATH} kubectl rollout restart {workload_ref} -n clustershell"
            ),
        ],
    )
    .await?;
    if restart_exit != 0 {
        return Err(miette::miette!(
            "failed to restart clustershell {workload_ref} (exit code {restart_exit})\n{restart_output}"
        ));
    }
    ...
    let (status_output, status_exit) = exec_capture_with_exit(
        ...
        vec![
            ...
            "-c".to_string(),
            format!(
                "KUBECONFIG={KUBECONFIG_PATH} kubectl rollout status {workload_ref} -n clustershell --timeout=180s"
            ),
        ],
    )
    .await?;
    if status_exit != 0 {
        return Err(miette::miette!(
            "clustershell rollout status failed for {workload_ref} (exit code {status_exit})\n{status_output}"
        ));
    }
...
/// Check whether an clustershell workload exists in the cluster (`StatefulSet` or `Deployment`).
pub async fn clustershell_workload_exists(docker: &Docker, name: &str) -> Result<bool> {
    let cname = container_name(name);
    match detect_clustershell_workload_kind(docker, &cname).await {
        Ok(_) => Ok(true),
        Err(_) => Ok(false),
    }
}

/// Detect whether clustershell is deployed as a `StatefulSet` or `Deployment`.
/// Returns "statefulset" or "deployment".
async fn detect_clustershell_workload_kind(docker: &Docker, container_name: &str) -> Result<String> {
    ...
    let (_, ss_exit) = exec_capture_with_exit(
        ...
        vec![
            ...
            "-c".to_string(),
            format!(
                "KUBECONFIG={KUBECONFIG_PATH} kubectl get statefulset/clustershell -n clustershell -o name 2>/dev/null"
            ),
        ],
    )
    .await?;
    ...
    let (_, dep_exit) = exec_capture_with_exit(
        ...
        vec![
            ...
            "-c".to_string(),
            format!(
                "KUBECONFIG={KUBECONFIG_PATH} kubectl get deployment/clustershell -n clustershell -o name 2>/dev/null"
            ),
        ],
    )
    .await?;
    ...
    Err(miette::miette!(
        "no clustershell workload (statefulset or deployment) found in namespace 'clustershell'"
    ))
}
