...
pub fn mock_response(route: &ResolvedRoute, source_protocol: &str) -> ProxyResponse {
    ...
    ProxyResponse {
        ...
        headers: vec![
            ...
            ("x-clustershell-mock".to_string(), "true".to_string()),
        ],
        body: body_bytes,
    }
}
...
fn openai_chat_completion_body(model: &str) -> Vec<u8> {
    serde_json::to_vec(&serde_json::json!({
        ...
        "choices": [{
            ...
            "message": {
                "role": "assistant",
                "content": "Hello from clustershell mock backend"
            },
            "finish_reason": "stop"
        }],
        "usage": {
...
fn openai_completion_body(model: &str) -> Vec<u8> {
    serde_json::to_vec(&serde_json::json!({
        ...
        "choices": [{
            "index": 0,
            "text": "Hello from clustershell mock backend",
            "finish_reason": "stop"
        }],
        "usage": {
            "prompt_tokens": 1,
...
fn generic_body(model: &str) -> Vec<u8> {
    serde_json::to_vec(&serde_json::json!({
        "mock": true,
        "model": model,
        "message": "Hello from clustershell mock backend"
    }))
    .expect("static JSON must serialize")
}
...
mod tests {
    ...
    fn mock_openai_chat_completion() {
        ...
        assert_eq!(
            ...
            "Hello from clustershell mock backend"
        );
    }
    ...
    fn mock_response_includes_marker_header() {
        ...
        assert!(
            resp.headers
                .iter()
                .any(|(k, v)| k == "x-clustershell-mock" && v == "true")
        );
    }
}
