...
pub use clustershell_core::inference::AuthHeader;
...
impl RouteConfig {
    ...
    fn resolve(&self) -> Result<ResolvedRoute, RouterError> {
        let protocols = clustershell_core::inference::normalize_protocols(&self.protocols);
...
///
/// Delegates to [`clustershell_core::inference::auth_for_provider_type`] which
...
fn auth_from_provider_type(provider_type: Option<&str>) -> (AuthHeader, Vec<(String, String)>) {
    clustershell_core::inference::auth_for_provider_type(provider_type.unwrap_or(""))
}
