...
//! `ClusterShell` Server library.
//!
//! This crate provides the server implementation for `ClusterShell`, including:
...
use clustershell_core::{Config, Error, Result};
...
pub use grpc::ClusterShellService;
...
pub async fn run_server(config: Config, tracing_log_bus: TracingLogBus) -> Result<()> {
    ...
    if config.ssh_handshake_secret.is_empty() {
        return Err(Error::config(
            "ssh_handshake_secret is required. Set --ssh-handshake-secret or CLUSTERSHELL_SSH_HANDSHAKE_SECRET",
        ));
    }
