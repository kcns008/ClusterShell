...
use clustershell_router::config::ResolvedRoute as RouterResolvedRoute;
use clustershell_router::{ValidationFailureKind, verify_backend_endpoint};
use std::sync::Arc;
        ...
        let route = self
            ...
            .ok_or_else(|| {
                Status::not_found(format!(
                    "inference route '{route_name}' is not configured; run 'clustershell inference set --provider <name> --model <id>'"
                ))
            })?;
...
fn resolve_provider_route(provider: &Provider) -> Result<ResolvedProviderRoute, Status> {
    ...
    let profile = clustershell_core::inference::profile_for(&provider_type).ok_or_else(|| {
