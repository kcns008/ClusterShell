...
//! `ClusterShell` Server - gRPC/HTTP server with protocol multiplexing.
...
use clustershell_server::{run_server, tracing_bus::TracingLogBus};

/// `ClusterShell` Server - gRPC and HTTP server with protocol multiplexing.
#[derive(Parser, Debug)]
#[command(name = "clustershell-server")]
#[command(version = clustershell_core::VERSION)]
#[command(about = "ClusterShell gRPC/HTTP server", long_about = None)]
struct Args {
    /// Port to bind the server to (all interfaces).
    #[arg(long, default_value_t = 8080, env = "CLUSTERSHELL_SERVER_PORT")]
    port: u16,

    /// Log level (trace, debug, info, warn, error).
    #[arg(long, default_value = "info", env = "CLUSTERSHELL_LOG_LEVEL")]
    log_level: String,

    /// Path to TLS certificate file (required unless --disable-tls).
    #[arg(long, env = "CLUSTERSHELL_TLS_CERT")]
    tls_cert: Option<PathBuf>,

    /// Path to TLS private key file (required unless --disable-tls).
    #[arg(long, env = "CLUSTERSHELL_TLS_KEY")]
    tls_key: Option<PathBuf>,

    /// Path to CA certificate for client certificate verification (mTLS).
    #[arg(long, env = "CLUSTERSHELL_TLS_CLIENT_CA")]
    tls_client_ca: Option<PathBuf>,

    /// Database URL for persistence.
    #[arg(long, env = "CLUSTERSHELL_DB_URL", required = true)]
    db_url: String,

    /// Kubernetes namespace for sandboxes.
    #[arg(long, env = "CLUSTERSHELL_SANDBOX_NAMESPACE", default_value = "default")]
    sandbox_namespace: String,

    /// Default container image for sandboxes.
    #[arg(long, env = "CLUSTERSHELL_SANDBOX_IMAGE")]
    sandbox_image: Option<String>,

    /// Kubernetes imagePullPolicy for sandbox pods (Always, IfNotPresent, Never).
    #[arg(long, env = "CLUSTERSHELL_SANDBOX_IMAGE_PULL_POLICY")]
    sandbox_image_pull_policy: Option<String>,

    /// gRPC endpoint for sandboxes to callback to `ClusterShell`.
    /// This should be reachable from within the Kubernetes cluster.
    #[arg(long, env = "CLUSTERSHELL_GRPC_ENDPOINT")]
    grpc_endpoint: Option<String>,

    /// Public host for the SSH gateway.
    #[arg(long, env = "CLUSTERSHELL_SSH_GATEWAY_HOST", default_value = "127.0.0.1")]
    ssh_gateway_host: String,

    /// Public port for the SSH gateway.
    #[arg(long, env = "CLUSTERSHELL_SSH_GATEWAY_PORT", default_value_t = 8080)]
    ssh_gateway_port: u16,

    /// HTTP path for SSH CONNECT/upgrade.
    #[arg(
        long,
        env = "CLUSTERSHELL_SSH_CONNECT_PATH",
        default_value = "/connect/ssh"
    )]
    ssh_connect_path: String,

    /// SSH port inside sandbox pods.
    #[arg(long, env = "CLUSTERSHELL_SANDBOX_SSH_PORT", default_value_t = 2222)]
    ...
    #[arg(long, env = "CLUSTERSHELL_SSH_HANDSHAKE_SKEW_SECS", default_value_t = 300)]
    ssh_handshake_skew_secs: u64,

    /// Kubernetes secret name containing client TLS materials for sandbox pods.
    #[arg(long, env = "CLUSTERSHELL_CLIENT_TLS_SECRET_NAME")]
    ...
    #[arg(long, env = "CLUSTERSHELL_HOST_GATEWAY_IP")]
    ...
    #[arg(long, env = "CLUSTERSHELL_DISABLE_TLS")]
    disable_tls: bool,
    ...
    #[arg(long, env = "CLUSTERSHELL_DISABLE_GATEWAY_AUTH")]
...
async fn main() -> Result<()> {
    ...
    let tls = if args.disable_tls {
        ...
        Some(clustershell_core::TlsConfig {
            cert_path,
            key_path,
            client_ca_path,
    ...
    let mut config = clustershell_core::Config::new(tls)
    ...
    info!(bind = %config.bind_address, "Starting ClusterShell server");
