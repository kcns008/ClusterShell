...
//! Capture clustershell-server tracing logs for streaming over gRPC.

use std::collections::{HashMap, VecDeque};
use std::sync::{Arc, Mutex};
use std::time::{SystemTime, UNIX_EPOCH};

use clustershell_core::proto::{SandboxLogLine, SandboxStreamEvent};
...
impl TracingLogBus {
    ...
    pub fn publish_external(&self, log: SandboxLogLine) {
        let evt = SandboxStreamEvent {
            payload: Some(clustershell_core::proto::sandbox_stream_event::Payload::Log(
                log.clone(),
            )),
        };
...
mod tests {
    ...
    fn platform_event_bus_tail_returns_buffered_events() {
        use clustershell_core::proto::{PlatformEvent, sandbox_stream_event};
