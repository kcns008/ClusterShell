...
const SANDBOX_ID_LABEL: &str = "clustershell.ai/sandbox-id";
const SANDBOX_MANAGED_LABEL: &str = "clustershell.ai/managed-by";
const SANDBOX_MANAGED_VALUE: &str = "clustershell";
...
pub struct SandboxClient {
    ...
    /// `host.docker.internal` and `host.clustershell.internal` to this IP.
    ...
    pub async fn validate_gpu_support(&self) -> Result<(), tonic::Status> {
        ...
        if !runtime_class_exists {
            return Err(tonic::Status::failed_precondition(
                "GPU sandbox requested, but the active gateway is not GPU-enabled. To start a gateway with GPU support run: `clustershell gateway start --gpu`",
            ));
        }
        ...
        if !has_gpu_capacity {
            return Err(tonic::Status::failed_precondition(
                "GPU sandbox requested, but the active gateway has no allocatable GPUs. Please refer to documentation and use `clustershell doctor` commands to inspect GPU support and gateway configuration.",
            ));
        }
...
const SUPERVISOR_MOUNT_PATH: &str = "/opt/clustershell/bin";

/// Name of the volume used to side-load the supervisor binary.
const SUPERVISOR_VOLUME_NAME: &str = "clustershell-supervisor-bin";
...
const SUPERVISOR_HOST_PATH: &str = "/opt/clustershell/bin";
...
fn apply_supervisor_sideload(pod_template: &mut serde_json::Value) {
    ...
    if let Some(container) = containers.get_mut(index).and_then(|v| v.as_object_mut()) {
        ...
        container.insert(
            "command".to_string(),
            serde_json::json!([format!("{}/clustershell-sandbox", SUPERVISOR_MOUNT_PATH)]),
        );
...
fn sandbox_template_to_k8s(
    ...
    // Build environment variables - start with ClusterShell-required vars
    ...
    if !client_tls_secret_name.is_empty() {
        container.insert(
            ...
            serde_json::json!([{
                "name": "clustershell-client-tls",
                "mountPath": "/etc/clustershell-tls/client",
                "readOnly": true
            }]),
        );
    }
    ...
    if !host_gateway_ip.is_empty() {
        spec.insert(
            ...
            serde_json::json!([{
                "ip": host_gateway_ip,
                "hostnames": ["host.docker.internal", "host.clustershell.internal"]
            }]),
        );
    }
...
fn apply_required_env(
    ...
    upsert_env(env, "CLUSTERSHELL_SANDBOX_ID", sandbox_id);
    upsert_env(env, "CLUSTERSHELL_SANDBOX", sandbox_name);
    upsert_env(env, "CLUSTERSHELL_ENDPOINT", grpc_endpoint);
    upsert_env(env, "CLUSTERSHELL_SANDBOX_COMMAND", "sleep infinity");
    if !ssh_listen_addr.is_empty() {
        upsert_env(env, "CLUSTERSHELL_SSH_LISTEN_ADDR", ssh_listen_addr);
    }
    upsert_env(env, "CLUSTERSHELL_SSH_HANDSHAKE_SECRET", ssh_handshake_secret);
    upsert_env(
        env,
        "CLUSTERSHELL_SSH_HANDSHAKE_SKEW_SECS",
    ...
    if tls_enabled {
        upsert_env(env, "CLUSTERSHELL_TLS_CA", "/etc/clustershell-tls/client/ca.crt");
        upsert_env(
            env,
            "CLUSTERSHELL_TLS_CERT",
            "/etc/clustershell-tls/client/tls.crt",
        );
        upsert_env(
            env,
            "CLUSTERSHELL_TLS_KEY",
            "/etc/clustershell-tls/client/tls.key",
        );
    }
}
...
fn rewrite_user_facing_conditions(status: &mut Option<SandboxStatus>, spec: Option<&SandboxSpec>) {
    ...
    if let Some(status) = status {
        for condition in &mut status.conditions {
            ...
            {
                condition.message = "GPU sandbox could not be scheduled on the active gateway. Another GPU sandbox may already be using the available GPU, or the gateway may not currently be able to satisfy GPU placement. Please refer to documentation and use `clustershell doctor` commands to inspect GPU support and gateway configuration.".to_string();
            }
        }
    }
}
...
mod tests {
    ...
    fn rewrite_user_facing_conditions_rewrites_gpu_unschedulable_message() {
        ...
        assert_eq!(
            message,
            "GPU sandbox could not be scheduled on the active gateway. Another GPU sandbox may already be using the available GPU, or the gateway may not currently be able to satisfy GPU placement. Please refer to documentation and use `clustershell doctor` commands to inspect GPU support and gateway configuration."
        );
    }
    ...
    fn apply_required_env_always_injects_ssh_handshake_secret() {
        ...
        let secret_entry = env
            .iter()
            .find(|e| {
                e.get("name").and_then(|v| v.as_str()) == Some("CLUSTERSHELL_SSH_HANDSHAKE_SECRET")
            })
            .expect("CLUSTERSHELL_SSH_HANDSHAKE_SECRET must be present in env");
        assert_eq!(
    ...
    fn supervisor_sideload_injects_hostpath_volume_and_mount() {
        ...
        assert_eq!(
            ...
            format!("{}/clustershell-sandbox", SUPERVISOR_MOUNT_PATH)
        );
    ...
    fn tls_env_vars_match_volume_mount_path() {
        ...
        const TLS_MOUNT_PATH: &str = "/etc/clustershell-tls/client";
        ...
        let tls_ca = get_env("CLUSTERSHELL_TLS_CA").expect("CLUSTERSHELL_TLS_CA must be set");
        let tls_cert = get_env("CLUSTERSHELL_TLS_CERT").expect("CLUSTERSHELL_TLS_CERT must be set");
        let tls_key = get_env("CLUSTERSHELL_TLS_KEY").expect("CLUSTERSHELL_TLS_KEY must be set");

        // All TLS paths must be within the mount path
        assert!(
            tls_ca.starts_with(TLS_MOUNT_PATH),
            "CLUSTERSHELL_TLS_CA path '{tls_ca}' must start with mount path '{TLS_MOUNT_PATH}'"
        );
        assert!(
            tls_cert.starts_with(TLS_MOUNT_PATH),
            "CLUSTERSHELL_TLS_CERT path '{tls_cert}' must start with mount path '{TLS_MOUNT_PATH}'"
        );
        assert!(
            tls_key.starts_with(TLS_MOUNT_PATH),
            "CLUSTERSHELL_TLS_KEY path '{tls_key}' must start with mount path '{TLS_MOUNT_PATH}'"
        );
    }
    ...
    fn host_aliases_injected_when_gateway_ip_set() {
        ...
        assert!(hostnames.contains(&serde_json::json!("host.clustershell.internal")));
    }
    ...
    fn tls_secret_volume_uses_restrictive_default_mode() {
        ...
        let tls_vol = volumes
            .iter()
            .find(|v| v["name"] == "clustershell-client-tls")
