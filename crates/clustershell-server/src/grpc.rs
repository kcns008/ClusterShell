...
use clustershell_core::proto::{
    ...
    WatchSandboxRequest, open_shell_server::ClusterShell,
};
use clustershell_core::proto::{
...
/// ClusterShell gRPC service implementation.
#[derive(Debug, Clone)]
pub struct ClusterShellService {
    state: Arc<ServerState>,
}

impl ClusterShellService {
    /// Create a new ClusterShell service.
    #[must_use]
...
#[tonic::async_trait]
impl ClusterShell for ClusterShellService {
    async fn health(
        &self,
        ...
        Ok(Response::new(HealthResponse {
            ...
            version: clustershell_core::VERSION.to_string(),
        }))
    }
    ...
    async fn create_sandbox(
        ...
        if let Some(ref mut policy) = spec.policy {
            clustershell_policy::ensure_sandbox_process_identity(policy);
        ...
        tokio::spawn(async move {
                ...
                Ok(Some(sandbox)) => {
                    ...
                    let _ = tx
                        .send(Ok(SandboxStreamEvent {
                            payload: Some(
                                clustershell_core::proto::sandbox_stream_event::Payload::Sandbox(
                                    sandbox.clone(),
                                ),
                            ),
                        }))
            ...
            if follow_logs {
                for evt in state.tracing_log_bus.tail(&sandbox_id, log_tail as usize) {
                    if let Some(clustershell_core::proto::sandbox_stream_event::Payload::Log(
                        ref log,
                    )) = evt.payload
                    {
                        ...
                        match res {
                            Ok(()) => {
                                match state.store.get_message::<Sandbox>(&sandbox_id).await {
                                    Ok(Some(sandbox)) => {
                                        state.sandbox_index.update_from_sandbox(&sandbox);
                                        if tx.send(Ok(SandboxStreamEvent { payload: Some(clustershell_core::proto::sandbox_stream_event::Payload::Sandbox(sandbox.clone()))})).await.is_err() {
                                            return;
                                        }
                                        if stop_on_terminal {
                    ...
                    res = async {
                        ...
                        match res {
                            Ok(evt) => {
                                ...
                                if let Some(clustershell_core::proto::sandbox_stream_event::Payload::Log(ref log)) = evt.payload {
                                    if !log_sources.is_empty() && !source_matches(&log.source, &log_sources) {
                                        continue;
                                    }
                                    if !level_matches(&log.level, &log_min_level) {
    ...
    async fn update_config(
        ...
        if req.global {
            ...
            if has_policy {
                ...
                clustershell_policy::ensure_sandbox_process_identity(&mut new_policy);
...
fn draft_chunk_record_to_proto(record: &DraftChunkRecord) -> Result<PolicyChunk, Status> {
    use clustershell_core::proto::NetworkPolicyRule;
...
fn validate_sandbox_spec(
    name: &str,
    spec: &clustershell_core::proto::SandboxSpec,
) -> Result<(), Status> {
    // --- request.name ---
...
///
/// Delegates to [`clustershell_policy::validate_sandbox_policy`] and converts
/// violations into a gRPC `INVALID_ARGUMENT` status.
fn validate_policy_safety(policy: &ProtoSandboxPolicy) -> Result<(), Status> {
    if let Err(violations) = clustershell_policy::validate_sandbox_policy(policy) {
...
fn validate_filesystem_additive(
    baseline: Option<&clustershell_core::proto::FilesystemPolicy>,
    new: Option<&clustershell_core::proto::FilesystemPolicy>,
) -> Result<(), Status> {
    match (baseline, new) {
        (Some(base), Some(upd)) => {
...
fn resolve_gateway(config: &clustershell_core::Config) -> (String, u16) {
            ...
            .await
            {
                ...
                let _ = tx
                    .send(Ok(ExecSandboxEvent {
                        payload: Some(clustershell_core::proto::exec_sandbox_event::Payload::Exit(
    ...
    while let Some(msg) = channel.wait().await {
        match msg {
            ChannelMsg::Data { data } => {
                let _ = tx
                    .send(Ok(ExecSandboxEvent {
                        payload: Some(clustershell_core::proto::exec_sandbox_event::Payload::Stdout(
                            ExecSandboxStdout {
                                data: data.to_vec(),
                            },
                        )),
            ...
            ChannelMsg::ExtendedData { data, .. } => {
                let _ = tx
                    .send(Ok(ExecSandboxEvent {
                        payload: Some(clustershell_core::proto::exec_sandbox_event::Payload::Stderr(
                            ExecSandboxStderr {
                                data: data.to_vec(),
                            },
                        )),
...
mod tests {
    ...
    use clustershell_core::proto::{Provider, SandboxSpec, SandboxTemplate};
    use prost::Message;
    ...
    fn env_key_validation_accepts_valid_keys() {
        ...
        assert!(is_valid_env_key("_CLUSTERSHELL_VALUE_1"));
    }
    ...
    async fn handler_flow_resolves_credentials_from_sandbox_providers() {
        use clustershell_core::proto::{Sandbox, SandboxPhase, SandboxSpec};
    ...
    async fn handler_flow_returns_none_for_unknown_sandbox() {
        use clustershell_core::proto::Sandbox;
    ...
    fn validate_policy_safety_rejects_path_traversal() {
        use clustershell_core::proto::{FilesystemPolicy, SandboxPolicy as ProtoSandboxPolicy};
    ...
    fn validate_policy_safety_accepts_valid_policy() {
        let policy = clustershell_policy::restrictive_default_policy();
    ...
    async fn merge_chunk_into_policy_adds_first_network_rule_to_empty_policy() {
        use clustershell_core::proto::{NetworkBinary, NetworkEndpoint, NetworkPolicyRule};
        ...
        let policy = clustershell_core::proto::SandboxPolicy::decode(latest.policy_payload.as_slice())
    ...
    fn validate_sandbox_spec_rejects_oversized_policy() {
        use clustershell_core::proto::NetworkPolicyRule;
        use clustershell_core::proto::SandboxPolicy as ProtoSandboxPolicy;
    ...
    fn merge_effective_settings_global_overrides_sandbox_key() {
        ...
        assert_eq!(
            log_level.scope,
            clustershell_core::proto::SettingScope::Global as i32
        );
        assert_eq!(
            log_level.value.as_ref().and_then(|v| v.value.as_ref()),
            Some(&clustershell_core::proto::setting_value::Value::StringValue(
                "warn".to_string(),
            ))
        );
        ...
        assert_eq!(
            dummy_bool.scope,
            clustershell_core::proto::SettingScope::Sandbox as i32
        );
    ...
    fn merge_effective_settings_includes_unset_registered_keys() {
        ...
        let merged = super::merge_effective_settings(&global, &sandbox).unwrap();
        for registered in clustershell_core::settings::REGISTERED_SETTINGS {
            let setting = merged
                .get(registered.key)
            ...
            assert_eq!(
                setting.scope,
                clustershell_core::proto::SettingScope::Unspecified as i32
            );
        }
    }
    ...
    fn decode_policy_from_global_settings_round_trip() {
        let policy = clustershell_core::proto::SandboxPolicy {
            version: 7,
            ..Default::default()
        };
    ...
    fn config_revision_changes_when_effective_setting_changes() {
        let policy = clustershell_core::proto::SandboxPolicy::default();
        let mut settings = HashMap::new();
        settings.insert(
            "mode".to_string(),
            clustershell_core::proto::EffectiveSetting {
                value: Some(clustershell_core::proto::SettingValue {
                    value: Some(clustershell_core::proto::setting_value::Value::StringValue(
                        "strict".to_string(),
                    )),
                }),
                scope: clustershell_core::proto::SettingScope::Sandbox.into(),
            },
        );
        ...
        let rev_a = super::compute_config_revision(
            Some(&policy),
            &settings,
            clustershell_core::proto::PolicySource::Sandbox,
        );
        settings.insert(
            "mode".to_string(),
            clustershell_core::proto::EffectiveSetting {
    ...
    fn proto_setting_to_stored_rejects_unknown_key() {
        let value = clustershell_core::proto::SettingValue {
            value: Some(clustershell_core::proto::setting_value::Value::StringValue(
                "hello".to_string(),
            )),
        };
    ...
    fn proto_setting_to_stored_accepts_bool_for_registered_bool_key() {
        let value = clustershell_core::proto::SettingValue {
            value: Some(clustershell_core::proto::setting_value::Value::BoolValue(true)),
        };
    ...
    fn config_revision_changes_when_policy_changes() {
        let policy_a = clustershell_core::proto::SandboxPolicy {
            version: 1,
            ..Default::default()
        };
        let policy_b = clustershell_core::proto::SandboxPolicy {
            version: 2,
            ..Default::default()
        };
    ...
    fn config_revision_changes_when_policy_source_changes() {
        ...
        let rev_b = super::compute_config_revision(
            Some(&policy),
            &settings,
            clustershell_core::proto::PolicySource::Global,
        );
        assert_ne!(
            rev_a, rev_b,
