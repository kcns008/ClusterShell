...
use clustershell_core::proto::{
    inference_server::InferenceServer, open_shell_server::ClusterShellServer,
};
...
use crate::{ClusterShellService, ServerState, http_router, inference::InferenceService};
...
impl MultiplexService {
    ...
    {
        let clustershell = ClusterShellServer::new(ClusterShellService::new(self.state.clone()))
            .max_decoding_message_size(MAX_GRPC_DECODE_SIZE);
        let inference = InferenceServer::new(InferenceService::new(self.state.clone()))
            .max_decoding_message_size(MAX_GRPC_DECODE_SIZE);
        let grpc_service = GrpcRouter::new(clustershell, inference);
...
/// Combined gRPC service that routes between `ClusterShell` and Inference services
...
impl<N, I> GrpcRouter<N, I> {
    fn new(clustershell: N, inference: I) -> Self {
        Self {
            clustershell,
            inference,
        }
...
const INFERENCE_PATH_PREFIX: &str = "/clustershell.inference.v1.Inference/";
    ...
    B: Send + 'static,
{
    ...
    fn call(&mut self, req: Request<B>) -> Self::Future {
        ...
        if is_inference {
            ...
            let mut svc = self.clustershell.clone();
            Box::pin(async move { svc.ready().await?.call(req).await })
        }
    }
}
