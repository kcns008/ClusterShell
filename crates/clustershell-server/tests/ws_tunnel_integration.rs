...
//!                          │
//!                     MultiplexedService ──▶ TestClusterShell
//! ```
//!
...
use clustershell_core::proto::{
    ...
    open_shell_client::ClusterShellClient,
...
// ---------------------------------------------------------------------------
// Minimal ClusterShell implementation (same as other integration tests)
// ---------------------------------------------------------------------------
...
type TestTunnelService = MultiplexedService<ClusterShellServer<TestClusterShell>, Router>;
...
// ---------------------------------------------------------------------------
// Test client-side TCP↔WS bridge (mirrors edge_tunnel.rs from clustershell-cli)
// ---------------------------------------------------------------------------
...
/// WS tunnel handler → loopback TCP → `MultiplexedService` → `TestClusterShell`.
#[tokio::test]
async fn ws_tunnel_grpc_health_through_websocket() {
    let stack = TestStack::start().await;

    // Baseline: direct gRPC works
    let mut direct_client = ClusterShellClient::connect(format!("http://{}", stack.grpc_addr))
        .await
    ...
    let mut tunnel_client = ClusterShellClient::connect(format!("http://{}", stack.proxy_addr))
        .await
...
async fn ws_tunnel_bidirectional_data_integrity() {
    ...
    let mut client = ClusterShellClient::connect(format!("http://{}", stack.proxy_addr))
        .await
        .expect("tunnel gRPC connect failed");
...
async fn ws_tunnel_concurrent_connections() {
    ...
    for i in 0..5 {
        ...
        handles.push(tokio::spawn(async move {
            let mut client = ClusterShellClient::connect(format!("http://{proxy_addr}"))
                .await
