...
use clustershell_server::{MultiplexedService, TlsAcceptor, health_router};
...
/// Minimal ClusterShell implementation for testing.
...
async fn grpc_client_mtls(
    ...
    ClusterShellClient::new(channel)
}
...
async fn grpc_client_with_cf_header(
    ...
    ca_pem: Vec<u8>,
    token: &str,
) -> ClusterShellClient<tonic::service::interceptor::InterceptedService<Channel, CfInterceptor>> {
    let ca_cert = tonic::transport::Certificate::from_pem(ca_pem);
    let tls = ClientTlsConfig::new()
        .ca_certificate(ca_cert)
        .domain_name("localhost");
    let endpoint = Endpoint::from_shared(format!("https://localhost:{}", addr.port()))
        ...
        .expect("failed to set tls");
    let channel = endpoint.connect().await.expect("failed to connect");
    ClusterShellClient::with_interceptor(
        channel,
        CfInterceptor {
...
async fn baseline_no_cert_rejected_with_mandatory_mtls() {
    ...
    if let Ok(channel) = result {
        let mut client = ClusterShellClient::new(channel);
