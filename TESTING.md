...
python/clustershell/      # Python unit tests (*_test.py suffix)
...
and live alongside the source in `python/clustershell/`. They use mock-based
...
Rust-based e2e tests that exercise the `clustershell` CLI binary as a subprocess.
They live in the `clustershell-e2e` crate and use a shared harness for sandbox
...
| Module | Purpose |
|---|---|
| `binary` | Builds and resolves the `clustershell` binary from the workspace |
...
| Variable | Purpose |
|---|---|
| `CLUSTERSHELL_GATEWAY` | Override active gateway name for E2E tests |
