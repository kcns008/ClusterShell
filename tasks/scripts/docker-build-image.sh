...
case "${TARGET}" in
  gateway)
    IS_FINAL_IMAGE=1
    IMAGE_NAME="clustershell/gateway"
    DOCKER_TARGET="gateway"
    ;;
  cluster)
    IS_FINAL_IMAGE=1
    IMAGE_NAME="clustershell/cluster"
...
if [[ -n "${IMAGE_REGISTRY:-}" && "${IS_FINAL_IMAGE}" == "1" ]]; then
  IMAGE_NAME="${IMAGE_REGISTRY}/${IMAGE_NAME#clustershell/}"
fi
...
if [[ -n "${CLUSTERSHELL_CARGO_VERSION:-}" ]]; then
  VERSION_ARGS=(--build-arg "CLUSTERSHELL_CARGO_VERSION=${CLUSTERSHELL_CARGO_VERSION}")
elif [[ -n "${CI:-}" ]]; then
  CARGO_VERSION=$(uv run python tasks/scripts/release.py get-version --cargo 2>/dev/null || true)
  if [[ -n "${CARGO_VERSION}" ]]; then
    VERSION_ARGS=(--build-arg "CLUSTERSHELL_CARGO_VERSION=${CARGO_VERSION}")
  fi
fi
...
if [[ "${TARGET}" == "cluster" ]]; then
  ...
  helm package deploy/helm/clustershell -d deploy/docker/.build/charts/ >/dev/null
fi
