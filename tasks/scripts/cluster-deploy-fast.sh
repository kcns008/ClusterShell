...
CONTAINER_NAME="clustershell-cluster-${CLUSTER_NAME}"
IMAGE_REPO_BASE=${IMAGE_REPO_BASE:-${CLUSTERSHELL_REGISTRY:-127.0.0.1:5000/clustershell}}
...
CONTAINER_CHART_DIR=/tmp/clustershell-chart
...
matches_gateway() {
  ...
  case "${path}" in
    ...
    deploy/docker/Dockerfile.images|tasks/scripts/docker-build-image.sh)
      return 0
      ;;
    crates/clustershell-core/*|crates/clustershell-policy/*|crates/clustershell-providers/*)
      return 0
      ;;
    crates/clustershell-router/*|crates/clustershell-server/*)
      return 0
      ;;
    *)
      return 1
...
matches_supervisor() {
  ...
  case "${path}" in
    ...
    deploy/docker/Dockerfile.images|tasks/scripts/docker-build-image.sh)
      return 0
      ;;
    crates/clustershell-core/*|crates/clustershell-policy/*|crates/clustershell-router/*)
      return 0
      ;;
    crates/clustershell-sandbox/*)
      return 0
      ;;
    *)
      return 1
...
compute_fingerprint() {
  ...
  case "${component}" in
    gateway)
      committed_trees=$(git ls-tree HEAD Cargo.toml Cargo.lock proto/ deploy/docker/cross-build.sh deploy/docker/Dockerfile.images tasks/scripts/docker-build-image.sh crates/clustershell-core/ crates/clustershell-policy/ crates/clustershell-providers/ crates/clustershell-router/ crates/clustershell-server/ 2>/dev/null || true)
      ;;
    supervisor)
      committed_trees=$(git ls-tree HEAD Cargo.toml Cargo.lock proto/ deploy/docker/cross-build.sh deploy/docker/Dockerfile.images tasks/scripts/docker-build-image.sh crates/clustershell-core/ crates/clustershell-policy/ crates/clustershell-router/ crates/clustershell-sandbox/ 2>/dev/null || true)
      ;;
    helm)
      committed_trees=$(git ls-tree HEAD deploy/helm/clustershell/ 2>/dev/null || true)
      ;;
  esac
...
# The binary lives at /opt/clustershell/bin/clustershell-sandbox on the node
...
if [[ "${build_supervisor}" == "1" ]]; then
  ...
  _cargo_version=${CLUSTERSHELL_CARGO_VERSION:-}
  ...
  CLUSTERSHELL_CARGO_VERSION="${_cargo_version}" \
    tasks/scripts/docker-build-image.sh supervisor-output

  # Copy the built binary into the running k3s container
  docker exec "${CONTAINER_NAME}" mkdir -p /opt/clustershell/bin
  docker cp "${SUPERVISOR_BUILD_DIR}/clustershell-sandbox" \
    "${CONTAINER_NAME}:/opt/clustershell/bin/clustershell-sandbox"
  docker exec "${CONTAINER_NAME}" chmod 755 /opt/clustershell/bin/clustershell-sandbox
...
if [[ "${build_gateway}" == "1" ]]; then
  docker tag "clustershell/gateway:${IMAGE_TAG}" "${IMAGE_REPO_BASE}/gateway:${IMAGE_TAG}" 2>/dev/null || true
...
if [[ "${needs_helm_upgrade}" == "1" ]]; then
  ...
  docker cp deploy/helm/clustershell "${CONTAINER_NAME}:${CONTAINER_CHART_DIR}"
  ...
  EXISTING_SECRET=$(cluster_exec "helm get values clustershell -n clustershell -o json 2>/dev/null \
  ...
  # that hostAliases for host.clustershell.internal are preserved across fast deploys.
  HOST_GATEWAY_IP=$(cluster_exec "kubectl -n kube-system get helmchart clustershell -o jsonpath='{.spec.valuesContent}' 2>/dev/null \
  ...
  cluster_exec "helm upgrade clustershell ${CONTAINER_CHART_DIR} \
    --namespace clustershell \
    --set image.repository=${IMAGE_REPO_BASE}/gateway \
    --set image.tag=${IMAGE_TAG} \
    --set image.pullPolicy=Always \
    --set-string server.grpcEndpoint=https://clustershell.clustershell.svc.cluster.local:8080 \
    --set server.tls.certSecretName=clustershell-server-tls \
    --set server.tls.clientCaSecretName=clustershell-server-client-ca \
    --set server.tls.clientTlsSecretName=clustershell-client-tls \
...
if [[ "${build_gateway}" == "1" ]]; then
  ...
  if cluster_exec "kubectl get statefulset/clustershell -n clustershell" >/dev/null 2>&1; then
    cluster_exec "kubectl rollout restart statefulset/clustershell -n clustershell"
    cluster_exec "kubectl rollout status statefulset/clustershell -n clustershell"
  elif cluster_exec "kubectl get deployment/clustershell -n clustershell" >/dev/null 2>&1; then
    cluster_exec "kubectl rollout restart deployment/clustershell -n clustershell"
    cluster_exec "kubectl rollout status deployment/clustershell -n clustershell"
  else
    echo "Warning: no clustershell workload found to roll out in namespace 'clustershell'."
  fi
  rollout_end=$(date +%s)
