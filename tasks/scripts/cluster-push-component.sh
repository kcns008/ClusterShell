...
SOURCE_IMAGE="clustershell/${component}:${IMAGE_TAG}"
TARGET_IMAGE="${IMAGE_REPO_BASE}/${component}:${IMAGE_TAG}"

source_candidates=(
  "clustershell/${component}:${IMAGE_TAG}"
  "localhost:5000/clustershell/${component}:${IMAGE_TAG}"
  "127.0.0.1:5000/clustershell/${component}:${IMAGE_TAG}"
)
...
if [ -z "${resolved_source_image}" ]; then
  ...
  resolved_source_image="clustershell/${component}:${IMAGE_TAG}"
fi
