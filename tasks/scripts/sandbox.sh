...
if clustershell sandbox get "${SANDBOX_NAME}" >/dev/null 2>&1; then
  ...
  if [[ -n "${deploy_fingerprint}" && "${deploy_fingerprint}" == "${previous_deploy_fingerprint}" ]]; then
    ...
    clustershell sandbox delete "${SANDBOX_NAME}" || true
  fi
fi
...
ensure_anthropic_provider() {
  ...
  if clustershell provider get anthropic >/dev/null 2>&1; then
...
if [[ "${need_create}" == "1" ]]; then
  ...
  clustershell sandbox create --name "${SANDBOX_NAME}" "${PROVIDER_ARGS[@]}" --tty -- "${CMD[@]}"
else
  echo "Connecting to existing sandbox '${SANDBOX_NAME}'..."
  clustershell sandbox connect "${SANDBOX_NAME}"
fi
