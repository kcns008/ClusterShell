...
ENV_FILE=.env
PUBLISHED_IMAGE_REPO_BASE_DEFAULT=ghcr.io/nvidia/clustershell
LOCAL_REGISTRY_CONTAINER=clustershell-local-registry
LOCAL_REGISTRY_ADDR=127.0.0.1:5000

if [ -n "${CI:-}" ] && [ -n "${CI_REGISTRY_IMAGE:-}" ]; then
  IMAGE_REPO_BASE_DEFAULT=${CI_REGISTRY_IMAGE}
elif [ "${MODE}" = "fast" ]; then
  IMAGE_REPO_BASE_DEFAULT=${LOCAL_REGISTRY_ADDR}/clustershell
else
  IMAGE_REPO_BASE_DEFAULT=${LOCAL_REGISTRY_ADDR}/clustershell
fi

IMAGE_REPO_BASE=${IMAGE_REPO_BASE:-${CLUSTERSHELL_REGISTRY:-${IMAGE_REPO_BASE_DEFAULT}}}
REGISTRY_HOST=${CLUSTERSHELL_REGISTRY_HOST:-${IMAGE_REPO_BASE%%/*}}
REGISTRY_NAMESPACE_DEFAULT=${IMAGE_REPO_BASE#*/}

if [ "${REGISTRY_NAMESPACE_DEFAULT}" = "${IMAGE_REPO_BASE}" ]; then
  REGISTRY_NAMESPACE_DEFAULT=clustershell
fi
...
CLUSTERSHELL_GATEWAY=${CLUSTERSHELL_GATEWAY:-${CLUSTER_NAME}}
GATEWAY_PORT=${RESOLVED_GATEWAY_PORT}

append_env_if_missing "GATEWAY_PORT" "${GATEWAY_PORT}"
append_env_if_missing "CLUSTERSHELL_GATEWAY" "${CLUSTERSHELL_GATEWAY}"

export CLUSTER_NAME
export GATEWAY_PORT
export CLUSTERSHELL_GATEWAY
...
ensure_local_registry() {
  ...
  echo "       Ensure a registry is running on port 5000 (e.g. docker run -d --name clustershell-local-registry -p 5000:5000 registry:2)." >&2
  docker ps -a >&2 || true
...
export CLUSTERSHELL_REGISTRY_HOST=${CLUSTERSHELL_REGISTRY_HOST:-${REGISTRY_HOST}}
export CLUSTERSHELL_REGISTRY_ENDPOINT=${CLUSTERSHELL_REGISTRY_ENDPOINT:-${REGISTRY_ENDPOINT_DEFAULT}}
export CLUSTERSHELL_REGISTRY_NAMESPACE=${CLUSTERSHELL_REGISTRY_NAMESPACE:-${REGISTRY_NAMESPACE_DEFAULT}}
export CLUSTERSHELL_REGISTRY_INSECURE=${CLUSTERSHELL_REGISTRY_INSECURE:-${REGISTRY_INSECURE_DEFAULT}}
...
if [ -n "${CI:-}" ] && [ -n "${CI_REGISTRY:-}" ] && [ -n "${CI_REGISTRY_USER:-}" ] && [ -n "${CI_REGISTRY_PASSWORD:-}" ]; then
  ...
  export CLUSTERSHELL_REGISTRY_USERNAME=${CLUSTERSHELL_REGISTRY_USERNAME:-${CI_REGISTRY_USER}}
  export CLUSTERSHELL_REGISTRY_PASSWORD=${CLUSTERSHELL_REGISTRY_PASSWORD:-${CI_REGISTRY_PASSWORD}}
fi
...
VOLUME_NAME="clustershell-cluster-${CLUSTER_NAME}"
...
if [ "${MODE}" = "fast" ]; then
  if docker inspect "${CONTAINER_NAME}" >/dev/null 2>&1 || docker volume inspect "${VOLUME_NAME}" >/dev/null 2>&1; then
    ...
    clustershell gateway destroy --name "${CLUSTER_NAME}"
  fi
fi
...
if [ -z "${CLUSTERSHELL_CLUSTER_IMAGE:-}" ]; then
  export CLUSTERSHELL_CLUSTER_IMAGE="clustershell/cluster:${IMAGE_TAG}"
fi

DEPLOY_CMD=(clustershell gateway start --name "${CLUSTER_NAME}" --port "${GATEWAY_PORT}")
