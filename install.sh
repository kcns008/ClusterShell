...
#   CLUSTERSHELL_VERSION     - Release tag to install (default: latest tagged release)
#   CLUSTERSHELL_INSTALL_DIR - Directory to install into (default: ~/.local/bin)
#
set -eu
...
resolve_version() {
  if [ -n "${CLUSTERSHELL_VERSION:-}" ]; then
    echo "$CLUSTERSHELL_VERSION"
    return 0
  fi
...
get_install_dir() {
  if [ -n "${CLUSTERSHELL_INSTALL_DIR:-}" ]; then
    echo "$CLUSTERSHELL_INSTALL_DIR"
  else
    echo "${HOME}/.local/bin"
  fi
}
