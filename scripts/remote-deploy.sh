...
# updates a gateway using `clustershell gateway start`.
...
Options:
  --remote-dir DIR            Remote checkout directory (default: clustershell)
  --name NAME                 Cluster name (default: clustershell)
  ...
  --cargo-version VERSION     Override CLUSTERSHELL_CARGO_VERSION for remote Docker builds
  --help                      Show this help
...
Examples:
  ...
  ./scripts/remote-deploy.sh my-sandbox -./scripts/remote-deploy.sh my-sandbox --remote-dir --name clustershell --port 8080 --recreate --plaintext
EOF
}
...
REMOTE_HOST=""
REMOTE_DIR=${REMOTE_DIR:-clustershell}
CLUSTER_NAME=${CLUSTER_NAME:-clustershell}
GATEWAY_PORT=${GATEWAY_PORT:-8080}
SSH_KEY="${SSH_KEY:-}"
IMAGE_TAG=${IMAGE_TAG:-dev}
CARGO_VERSION=${CLUSTERSHELL_CARGO_VERSION:-0.0.0-dev}
SKIP_SYNC=false
RECREATE=false
PLAINTEXT=false
...
echo "==> Building clustershell CLI..."
mise exec -- cargo build --release -p clustershell-cli
mkdir -p "$HOME/.local/bin"
install -m 0755 target/release/clustershell "$HOME/.local/bin/clustershell"

# Ensure `mise exec -- clustershell` uses the release binary rather than the local
# development shim, which expects git metadata that is not synced to the VM.
install -m 0755 target/release/clustershell scripts/bin/clustershell

# Prevent a stale repo-local .env from changing the deployment unexpectedly.
rm -f .env

echo "==> Building Docker images (tag=${IMAGE_TAG})..."
export CLUSTERSHELL_CARGO_VERSION="${CARGO_VERSION}"
export IMAGE_TAG
...
export CLUSTERSHELL_PUSH_IMAGES="clustershell/gateway:${IMAGE_TAG}"
...
mise exec -- clustershell "${start_args[@]}"
