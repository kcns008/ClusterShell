...
kubectl -n clustershell get statefulset/clustershell >/dev/null 2>&1 || exit 1
kubectl -n clustershell wait --for=jsonpath='{.status.readyReplicas}'=1 statefulset/clustershell --timeout=1s >/dev/null 2>&1 || exit 1

# ---------------------------------------------------------------------------
# Verify the sandbox supervisor binary exists on the node filesystem.
# Sandbox pods mount /opt/clustershell/bin as a read-only hostPath volume and
# exec /opt/clustershell/bin/clustershell-sandbox as their entrypoint. If the binary
...
# ---------------------------------------------------------------------------
if [ ! -x /opt/clustershell/bin/clustershell-sandbox ]; then
    echo "HEALTHCHECK_MISSING_SUPERVISOR: /opt/clustershell/bin/clustershell-sandbox not found" >&2
    exit 1
fi

# Verify TLS secrets exist (created by clustershell-bootstrap before the StatefulSet starts)
# Skip when TLS is disabled — secrets are not required.
if [ "${DISABLE_TLS:-}" != "true" ]; then
    kubectl -n clustershell get secret clustershell-server-tls >/dev/null 2>&1 || exit 1
    kubectl -n clustershell get secret clustershell-client-tls >/dev/null 2>&1 || exit 1
fi
