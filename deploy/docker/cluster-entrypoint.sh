...
# Entrypoint script for ClusterShell cluster image.
#
...
# These are stored in /opt/clustershell/charts/ because the volume mount
...
BUNDLED_CHARTS="/opt/clustershell/charts"
CHART_CHECKSUM=""
...
if [ -d "$BUNDLED_CHARTS" ]; then
    ...
    # Compute a checksum of the clustershell chart so we can inject it into the
    ...
    CLUSTERSHELL_CHART="$BUNDLED_CHARTS/clustershell-0.1.0.tgz"
    if [ -f "$CLUSTERSHELL_CHART" ]; then
        if command -v sha256sum >/dev/null 2>&1; then
            CHART_CHECKSUM=$(sha256sum "$CLUSTERSHELL_CHART" | cut -d ' ' -f 1)
        elif command -v shasum >/dev/null 2>&1; then
            CHART_CHECKSUM=$(shasum -a 256 "$CLUSTERSHELL_CHART" | cut -d ' ' -f 1)
        fi
    fi
fi

# Copy bundled manifests to k3s manifests directory.
# These are stored in /opt/clustershell/manifests/ because the volume mount
# on /var/lib/rancher/k3s overwrites any files baked into that path.
#
...
# We remove any clustershell-managed manifests that no longer exist in the
# bundled set so k3s does not keep installing removed components.
K3S_MANIFESTS="/var/lib/rancher/k3s/server/manifests"
BUNDLED_MANIFESTS="/opt/clustershell/manifests"
...
if [ -d "$BUNDLED_MANIFESTS" ]; then
    ...
    # Remove clustershell-managed manifests that are no longer bundled.
    # Only clean up files that look like clustershell manifests (clustershell-* or
    # envoy-gateway-* or agent-*) to avoid removing built-in k3s manifests.
    for existing in "$K3S_MANIFESTS"/clustershell-*.yaml \
...
if [ "${GPU_ENABLED:-}" = "true" ]; then
    ...
    GPU_MANIFESTS="/opt/clustershell/gpu-manifests"
...
HELMCHART="/var/lib/rancher/k3s/server/manifests/clustershell-helmchart.yaml"
...
# Inject SSH gateway host/port into the HelmChart manifest so the clustershell
...
if [ -f "$HELMCHART" ]; then
    ...
    if [ "${DISABLE_TLS:-}" = "true" ]; then
        ...
        # CLUSTERSHELL_GRPC_ENDPOINT when disableTls is true, so no sed needed here.
    else
...
# reach services on the Docker host via host.docker.internal / host.clustershell.internal.
