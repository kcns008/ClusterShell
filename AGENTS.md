...
This file is the primary instruction surface for agents contributing to ClusterShell. It is injected into your context on every interaction — keep that in mind when proposing changes to it.
...
ClusterShell is built agent-first. We design systems and use agents to implement them — this is not vibe coding. The product provides safe, sandboxed runtimes for autonomous AI agents, and the project itself is built using the same agent-driven workflows it enables.
...
|------|-----------|---------|
| `crates/clustershell-cli/` | CLI binary | User-facing command-line interface |
| `crates/clustershell-server/` | Gateway server | Control-plane API, sandbox lifecycle, auth boundary |
| `crates/clustershell-sandbox/` | Sandbox runtime | Container supervision, policy-enforced egress routing |
| `crates/clustershell-policy/` | Policy engine | Filesystem, network, process, and inference constraints |
| `crates/clustershell-router/` | Privacy router | Privacy-aware LLM routing |
| `crates/clustershell-bootstrap/` | Cluster bootstrap | K3s cluster setup, image loading, mTLS PKI |
| `crates/clustershell-core/` | Shared core | Common types, configuration, error handling |
| `crates/clustershell-providers/` | Provider management | Credential provider backends |
| `crates/clustershell-tui/` | Terminal UI | Ratatui-based dashboard for monitoring |
| `python/clustershell/` | Python SDK | Python bindings and CLI packaging |
...
- If you change cluster bootstrap infrastructure (e.g., `clustershell-bootstrap` crate, `deploy/docker/Dockerfile.images`, `cluster-entrypoint.sh`, `cluster-healthcheck.sh`, deploy logic in `clustershell-cli`), update the `debug-clustershell-cluster` skill in `.agents/skills/debug-clustershell-cluster/SKILL.md` to reflect those changes.
