...
from clustershell._proto import clustershell_pb2
from clustershell.sandbox import (
...
class _FakeStub:
    def __init__(self) -> None:
        self.request: clustershell_pb2.ExecSandboxRequest | None = None
    ...
    def ExecSandbox(
        ...
        _ = timeout
        yield clustershell_pb2.ExecSandboxEvent(
            exit=clustershell_pb2.ExecSandboxExit(exit_code=0)
        )
...
def test_exec_python_serializes_callable_payload() -> None:
    ...
    assert stub.request.environment["CLUSTERSHELL_PYFUNC_B64"]
...
def test_from_active_cluster_reads_gateway_metadata_layout(
    ...
    gateway_dir = tmp_path / "clustershell" / "gateways" / gateway_name
    mtls_dir = gateway_dir / "mtls"
    mtls_dir.mkdir(parents=True)
    (tmp_path / "clustershell" / "active_gateway").write_text(gateway_name)
    ...
    monkeypatch.delenv("CLUSTERSHELL_GATEWAY", raising=False)
...
def test_from_active_cluster_prefers_clustershell_gateway_env(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    ...
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    monkeypatch.setenv("CLUSTERSHELL_GATEWAY", gateway_name)
