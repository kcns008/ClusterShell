...
class SandboxClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._stub = clustershell_pb2_grpc.ClusterShellStub(self._channel)
    ...
    def from_active_cluster(
        ...
        *,
        ...
        if parsed.scheme == "https":
            mtls_dir = (
                _xdg_config_home() / "clustershell" / "gateways" / cluster_name / "mtls"
            )
            tls = TlsConfig(
    ...
    def health(self) -> clustershell_pb2.HealthResponse:
        return self._stub.Health(clustershell_pb2.HealthRequest(), timeout=self._timeout)
    ...
    def create(
        ...
        *,
        ...
        response = self._stub.CreateSandbox(
            clustershell_pb2.CreateSandboxRequest(spec=request_spec),
    ...
    def get(self, sandbox_name: str) -> SandboxRef:
        response = self._stub.GetSandbox(
            clustershell_pb2.GetSandboxRequest(name=sandbox_name),
            timeout=self._timeout,
        )
        return _sandbox_ref(response.sandbox)
    ...
    def list(self, *, limit: int = 100, offset: int = 0) -> builtins.list[SandboxRef]:
        response = self._stub.ListSandboxes(
            clustershell_pb2.ListSandboxesRequest(limit=limit, offset=offset),
            timeout=self._timeout,
        )
    ...
    def delete(self, sandbox_name: str) -> bool:
        response = self._stub.DeleteSandbox(
            clustershell_pb2.DeleteSandboxRequest(name=sandbox_name),
            timeout=self._timeout,
        )
    ...
    def exec_stream(
        ...
        *,
        ...
        request = clustershell_pb2.ExecSandboxRequest(
    ...
    def exec_python(
        ...
        *,
        ...
        exec_env["CLUSTERSHELL_PYFUNC_B64"] = _serialize_python_callable(
            function,
            args=args,
            kwargs=kwargs,
        )
...
_PYTHON_CLOUDPICKLE_BOOTSTRAP = (
    ...
    "payload=base64.b64decode(os.environ['CLUSTERSHELL_PYFUNC_B64']);"
...
def _default_spec() -> datamodel_pb2.SandboxSpec:
    ...
    # from /etc/clustershell/policy.yaml (baked into the image at build time).
...
def _resolve_active_cluster() -> str:
    env_gateway = os.environ.get("CLUSTERSHELL_GATEWAY")
    if env_gateway:
        return env_gateway
    active_file = _xdg_config_home() / "clustershell" / "active_gateway"
